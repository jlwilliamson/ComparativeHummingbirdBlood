...
date: "12/1/2020; last revised 2023-04-20"
...
This script also produces basic components of this figure, which were then read out and processed in Adobe Illustrator for more creative and formatting freedom. See annotations below for where 'final' R components were then manipulated in Illustrator. 
...
final <- read.csv("/Users/Jessie/Dropbox (MSBbirds)/Rdirectory/ComparativeHummingbirdBlood/AllHummingbirdBlood_Wrangled_withBioClim_andPCA_06-14-21.csv", stringsAsFactors = TRUE)
...
But note that it was originally run in HumBLood_DataWrangling.Rmd script...so it's kinda repetitive here. 
```{r}
...
# FINAL MAP VERSION: Simpler map with full extent of Peru and Chile, no spatial extent cropping, lat/lon boxes will be removed
THIS MAP IS THE FINAL VERSION INCLUDED IN OUR PAPER FIGURE 1! 
```{r}
...
# Version 1: magma-shaded elevs, "naked" map
...
# Version 1: grayscale-shaded elevs, "naked" map
...
## Other map versions, not included in our paper:


# MAP ALT VERSION 1: Cropped and bounded by extend w/ lat and lon labeled
(not used in paper)
```{r}
...
# MAP ALT VERSION 2: Simpler map with full extent of Peru and Chile, no spatial extent cropping, POINTS COLORED BY ELEV BIN
Create spatial points data frames binned by elevation
```{r}
# This would be more straightforward with a nice loop or two, but...it was also super simple to execute this clunkier way.
...
# Colors are awful...but are trial colors only
# But now I almost have *too* much information about elevation: map is shaded by elev AND sampling 
# points are colored by elev - it's a little much. 
# Other issue = many overlapping points obscure interesting sampling across elevations, so the colors don't convey as 
# much info as I'd wanted. And, as I discovered for OLWA-BHCO project, there's no way to jitter a spatial points df
...
# MAP ALT VERSION 3: All of South America, no cropping, non-Chile, non-Peru countries faded 
```{r}
# Peru and Chile sampling localities w/ rest of South America 
...
In this section, I create the elevational range portion of Figure 1 that's read into Illustrator and combined with the map components (from above) and phylogeny (from `HumBlood_Phylogeny.Rmd`) and other graphical elements (i.e., hummingbird illustrations, timescale, sampling numbers) to produce the final version of Figure 1 in our paper. 

# Phylo elev range, step 1: Calculate species mean elevational midpoints for plotting
Create basic species mean subset, order the subset by phylogeny order, calculate midpoints.
```{r, echo=FALSE,  message=FALSE, warning=FALSE}
# Create a basic species mean dataset
...
# I used to assign Clade names here, but now do that in `HumBlood_DataWrangling.Rmd`

# ARRANGE ORDER based on phylogeny branch order (for elev_range plotting)
species.means$phylo_order <- NA # Instantiate new col; capitalize this for easy and nice plot formatting

# COQUETTES 
# I specifically use weird decimals becuase it was the easiest way I could think of to keep the necessary order
species.means$phylo_order[species.means$species=="Phlogophilus harterti"] <- paste(.1) 
...
species.means$phylo_order[species.means$species=="Sephanoides sephaniodes"] <- paste(20)   
...
species.means$phylo_order[species.means$species=="Haplophaedia aureliae"] <- paste(39) 

# PATAGONA
species.means$phylo_order[species.means$species=="Patagona gigas"] <- paste(40) 
...
species.means$phylo_order[species.means$species=="Chaetocercus mulsant"] <- paste(46) 

# MOUNTAIN-GEMS
species.means$phylo_order[species.means$species=="Heliomaster longirostris"] <- paste(47)  
...
species.means$phylo_order[species.means$species=="Thalurania furcata"] <- paste(59)
...
species.means$phylo_order[species.means$species=="Schistes geoffroyi"] <- paste(64) 

# TOPAZES
species.means$phylo_order[species.means$species=="Florisuga mellivora"] <- paste(65) 
...
species.means$phylo_order[species.means$species=="Phaethornis ruber"] <- paste(77) 
...
# Bc midpoints will be plotted with correct numbers but on full elev x-axis scale, midpoints don't align properly w/in 
# associated elev ranges (i.e. if range = 1000-2000, midpoint=500, but point will be plotted AT 500 instead of 1,500)
# To correct for this, I'll add the elev_min "offset" to each midpoint, which makes its starting point the elev_min num
# After testing this out, things look correct: 
# All elev midpoints plot accurately at their midpoints within their elev range (!)
...
# I'm not quite sure why there are some NaNs in blood data
# Shouldn't be caused by species with <2 individuals because NaNs occur in species with e.g. 8 indivs
# And NaNs don't exist for elev data; just blood data
# REGARDLESS, this shouldn't matter because these data aren't used for downstream analyses 
# (when we do need species mean data, we calculate them on a trait-subset specific basis in modeling files)
```


# Phylo elev range, step 2: Add phylo_order as a variable to copy of `final` dataset 
This is just a step to be able to plot properly
```{r}
...
# COQUETTES
# As above, weird decimals were the only way to keep the order I needed
sampling$phylo_order[sampling$species=="Phlogophilus harterti"] <- paste(.1) 
...
sampling$phylo_order[sampling$species=="Sephanoides sephaniodes"] <- paste(20)  
...
sampling$phylo_order[sampling$species=="Haplophaedia aureliae"] <- paste(39) 

# PATAGONA
sampling$phylo_order[sampling$species=="Patagona gigas"] <- paste(40)
...
sampling$phylo_order[sampling$species=="Chaetocercus mulsant"] <- paste(46)  

# MOUNTAIN-GEMS
sampling$phylo_order[sampling$species=="Heliomaster longirostris"] <- paste(47)  
...
sampling$phylo_order[sampling$species=="Thalurania furcata"] <- paste(59) 
...
sampling$phylo_order[sampling$species=="Schistes geoffroyi"] <- paste(64) 

# TOPAZES
sampling$phylo_order[sampling$species=="Florisuga mellivora"] <- paste(65) 
...
# Plot phylo elev range data for Fig 1! 
This produces the raw phylo-ordered elev ranges for each species, with gray points representing sampling of single individuals along the gradient, and red diamonds representing species elevational midpoints. 
```{r}
#sort by phylo_order
...
# Species elevational ranges in phylogenetic order
p1 <- ggplot(data=species.means, aes(y=range, x=reorder(phylo_order, -phylo_order))) + # Add fill=Clade to color by clade 
      ...
      # 4/20/23: this note obvi from ENSM paper; I think translation is: 
            # "reorder phylo_order on x-axis by phylo_order descending
      # geom_bar(stat="identity", position=position_dodge(), width=0.2) + # trick: don't plot these! 
      # geom_line(aes(ymin=elev_min, ymax=elev_max)) +
      geom_errorbar(aes(ymin=elev_min, ymax=elev_max), width=0.5) + # specifies elev range; width controls error bar caps
      ...
      geom_point(aes(y=elevmidpt_offset, x=reorder(phylo_order, -phylo_order)), # ADD ELEVATION MIDPOINTs 
                 shape=18, color="firebrick1", size=1.4) +
      coord_flip() + # This flips x and y axes so plot is "vertical" orientation
...
# elev range layer - no resizing necessary (i.e., dimensions are ready to go into our Illustrator fig).
# Only modifications in Illustrator: Make elev range line widths 0.5 and make top tick marks black (vs. dark gray)  
# Remember that order of this figure matches phylogenetic order (see notes on this in the above 2 code chunks)


#####

# TRIAL AND ERROR NOTES ON PLOTTING ELEV RANGES: 
# geom_pointrange() plots the midpoint of your line segment IF the ymin and ymax are -/+ some number from the y variable
