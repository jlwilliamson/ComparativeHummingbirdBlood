...
Script to produce our sampling locality map and figure components associted with Figure XX, the 'phylo elev range map fig'. Note that all spatial data were wrangled and processed in HumBlood_DataWrangling.Rmd - this script includes only the code to load in those data and produce the map. 
...
# Attempts at making a much simpler map 


# Peru and Chile sampling localities w/ rest of South America 
# I don't want to contraint extent 
# no outlines of surrounding countries 
# Set much simpler extent for map (can crop out map boundaries, but extent seems non-negotiable )
ex.big <- extent(c(-86, -47.4, -47, 0.8)) # These go: xmin (left), xmax (right), ymin (bottom), ymax (top)

pdf("/Users/Jessie/Dropbox (MSBbirds)/Rdirectory/ComparativeHummingbirdBlood/TestMap_PeruChileOnly_gray.pdf", useDingbats = F)
plot(peru_chile_elev, col=colorRampPalette(c("grey20","grey80"))(100), cex.axis=1.5)
...
mapa <- borders("world", regions = c("Peru", "Chile"), fill = "grey70", colour = "black")
