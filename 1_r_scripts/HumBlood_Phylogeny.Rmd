...
t = time
D = # base pair differences
2 = number of diverging lineages
r = rate (~2% per million years, or 0.029)
```{r}
 # CALCULATE RATES 
...
# PREP KEEP.TIP DATA
# We want to create a pruned tree with just species tips for CeNS modeling. But, we split Patagona into PgigN and PgigS. 
# We don't want to combine them since functional differences were the basis for classifying as 2 different haplotypes. 
# Drop PgigS and change name of PgigN back to P gigas
cens.data.for.tree <- cens.ss.mean
cens.data.for.tree <- cens.data.for.tree[-which(cens.data.for.tree$species == "Patagona_gigas_S"),]
levels(cens.data.for.tree$species)[levels(cens.data.for.tree$species)=="Patagona_gigas_N"] <- "Patagona_gigas"

# Check data structures 
# To use keep.tip, variable "species" MUST BE A CHARACTER, not a factor! 
str(cens.data.for.tree)
cens.data.for.tree$species <- as.character(cens.data.for.tree$species)

# Prune CeNS sub-tree
tree.cens <- keep.tip(tree.final, cens.data.for.tree$species)

# Check to make sure things look ok
plot(tree.cens)
class(tree.cens)
tree.cens$tip.label # fantastic; just the 29 tip labels we want 
