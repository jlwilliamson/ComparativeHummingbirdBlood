...
date: "7/19/2020; last revised 2023-04-20"
output:
  md_document:
...
Script to process phylogeny for 'Hummingbird blood traits track oxygen availability across space and time'
...
This will get read into several analysis scripts for modeling.
...
final <- read.csv("AllHummingbirdBlood_Wrangled_withBioClim_andPCA_06-14-21.csv", stringsAsFactors = FALSE)
final <- final[ , !(colnames(final) %in% c("X"))] # drop weird X column 1 if reading in from read.csv

# (These datasets are made in modeling scripts)
...
# I sorted by "drop" in Excel, concatenated to include parenthesis and proper commas, then pasted values here 
```


# Prune master McGuire tree to desired species subset (77 tips)
```{r}
# So...this is clunky, but manually select all tip labels you want to drop
# Variable names make this tricky and hard to automate
...
# Add in missing species (*Oreotrochilus leucopleurus* and *Phaethornis stuarti*) and adjust branch lengths
```{R}
...
# Oreotrochilus leucopleurus is sister to Oreotrochilus estella (per Sornoza-Molina et al. 2018)
...
- *Oreotrochilus* is  good and reflects current taxonomy of Sornoza-Molina et al. 2018, Auk 
- My way of doing this for *Phaethornis stuarti* is clunky. McGuire doesn't have *stuarti*, and the only other published *Phaethornis* phylogeny I could find was from 1997 (and looked outdated). BirdTree puts *ruber* sister to *atrimentalis* with *stuarti* on an earlier and longer branch. 
...
- THEN, I adjust branch lengths to retain McGuire estimates for the ruber-atrimentalis split and align tip labels. 
...
# Now switch atrimentalis and stuarti to be consistent with BirdTree estimation of relationships
...
tree.prune3[[2]][149] <- 0.5965517 # Branch  to O. estella and melanogaster (estimate tweaked from S-M et al. 2018)
# Notes on this last one: adjust this to get tips aligned. 
# This was Originally 0.7480506 in McGuire; I calculated melanogaster-estella/leucopleurus rate (t=0.0288/(2*0.029)) from # Sornoza-Molina, which gave me 0.4965517 and then I bumped this up to 0.5965517 to make tips flush.
...
# Make blood trait-specific tree subsets and save subset tree files
```{r}
...
# MAKE A SPECIES MEAN TREE WITH PGIGN AND PGIGS (*Patagona gigas* north and *Patagona gigas* south)
Because we'll analyze Patagona_gigas_S and Patagona_gigas_N as functionally different lineages, add in Patagona_gigas_S to final pruned tree and adjust branch lengths to reflect known percent divergence based on our preliminary ND2 data. 
(This is used for CeNS analysis)
```{r}
...
# This is ~7.3% divergence; ~72/973 bp of ND2 (based in Williamson et al., In Prep)
