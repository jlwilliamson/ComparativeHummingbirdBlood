...
date: "4/26/2021; last revised 1/23/22"
...
**2) CeNS index:** 
    ...
    - Take these estimates and run another model: `cens.index ~ mass + elev + (1|phylo)` to see what predicts this index 
...
# We know MCV has heavy tails, responds well to skew_normal, but *might* fit better with mcv.log variable
# we'll see below.
...
cens.il.m2.unstandardized <- 
  update(cens.il.m2,
        ...
        chains = 4, cores = 4)
save(cens.il.m2.unstandardized, file="cens.il.m3.unstandardized_Predictors_SpeciesRE.RData") # save model
})
...
# FIGURE S1: Species-specific Conditional effects plots 
...
JESSIE NOTE: Would be cool to figure out how to get horizontal lines at zero on all plots as an x-axis marker (or set axes to all scame scale)
...
# HB GENOTYPE DOES NOT AFFECT CENS INDEX - Figure S5 in paper; see the polished version in Plots & Figs script
...
ggsave(FigureS5, filename="CensIndex_vs_Genotype_2021_07-27.pdf", height=7, width=9, units="in")
...
Wwhat's interesting here is that after modeling this connection between blood parameters, we actually find *the opposite* of what we do by analyzing the raw data: That is; we do *not* see the concave/flipped parabola shape, which suggests that middle-elev species are increasing blood-O2 carrying capacity through higher MCV; instead what we find is that middle elev species are primarily increasing blood-O2 carrying capacity through increases in TRBC. 
...
In addition to the above contmaps and tests of phylo signal, I ran a first pass at models w/ phylogeny incorporated. Buerkner's hypothesis test supported lack of phylo signal, and only ~4% of the variance was explained by phylogeny. Since this is a poor predictor, remove this from our models 
```{r}
...
# load("cens.m1_CeNS_Intercept-Only.Rdata") 
...
save(cens.m3, file="cens.m3_CeNS.RData") # save model
load("cens.m3_CeNS.RData") # If loading from pre-saved file and not re-running
...
save(cens.m4, file="cens.m4_CeNS.RData") # save model
# load("cens.m4_CeNS.RData") # If loading from pre-saved file and not re-running
})
```
...
# load("cens.m5_CeNS_ReducedModel.RData") # If loading from pre-saved file and not re-running
...
- Strong relationship between CeNS index and elevation: middle elevation species appear to adjust O2 carrying capacity through more cells while lower and high elev species adjust O2 carrying capacity through increases to cell size (we provide detailed analysis of this in our paper). 
