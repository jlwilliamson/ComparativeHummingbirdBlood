---
title: "Comparative Hummingbird Blood: Cell Number vs. Size Analysis"
author: "Jessie Williamson"
date: "4/26/2021; last revised 2023-04-20"
output: html_document
---

Script to try to get to the bottom of 'cell number versus cell size tradeoff' plus creation of the *CeNS* Index. 
...
**2) CeNS index:** 
    ...
    - Take these estimates and run another model: `cens.index ~ mass + elev + (1|phylo)` to see what predicts the index 
...
# Load functions 
source("/Users/Jessie/MSBbirds Dropbox/Jessie Williamson/Rdirectory/ComparativeHummingbirdBlood/1_r_scripts/ada_functions.R") 
...
final <- read.csv("ComparativeHummingbirdBlood_FinalDataset_ForModeling_06-14-21.csv", stringsAsFactors = FALSE)
...
# We know MCV has heavy tails, responds well to skew_normal, but *might* fit better with mcv.log variable; we'll see below

# Standardize predictors 
censdat$mcv.z <- standardize(censdat$mcv)
censdat$trbc.z <- standardize(censdat$trbc)
censdat$mchc.z <- standardize(censdat$mchc)
...
Starting to feel like this has been done repetitively across scripts...again, was a little hard to keep track of in the 2 years of making this paper, so bear with me. 
```{r}
cens.sp <- censdat # Make the final dataset copy that you'll use to wrangle species mean data 
str(cens.sp)

# ## COMMENT DURING REVIEW --this came up: Why does this dataset have a minimum of 3 individuals, when the cutoff was 2 individuals for other 
# among-species blood models? Let's do a quick test to see how many more species we retain if using a cut-off of 2 individuals. 
# cens.two.test <- subset(cens.sp, with(cens.sp, species %in% names(which(table(species)>=2))))
# 
# # species mean summary - should be no NAs here
# cens.two.test <- cens.two.test %>% group_by(species) %>% summarise(hb = mean(hb),
#                                                   elev = mean(elev),
#                                                   mass = mean(mass),
#                                                   wl = mean(wl),
#                                                   mcv = mean(mcv),
...
# Now, continue with dropping species with fewer than 3 records 
...
## Quick comparison of 'cens.two.test' (min 2 individuals) vs 'cens.sp' (min 3 individuals) indicates that less conservatively using the 
## threshold of 2 only yields 3 more species (i.e., cens.two.test has 51 total species, while cens.sp has 48 species.)
## Let's proceed with the more conservative 3. 
## The three species cut if using the 3 minimum cut-off are: Chalcostigma herrani, Ensifera ensifera, and Eriocnemis aline
...
```{r}
# Need to ID min number of indiv/species necessary to run models - we'll use 8 individuals.
...
# Get dataset counts 
sp.count.cens <- censdat %>% group_by(species) %>% summarise(count = length(nk)); sp.count.cens # 59 species in indiv-level models
...
# Check distributions 
p <- ggpairs(subset(censdat, select = c(hb, mcv, trbc, mchc))); print(p)
```
...
cor(censdat[,c("hb", "mcv", "trbc", "mchc")]) 

#           hb         mcv       trbc        mchc
# hb   1.0000000  0.12852782  0.2621327  0.33061677
# mcv  0.1285278  1.00000000 -0.8447446 -0.07050376
# trbc 0.2621327 -0.84474462  1.0000000 -0.10779970
# mchc 0.3306168 -0.07050376 -0.1077997  1.00000000

# This describes what we know: that TRBC is highly negatively correlated w/ MCV. BUT, these are measurements describing
# very different things and both affect Hb, so we leave them in. 

# Correlation matrix - species mean data 
cor(cens.sp[,c("hb", "mcv", "trbc", "mchc")])

#             hb         mcv        trbc        mchc
# hb   1.0000000  0.16266075  0.25105739  0.33003917
# mcv  0.1626607  1.00000000 -0.86430646 -0.03962787
# trbc 0.2510574 -0.86430646  1.00000000 -0.08447571
# mchc 0.3300392 -0.03962787 -0.08447571  1.00000000

# Same as above, only notable correlation is strong negative corrrelation between MCV and TRBC
```


# Calculate coefficients of variation (cv)
```{r}
# Coefficient of variation = standard deviation/mean; measure of relative variability

# Individual-level data 
cv(censdat$hb) # 0.08862862
cv(censdat$mcv) # 0.1759373
cv(censdat$trbc) # 0.1766653
cv(censdat$mchc) # 0.05978773
cv(censdat$mass) # 0.5154195 # High cv for mass

# Species mean data 
cv(cens.sp$hb) # 0.0494299
cv(cens.sp$mcv) # 0.1033924
cv(cens.sp$trbc) # 0.1061705
cv(cens.sp$mchc) # 0.03048832
cv(cens.sp$mass) # 0.5180711 # For perspective, cv is much, much higher for a trait like mass (which we expect.)
```

Coefficients of variation in individual-level data: Hb (0.08862862), MCV (0.1759373), TRBC (0.1766653), MCHC (0.05978773). Coefficients of variation in species mean data: Hb (0.0494299), MCV (0.1033924), TRBC (0.1061705), MCHC (0.03048832). 
...
Note: We don't want intercept only + (1|species) model, as this complicates comparison w/ the species mean dataset. 
(though note that we originally tried this)
...
save(cens.il.m1, file="Rdata_files/cens.il.m1_InterceptOnly.RData") # save model
#load("Rdata_files/cens.il.m1_InterceptOnly.RData") # If loading from pre-saved file and not re-running
...
save(cens.il.m2, file="Rdata_files/cens.il.m2_Predictors_SpeciesRE.RData") # save model
#load("Rdata_files/cens.il.m2_Predictors_SpeciesRE.RData")
})
...
# Cens individual-level: Plot outputs of posterior mean estimates and 95% credible intervals (full models, m1-m6)
```{r}
...
cens.il.m2.p1 <- mcmc_plot(cens.il.m2, variable=c("b_mcv.z", "b_trbc.z"),
...
cens.il.m2.p1
ggsave(cens.il.m2.p1, filename = "cens.il.m2.p1_PosteriorEstimates&CredibleIntervals_07-15-21.pdf", bg="transparent", height=7, width=9, units="in")
...
cens.il.m2.unstandardized <- 
  update(cens.il.m2,
        newdata = censdat,
        formula = hb ~ 1 + mcv + trbc + mchc + (1|species),
        chains = 4, cores = 4)
save(cens.il.m2.unstandardized, file="Rdata_files/cens.il.m3.unstandardized_Predictors_SpeciesRE.RData") # save model
})
# load("Rdata_files/cens.il.m2.unstandardized_Predictors_SpeciesRE.RData")
...
- Model 2, full model + species grouping variable fits best by var! Variance explained by species: 14%. R^2 = 0.85. 
...
Also might suggest that TRBC increases happen faster than MCV increases? 
...
save(cens.sm.m1, file="Rdata_files/cens.sm.m1_InterceptOnly.RData") # save model
#load("Rdata_files/cens.sm.m1_InterceptOnly.RData") # If loading from pre-saved file and not re-running
...
save(cens.sm.m2, file="Rdata_files/cens.sm.m2_Predictors_SpeciesRE.RData") # save model
#load("Rdata_files/cens.sm.m2_Predictors_SpeciesRE.RData")
...
#pairs(hb.m3)
color_scheme_set("viridisD") # nice default
cens.sm.m2.p1 <- mcmc_plot(cens.sm.m2, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
cens.sm.m2.unstandardized <-
  update(cens.sm.m2,
        ...
        chains = 4, cores = 4)
save(cens.sm.m2.unstandardized, file="Rdata_files/cens.sm.m2.unstandardized_Predictors_SpeciesRE.RData") # save model
})
#load("Rdata_files/cens.sm.m2.unstandardized_Predictors_SpeciesRE.RData")
...
save(cens.Amel, file="Rdata_files/cens.Amel.RData") # save model
...
save(cens.Acas, file="Rdata_files/cens.Acas.RData") # save model
...
save(cens.Akin, file="Rdata_files/cens.Akin.RData") # save model
...
save(cens.Aama, file="Rdata_files/cens.Aama.RData") # save model
...
save(cens.Clar, file="Rdata_files/cens.Clar.RData") # save model
...
save(cens.Coen, file="Rdata_files/cens.Coen.RData") # save model
...
save(cens.Ccoe, file="Rdata_files/cens.Ccoe.RData") # save model
...
save(cens.Ciri, file="Rdata_files/cens.Ciri.RData") # save model
...
save(cens.Cvio, file="Rdata_files/cens.Cvio.RData") # save model
...
save(cens.Ccor, file="Rdata_files/cens.Ccor.RData") # save model
...
save(cens.Eluc, file="Rdata_files/cens.Eluc.RData") # save model
...
save(cens.Econ, file="Rdata_files/cens.Econ.RData") # save model
...
save(cens.Fmel, file="Rdata_files/cens.Fmel.RData") # save model
...
save(cens.Ghir, file="Rdata_files/cens.Ghir.RData") # save model
...
save(cens.Hame, file="Rdata_files/cens.Hame.RData") # save model
...
save(cens.Hmic, file="Rdata_files/cens.Hmic.RData") # save model
...
save(cens.Hlea, file="Rdata_files/cens.Hlea.RData") # save model
...
save(cens.Llaf, file="Rdata_files/cens.Llaf.RData") # save model
...
save(cens.Lnun, file="Rdata_files/cens.Lnun.RData") # save model
...
save(cens.Lvic, file="Rdata_files/cens.Lvic.RData") # save model
...
save(cens.Meup, file="Rdata_files/cens.Meup.RData") # save model
...
save(cens.Mpho, file="Rdata_files/cens.Mpho.RData") # save model
...
save(cens.Mtyr, file="Rdata_files/cens.Mtyr.RData") # save model
...
save(cens.Ound, file="Rdata_files/cens.Ound.RData") # save model
...
save(cens.Oest, file="Rdata_files/cens.Oest.RData") # save model
...
save(cens.Omel, file="Rdata_files/cens.Omel.RData") # save model
...
save(cens.Pgign, file="Rdata_files/cens.Pgign.RData") # save model
...
save(cens.Pgigs, file="Rdata_files/cens.Pgigs.RData") # save model
...
save(cens.Pguy, file="Rdata_files/cens.Pguy.RData") # save model
...
save(cens.Pmal, file="Rdata_files/cens.Pmal.RData") # save model
...
save(cens.Pcya, file="Rdata_files/cens.Pcya.RData") # save model
...
save(cens.Tfur, file="Rdata_files/cens.Tfur.RData") # save model
...
Send to job - takes longer than I'd anticipate to load in data; may just want to try loading individually
```{R}
job::job({
load("Rdata_files/cens.Amel.RData")
load("Rdata_files/cens.Acas.RData")
load("Rdata_files/cens.Akin.RData")
load("Rdata_files/cens.Aama.RData")
load("Rdata_files/cens.Clar.RData")
load("Rdata_files/cens.Coen.RData")
load("Rdata_files/cens.Ccoe.RData")
load("Rdata_files/cens.Ciri.RData")
load("Rdata_files/cens.Cvio.RData")
load("Rdata_files/cens.Ccor.RData")
load("Rdata_files/cens.Eluc.RData")
load("Rdata_files/cens.Econ.RData")
load("Rdata_files/cens.Fmel.RData")
load("Rdata_files/cens.Ghir.RData")
load("Rdata_files/cens.Hame.RData")
load("Rdata_files/cens.Hmic.RData")
load("Rdata_files/cens.Hlea.RData")
load("Rdata_files/cens.Llaf.RData")
load("Rdata_files/cens.Lnun.RData")
load("Rdata_files/cens.Lvic.RData")
load("Rdata_files/cens.Meup.RData")
load("Rdata_files/cens.Mpho.RData")
load("Rdata_files/cens.Mtyr.RData")
load("Rdata_files/cens.Ound.RData")
load("Rdata_files/cens.Oest.RData")
load("Rdata_files/cens.Omel.RData")
load("Rdata_files/cens.Pgign.RData")
load("Rdata_files/cens.Pgigs.RData")
load("Rdata_files/cens.Pguy.RData")
load("Rdata_files/cens.Pmal.RData")
load("Rdata_files/cens.Pcya.RData")
load("Rdata_files/cens.Tfur.RData")
})
```
...
# FIGURE S2: Species-specific Conditional effects plots 
May want to move this to plots and figs script but leaving here now while I'm figuring stuff out. 
```{r}
...
cens.Amel.p1 <- mcmc_plot(cens.Amel, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
#labs(x="", subtitle = expression(paste(italic("A. melanogenys")))) + 
ggtitle(expression(paste(bold("A"),~~~~italic("A. melanogenys")))) + 
  theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
  # This makes panel header bold 
...
cens.Acas.p1 <- mcmc_plot(cens.Acas, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("B"),~~~~italic("A. castelnaudii")))) + 
  theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
...
cens.Akin.p1 <- mcmc_plot(cens.Akin, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("C"),~~~~italic("A. kingii")))) + 
  theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Aama.p1 <- mcmc_plot(cens.Aama, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("D"),~~~~italic("A. amazilia")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Clar.p1 <- mcmc_plot(cens.Clar, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "C. largipennis") + 
ggtitle(expression(paste(bold("E"),~~~~italic("C. largipennis")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Coen.p1 <- mcmc_plot(cens.Coen, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "C. oenone") +
ggtitle(expression(paste(bold("F"),~~~~italic("C. oenone")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Ccoe.p1 <- mcmc_plot(cens.Ccoe, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "C. coeligena") + 
ggtitle(expression(paste(bold("C"),~~~~italic("C. coeligena")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Ciri.p1 <- mcmc_plot(cens.Ciri, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("H"),~~~~italic("C. iris")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Cvio.p1 <- mcmc_plot(cens.Cvio, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("I"),~~~~italic("C. violifer")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("MCV","TRBC")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Ccor.p1 <- mcmc_plot(cens.Ccor, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "C. coruscans") + 
ggtitle(expression(paste(bold("J"),~~~~italic("C. coruscans")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Eluc.p1 <- mcmc_plot(cens.Eluc, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "C. luciani") + 
ggtitle(expression(paste(bold("K"),~~~~italic("C. luciani")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Econ.p1 <- mcmc_plot(cens.Econ, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("L"),~~~~italic("C. condamini")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Fmel.p1 <- mcmc_plot(cens.Fmel, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("M"),~~~~italic("F. mellivora")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Ghir.p1 <- mcmc_plot(cens.Ghir, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("N"),~~~~italic("G. hirsutus")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Hame.p1 <- mcmc_plot(cens.Hame, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("O"),~~~~italic("H. amethysticollis")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Hmic.p1 <- mcmc_plot(cens.Hmic, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "H. micraster") +
ggtitle(expression(paste(bold("P"),~~~~italic("H. micraster")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Hlea.p1 <- mcmc_plot(cens.Hlea, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("Q"),~~~~italic("H. leadbeateri")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("MCV","TRBC")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Llaf.p1 <- mcmc_plot(cens.Llaf, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("R"),~~~~italic("L. lafresnayi")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Lnun.p1 <- mcmc_plot(cens.Lnun, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("S"),~~~~italic("L. nuna")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Lvic.p1 <- mcmc_plot(cens.Lvic, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("T"),~~~~italic("L. victoriae")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Meup.p1 <- mcmc_plot(cens.Meup, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") +
ggtitle(expression(paste(bold("U"),~~~~italic("M. eupogon")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Mpho.p1 <- mcmc_plot(cens.Mpho, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") +
ggtitle(expression(paste(bold("V"),~~~~italic("M. phoebe")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Mtyr.p1 <- mcmc_plot(cens.Mtyr, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("W"),~~~~italic("M. tyrianthina")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Ound.p1 <- mcmc_plot(cens.Ound, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="", title = "") + 
ggtitle(expression(paste(bold("X"),~~~~italic("O. underwoodii")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Oest.p1 <- mcmc_plot(cens.Oest, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") +
ggtitle(expression(paste(bold("Y"),~~~~italic("O. estella")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("MCV","TRBC")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Omel.p1 <- mcmc_plot(cens.Omel, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("Z"),~~~~italic("O. melanogaster")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
# Pgign
cens.Pgign.p1 <- mcmc_plot(cens.Pgign, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("AA"),~~~~italic("P. gigas (North)")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
# Pgig s
cens.Pgigs.p1 <- mcmc_plot(cens.Pgigs, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("AB"),~~~~italic("P. gigas (South)")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Pguy.p1 <- mcmc_plot(cens.Pguy, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("AC"),~~~~italic("P. guy")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Pmal.p1 <- mcmc_plot(cens.Pmal, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("AD"),~~~~italic("P. malaris")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Pcya.p1 <- mcmc_plot(cens.Pcya, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("AE"),~~~~italic("P. cyanopterus")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
...
cens.Tfur.p1 <- mcmc_plot(cens.Tfur, variable=c("b_mcv.z", "b_trbc.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.89, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "") + 
ggtitle(expression(paste(bold("AF"),~~~~italic("T. furcata")))) + 
        theme(plot.title.position = "plot", # "plot" = you want "title" flush with y-axis
        plot.title = element_text(face="bold")) +
scale_y_discrete(labels=c("","x")) +
            theme(plot.title = element_text(family = "Helvetica", color="black",size=9)) + # font = panel header size
            theme(text = element_text(family="Helvetica",size=9)) # y axis labels
            # ggtitle("A") + # Assign panel number/header
            # theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
...
FigS1_PosteriorProbs <- (cens.Amel.p1 + cens.Acas.p1 + cens.Akin.p1 + cens.Aama.p1 + cens.Clar.p1 + cens.Coen.p1 + 
...
plot(FigS1_PosteriorProbs)
ggsave(FigS1_PosteriorProbs, filename = "FigS1_CeNS_Species-Specific_PosteriorProbs_2022-01-22.pdf", bg="transparent", height=7, width=12, units="in")
```

Would be cool to figure out how to get horizontal lines at zero on all plots as an x-axis marker (or set axes to all scame scale)...
...
cens.R2
length(cens.R2)
write.csv(cens.R2, "cens.R2_Values_06-14-21.csv")
```
...
cens.trbc
length(cens.trbc)
write.csv(cens.trbc, "cens.trbc_Values_06-14-21.csv")
...
cens.mcv
length(cens.mcv)
write.csv(cens.mcv, "cens.mcv_Values_06-14-21.csv")
```


# CALCULATE UNSTANDARDIZED CENS COEFFICIENTS FOR REAL-WORLD ESTIMATES 
Trying to isolate whether variation in CeNS index is caused by sensitivity or variation. First step is to get estimates from unstandardized models and to plot these against CeNS coefficients. 
```{r}
job::job({ # Send this to a job so you can use the console while it runs
  #Note that job() is kind of wonky, sometimes doesn't run properly, & sometimes produces errors/doesn't save right models
...
save(cens.unst.Amel, file="Rdata_files/cens.unst.Amel.RData") # save model
...
save(cens.unst.Acas, file="Rdata_files/cens.unst.Acas.RData") # save model
...
save(cens.unst.Akin, file="Rdata_files/cens.unst.Akin.RData") # save model
...
save(cens.unst.Aama, file="Rdata_files/cens.unst.Aama.RData") # save model
...
save(cens.unst.Clar, file="Rdata_files/cens.unst.Clar.RData") # save model
...
save(cens.unst.Coen, file="Rdata_files/cens.unst.Coen.RData") # save model
...
save(cens.unst.Ccoe, file="Rdata_files/cens.unst.Ccoe.RData") # save model
...
save(cens.unst.Ciri, file="Rdata_files/cens.unst.Ciri.RData") # save model
...
save(cens.unst.Cvio, file="Rdata_files/cens.unst.Cvio.RData") # save model
...
save(cens.unst.Ccor, file="Rdata_files/cens.unst.Ccor.RData") # save model
...
save(cens.unst.Eluc, file="Rdata_files/cens.unst.Eluc.RData") # save model
...
save(cens.unst.Econ, file="Rdata_files/cens.unst.Econ.RData") # save model
...
save(cens.unst.Fmel, file="Rdata_files/cens.unst.Fmel.RData") # save model
...
save(cens.unst.Ghir, file="Rdata_files/cens.unst.Ghir.RData") # save model
...
save(cens.unst.Hame, file="Rdata_files/cens.unst.Hame.RData") # save model
...
save(cens.unst.Hmic, file="Rdata_files/cens.unst.Hmic.RData") # save model
...
save(cens.unst.Hlea, file="Rdata_files/cens.unst.Hlea.RData") # save model
...
save(cens.unst.Llaf, file="Rdata_files/cens.unst.Llaf.RData") # save model
...
save(cens.unst.Lnun, file="Rdata_files/cens.unst.Lnun.RData") # save model
...
save(cens.unst.Lvic, file="Rdata_files/cens.unst.Lvic.RData") # save model
...
save(cens.unst.Meup, file="Rdata_files/cens.unst.Meup.RData") # save model
...
save(cens.unst.Mpho, file="Rdata_files/cens.unst.Mpho.RData") # save model
...
save(cens.unst.Mtyr, file="Rdata_files/cens.unst.Mtyr.RData") # save model
...
save(cens.unst.Ound, file="Rdata_files/cens.unst.Ound.RData") # save model
...
save(cens.unst.Oest, file="Rdata_files/cens.unst.Oest.RData") # save model
...
save(cens.unst.Omel, file="Rdata_files/cens.unst.Omel.RData") # save model
...
save(cens.unst.Pgign, file="Rdata_files/cens.unst.Pgign.RData") # save model
...
save(cens.unst.Pgigs, file="Rdata_files/cens.unst.Pgigs.RData") # save model
...
save(cens.unst.Pguy, file="Rdata_files/cens.unst.Pguy.RData") # save model
...
save(cens.unst.Pmal, file="Rdata_files/cens.unst.Pmal.RData") # save model
...
save(cens.unst.Pcya, file="Rdata_files/cens.unst.Pcya.RData") # save model
...
save(cens.unst.Tfur, file="Rdata_files/cens.unst.Tfur.RData") # save model
...
```{R}
job::job({
load("Rdata_files/cens.unst.Amel.RData")
load("Rdata_files/cens.unst.Acas.RData")
load("Rdata_files/cens.unst.Akin.RData")
load("Rdata_files/cens.unst.Aama.RData")
load("Rdata_files/cens.unst.Clar.RData")
load("Rdata_files/cens.unst.Coen.RData")
load("Rdata_files/cens.unst.Ccoe.RData")
load("Rdata_files/cens.unst.Ciri.RData")
load("Rdata_files/cens.unst.Cvio.RData")
load("Rdata_files/cens.unst.Ccor.RData")
load("Rdata_files/cens.unst.Eluc.RData")
load("Rdata_files/cens.unst.Econ.RData")
load("Rdata_files/cens.unst.Fmel.RData")
load("Rdata_files/cens.unst.Ghir.RData")
load("Rdata_files/cens.unst.Hame.RData")
load("Rdata_files/cens.unst.Hmic.RData")
load("Rdata_files/cens.unst.Hlea.RData")
load("Rdata_files/cens.unst.Llaf.RData")
load("Rdata_files/cens.unst.Lnun.RData")
load("Rdata_files/cens.unst.Lvic.RData")
load("Rdata_files/cens.unst.Meup.RData")
load("Rdata_files/cens.unst.Mpho.RData")
load("Rdata_files/cens.unst.Mtyr.RData")
load("Rdata_files/cens.unst.Ound.RData")
load("Rdata_files/cens.unst.Oest.RData")
load("Rdata_files/cens.unst.Omel.RData")
load("Rdata_files/cens.unst.Pgign.RData")
load("Rdata_files/cens.unst.Pgigs.RData")
load("Rdata_files/cens.unst.Pguy.RData")
load("Rdata_files/cens.unst.Pmal.RData")
load("Rdata_files/cens.unst.Pcya.RData")
load("Rdata_files/cens.unst.Tfur.RData")
})
```
...
#write.csv(cens.unst.trbc, "cens.unst.trbc_Values_06-14-21.csv")
...
#write.csv(cens.unst.mcv, "cens.unst.mcv_Values_06-14-21.csv")
```


# Make a CeNS Prediction modeling dataset 
```{r}
# If they aren't already read in, read in the vectors/dataframes of model coefficients and R^2 values
cens.R2 <- read.csv("cens.R2_Values_06-14-21.csv", stringsAsFactors = TRUE) # R^2 values from standardized CeNS models
names(cens.R2)[names(cens.R2) == "x"] <- "cens.R2"
cens.trbc <- read.csv("cens.trbc_Values_06-14-21.csv", stringsAsFactors = TRUE) # standardized TRBC coefficients
names(cens.trbc)[names(cens.trbc) == "x"] <- "cens.trbc"
cens.mcv <- read.csv("cens.mcv_Values_06-14-21.csv", stringsAsFactors = TRUE) # standardized MCV coefficients
names(cens.mcv)[names(cens.mcv) == "x"] <- "cens.mcv"
cens.unst.trbc <- read.csv("cens.unst.trbc_Values_06-14-21.csv", stringsAsFactors = TRUE) # unstandardized TRBC coef
names(cens.unst.trbc)[names(cens.unst.trbc ) == "x"] <- "trbc.unst" # Make this column header simpler
cens.unst.mcv <- read.csv("cens.unst.mcv_Values_06-14-21.csv", stringsAsFactors = TRUE) # unstandardized MCV coef
names(cens.unst.mcv)[names(cens.unst.mcv ) == "x"] <- "mcv.unst" # Make this column header simpler

model.indices <- cbind(cens.R2, cens.trbc, cens.mcv, cens.unst.trbc, cens.unst.mcv) # Combine all 
model.indices <- model.indices[, !(colnames(model.indices) %in% c("X"))] # Drop weird X columns 
...
# NEED TO USE CENS.SS for this because it contains all individual data for the pared-down species subset used for
# species-specific CeNS modeling (i.e., cens.sp is a mean dataset but has too many species in it)
...
cens.ss.mean <- cbind(cens.ss.mean, model.indices, cens.genotypes)
...
cens.ss.mean$mcv.z <- standardize(cens.ss.mean$mcv)
cens.ss.mean$trbc.z <- standardize(cens.ss.mean$trbc)
...
#write.csv(cens.ss.mean, "cens.ss.mean_ForCeNSmodeling_06-14-21.csv")
```
...
# IF JUST PLOTTING AND RUNNING CENS PREDICTION MODELS, READ IN DATA HERE! 
Otherwise you risk R crashing when reloading all cumbersome models and files. 
```{r}
...
Note that these are some quick diagnostic plots; final *CeNS* plot versions are in `HumBlood_Plots&Figs.Rmd`. 
```{r}
...
ggsave(p6, filename="CeNSindex_by_WingLoading_06-14-21.pdf", height=7, width=9, units="in")
...
Diagnostics to see how things are correlated. 
```{r}
...
(p <- ggplot(cens.ss.mean, aes(x=trbc, y=cens.index)) + # Had previously colored by MCV
 ...
 # geom_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1, colour="black", size=1.0) + # quadratic 
  ...
  labs(x=expression(TRBC~"("~RBC~x~10^{6}~"/"~mm^{3}~")"), # horrible expression language for getting formatted y axis 
     y="CeNS Index") + # Removed x-axis label because we don't want Panel A to have label
 # labs(x="TRBC", y="CeNS Index\n(TRBC/MCV + TRBC)") + # CeNS index = (TRBC/MCV+TRBC)
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") + 
 #annotate("text", x=7.1, y=0.70, label = "italic(R) ^ 2 == 0.19", parse = TRUE) + # Add R^2 value from lmer model
 # ggtitle("A") + # Assign panel number/header; this will be (a) because first in series of 3
  theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
         plot.title = element_text(face="bold")) + # This makes panel header bold 
       # This is good for labeling figure panels! Avoids having to manually toy w/ hjust and vjust
...
(p <- ggplot(cens.ss.mean, aes(x=mcv, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
 ...
 # geom_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1, colour="black", size=1.0) + # quadratic 
  geom_smooth(method = "lm", colour="black", size=1.0) + # linear 
  theme_classic() + 
  labs(x="MCV (fl)", y="CeNS Index") + # CeNS index = (TRBC/MCV+TRBC)
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") + 
  annotate("text", x=105, y=0.70, label = "italic(R) ^ 2 == 0.26", parse = TRUE) + # Add R^2 value from lmer model
 # ggtitle("A") + # Assign panel number/header; this will be (a) because first in series of 3
  theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
         plot.title = element_text(face="bold")) + # This makes panel header bold 
       # This is good for labeling figure panels! Avoids having to manually toy w/ hjust and vjust
...
# Correlation matrix 
cor(cens.ss.mean[,c("cens.index", "mcv")]) # 0.89
...
# HB GENOTYPE DOES NOT AFFECT CENS INDEX - Figure S9 in paper; see the polished version in Plots & Figs script
...
ggsave(FigureS9, filename="CensIndex_vs_Genotype_2021_07-27.pdf", height=7, width=9, units="in")
...
Notes: *Why did we choose to calculate cens index with estimates from models versus from raw data?* 
Because we want each parameter's relative contribution to driving [Hb], which we can't get without model estimates; pulling betas from models gives us the link to Hb. 

What's interesting here is that after modeling this connection between blood parameters, we actually find *the opposite* of what we do by analyzing the raw data: That is; we do *not* see the banana shape, the results of which which suggests that middle-elev species are increasing blood-O2 carrying capacity through higher MCV; instead what we find (i.e., the quadratic relationship) is that middle elev species are primarily increasing blood-O2 carrying capacity through increases in TRBC. 

This differs from the process of human acclimatization to altitude. Low and high elevation species both increase blood-o2 carrying capacity through having bigger cells and fewer of them (higher MCV), but this effect is much stronger in high elevation hummingbird species, which have larger cells. 
...
# Don't need to standardize inputs because remember we did this above 
```
...
p <- ggpairs(subset(cens.ss.mean, select = c(cens.index, elev.z, mass.z, wl.z, mcv, trbc, genotype))); p

# Correlation matrix 
cor(cens.ss.mean[,c("elev.z","mass.z", "wl.z", "cens.index", "mcv", "trbc")])
# Wing loading correlated with elev
```


# CeNS Phylogenetic Signal and Contmaps 
# FIGURE S4 IN PAPER (this one is *not* in `HumBlood_Plots&Figs.Rmd`)
```{r}
# CONTMAPS 
...
pdf(file="FigureS4_CeNS_Index_Contmap_2022-01-22.pdf", width=6, height=6) # Write plot to pdf 
#hct.cont.tree$cols[1:n] <- viridis(n) # Manually set color scale to viridis 
plot.contMap(cens.cont.tree, just.cens, res=200, fsize=0.4, lwd=2, leg.txt="CeNS (0.0-1.0)", outline=FALSE) # Manually plot using phytools
dev.off()
...
In addition to the above contmaps and tests of phylo signal, I ran a first pass at models w/ phylogeny incorporated. Buerkner's hypothesis test supported lack of phylo signal, and only ~4% of the variance was explained by phylogeny. Since phylogeny is a poor predictor, remove this from our models; use Blomberg's K, Pagel's lambda, and contmaps to evaluate phylo signal and proceed with simpler non-phylo brms models.
```{r}
...
save(cens.m1, file="Rdata_files/cens.m1_CeNS_Intercept-Only.RData") # save model
# load("Rdata_files/cens.m1_CeNS_Intercept-Only.Rdata") 
...
save(cens.m2, file="Rdata_files/cens.m2_CeNS_FullModel_PredictorsWithoutQuadratic.RData") # save model
# load("Rdata_files/cens.m2_CeNS_FullModel_PredictorsWithoutQuadratic.RData") # If loading from pre-saved file and not re-running
...
save(cens.m3, file="Rdata_files/cens.m3_CeNS.RData") # save model
load("Rdata_files/cens.m3_CeNS.RData") # If loading from pre-saved file and not re-running
...
save(cens.m4, file="Rdata_files/cens.m4_CeNS.RData") # save model
# load("Rdata_files/cens.m4_CeNS.RData") # If loading from pre-saved file and not re-running
})
```
...
#plot(conditional_effects(cens.m4, effects = "trbc.z"))

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(cens.m3)  # adjust prob to get 95% CI

# Residual plots 
# typical residual plot but w/ addition of uncertainty associated w/ each residual given by generating draws from the dist 
# associated with each residual
cens.ss.mean %>%
  add_residual_draws(cens.m3) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
cens.ss.mean %>%
  add_predicted_draws(cens.m3) %>%
  summarise(
...
# Cens: Plot outputs of posterior mean estimates and 95% credible intervals (full models)
```{r}
...
#pairs(hb.m3)
color_scheme_set("viridisD") # nice default
cens.m2.p1 <- mcmc_plot(cens.m2, variable=c("b_elev.z", "b_mass.z", "b_wl.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# Model 3 
cens.m3.p1 <- mcmc_plot(cens.m3, variable=c("b_elev.z","b_Ielev.zE2","b_mass.z","b_wl.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
summary(cens.m3)
# Predictors that don't overlap zero: elev.z, elev.z^2, mass.z

# Model 4 
cens.m4.p1 <- mcmc_plot(cens.m4, variable=c("b_elev.z", "b_Ielev.zE2","b_mass.z", "b_wl.z", "b_genotypeGlyMSer", "b_genotypeSerMSer"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
summary(cens.m4)
# Predictors that don't overlap zero: elev.z, elev.z^2

# Summary of reduced models: 
# Can't reduce M2 further 
# m5 (reduced m3) will be: elev.z, elev.z^2, mass.z
# m6 (reduced m4) will be: elev.z, elev.z^2
```
...
save(cens.m5, file="Rdata_files/cens.m5_CeNS_ReducedModel.RData") # save model
# load("Rdata_files/cens.m5_CeNS_ReducedModel.RData") # If loading from pre-saved file and not re-running

# MODEL 6: REDUCED MODEL (SAME AS M9 WOULD BE)
cens.m6 <- brm(
...
save(cens.m6, file="Rdata_files/cens.m6_CeNS_ReducedModel.RData") # save model
# load("Rdata_files/cens.m6_CeNS_ReducedModel.RData") 
```
...
# # Model 5
color_scheme_set("viridisD") # nice default
cens.m5.p1 <- mcmc_plot(cens.m5, variable=c("b_elev.z", "b_Ielev.zE2", "b_mass.z"),
                                    prob_outer=0.95, # 95% outer CI
                                    prob=0.50, # 50% inner CI
                                    point_est="mean") + # mean point est; default is median
labs(x="Effect on Cens Index", title = "cens.m5") +
scale_y_discrete(labels=c("Elevation", "Elevation^2", "Mass"))
cens.m5.p1
...
cens.m6.p1 <- mcmc_plot(cens.m6, variable=c("b_elev.z", "b_Ielev.zE2"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on Cens Index", title = "cens.m6") + 
scale_y_discrete(labels=c("Elevation", "Elevation^2"))   
cens.m6.p1
...
# Get model WAIC scores  
cens.m6.waic <- waic(cens.m5)
cens.m7.waic <- waic(cens.m6)
cens.waics.red <- cbind(cens.m5.waic, cens.m6.waic); cens.waics.red
...
# Calculate Bayesian R^2:
bayes_R2(cens.m2) # 0.21
bayes_R2(cens.m3) # 0.38
bayes_R2(cens.m4) # 0.41
bayes_R2(cens.m5) # 0.33
...
1) Model 3: Full model w/ elev, elev quadratic, and no genotype term. R^2 = 0.38.
2) (tied) Model 5: Reduced model w/ elev, elev quadratic, and mass. R^2 = 0.33.
3) Model 4: Other full model w/ elev quadratic and genotype term. R^2 = 0.41

All models are NOT substantially better than the intercept-only model. No phylogenetic signal in CeNS data. 


**Some CeNS take-homes:**
- Strong relationship between CeNS index and elevation: middle elevation species appear to adjust O2 carrying capacity through cell number while lower and high elev species adjust O2 carrying capacity through increases to cell size (we provide detailed analysis of this in our paper). 
- Strong relationship between CeNS index and mass (CeNS increases w/ increased mass)
- Moderate relationship between CeNS index and wing loading (CeNS increases w/ increased WL; CIs overlap zero)
...
# RELEVANT NOTES: A test of converting between standardized and unstandardized coefficients 
...
In short, converting standardized to unstandardized to estimate real world effects DOES NOT WORK - the converted "unstandardized" betas never match those estimated from unstandardized models. These notes describe attempts to understand why: 
```{r}
...
# glimpse(post)
# 
# # This example  confirms that yes, I'm doing the math right...but I still can't figure out why hand-estimated and model-
...
2) Run the top model with unstandardized coefficients. Use this model summary to estimate the magnitude of change in y relative to each x, but DO NOT compare among x variables. --> i.e., does not allow you to compare the magnitude of effect of elevation vs. temperature on Hb, but DOES allow you to say that for every 100 m increase in elev you expect [hb] to increase by 1.0 g/dl, for example (FYI I just made up those numbers for this hypothetical example). 
