...
# Take a look at # observations per species:
sp.count.cens <- censdat %>% group_by(species) %>% summarise(count = length(nk))

# Summary of species-specific datasets we'll model (all at least) 
ss.summary <- subset(censdat, with(censdat, species %in% names(which(table(species)>=10))))
ss.count.cens <- ss.summary %>% group_by(species) %>% summarise(count = length(nk))

# test datasets
Pguy <- censdat[which(censdat$species == "Phaethornis_guy"),] # w/ Phaethornis guy (small, n=10)
Pgigs <- censdat[which(censdat$species == "Patagona_gigas_S"),] 
Pgign <- censdat[which(censdat$species == "Patagona_gigas_N"),] 
Mtyr <- censdat[which(censdat$species == "Metallura_tyrianthina"),] # Metallura, n=66
...
# load("cens.il.m3_Predictors_SpeciesRE.RData")
})
```
...
plot(hyp.cens.il.m3)
# 12% variance explained; est = 0.12, CI = 0.06-0.2
...
cens.il.m3.unstandardized <- 
  update(cens.il.m3,
        newdata = censdat,
        formula = hb ~ 1 + mcv + trbc + (1|species),
        chains = 4, cores = 4)
save(cens.il.m3.unstandardized, file="cens.il.m3.unstandardized_Predictors_SpeciesRE.RData") # save model
})
# load("cens.il.m3.unstandardized_Predictors_SpeciesRE.RData")

summary(cens.il.m3.unstandardized)
# mcv = 0.12; trbc = 1.82; what this tells us: 
# For every unit increase in TRBC, [Hb] increases by 1.82 g/dl
...
save(cens.Pguy, file="cens.Pguy_Predictors_SpeciesRE.RData") # save model
# load("cens.Pguy_Predictors_SpeciesRE.RData")

# Metallura tyrianthina 
cens.Mtyr <- brm(
  formula = bf(hb ~ 1 + mcv + trbc),
  data = Mtyr,
  family = student(), # student might fit a bit better than gaussian?
  cores = 4,
  chains = 4,
  thin = 10,
  warmup = 10000, # default is iter/2; shouldn't ever be larger than iter
...
bayesplot::pp_check(cens.Pguy, resp = "hb", nsamples = 200) # pull 100 draws from model and cast back to original data 
