...
censdat <- subset(final, select=c(species, rowID, nk, department, month, elev, mass, wl, hb, mcv, mcv.log, trbc)) 
...
# Effect of TRBC is marginally stronger than MCV (estimates: TRBC = 2.05; MCV = 1.93)
...
- Model 2, full model + species grouping variable fits best by var! Variance explained by species: 13%. R^2 = 0.56. 
...
# Both MCV and TRBC have strong positive effects on Hb.
# As TRBC increases, Hb increases. As MCV increases, Hb increases.
# Effect of TRBC is marginally stronger than MCV (estimates: TRBC = 1.45; MCV = 1.40)
...
cens.ss.mean <- cbind(cens.ss.mean, cens.trbc, cens.mcv, cens.R2, cens.unst.trbc, cens.unst.mcv)
...
# write.csv(cens.ss.mean, "cens.ss.mean_ForCeNSmodeling_06-02-21.csv")
```
...
ggsave(p5, filename="CeNSindex_by_MeanElev_06-02-21.pdf", height=7, width=9, units="in")
...
ggsave(p6, filename="CeNSindex_by_WingLoading_06-02-21.pdf", height=7, width=9, units="in")
...
ggsave(p7, filename="CeNSindex_by_Mass_06-02-21.pdf", height=7, width=9, units="in")
...
# Correlation matrix 
cor(cens.ss.mean[,c("cens.index", "trbc")]) # -.42
...
JESSIE LEFT OFF HERE on 6/2/21 NEED TO MAKE NEW CENS TREEES WITH PATAGONA GIGAS SOUTH INCLUDED. 
...
# Phylogeny pruned from McGuire and adjusted (see script HumBlood_Phylogeny.Rmd)
tree.cens <- read.tree("McGuirePruned_AllHummingbirdComparativeTree_SpeciesMeans_PgigN&PgigS_FINAL.tre") # a list 
...
p <- ggpairs(subset(cens.ss.mean, select = c(cens.index, elev.z, mass.z, wl.z))); p
...
bayesplot::pp_check(cens.m2, resp = "cens.index", nsamples = 200) 
...
summary(cens.m2)
# Predictors that don't overlap zero: elev.z; strong trend of wing loading 
...
cens.m4 <- brm(
  formula = bf(cens.index ~ 1 + elev.z),
  data = cens.ss.mean,
...
# # Model 4 
color_scheme_set("viridisD") # nice default
cens.m4.p1 <- mcmc_plot(cens.m4, pars=c("b_elev.z", "b_Ielev.zE2", "b_mass.z"),
                                    prob_outer=0.95, # 95% outer CI
                                    prob=0.50, # 50% inner CI
                                    point_est="mean") + # mean point est; default is median
...
# bayes_R2(cens.m4) # 0.29
bayes_R2(cens.m5) # 0.29
...
1) Model 3: Full with quadratic elevation. R^2 = 0.38. 
2) Model 5: Reduced model with elev, elev quadratic, and mass. R^2= 0.29. 
3) Model 1: Null model. 
**All models, including null, are better than non quadratic model. 
