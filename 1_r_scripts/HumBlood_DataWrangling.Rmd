...
date: "11/23/2020; last revised 2023-04-20"
output:
  md_document:
...
setwd("/Users/Jessie/MSBbirds Dropbox/Jessie Williamson/Rdirectory/ComparativeHummingbirdBlood")
```
...
```{R, message=FALSE, warning=FALSE}
# Load functions 
source("./1_r_scripts/ada_functions.R") # Erik's ADA functions for clean & collated lm diag plots

# Raw hummingbird blood data
full <- read.csv("./AllHumBlood_Full_Dataset_EcoLett_2023-01-06.csv") 
full <- full[ , !(colnames(full) %in% c("X"))] # drop weird X column 1 created by reading in from read.csv
...
# Counts for each variable by numeric, not very helpful
#full %>% count(hb)

## At this point we have the following numbers of observations: 
# Hb: 1046
# Hct: 1149
# TRBC: 809
# MCV: 790
...
# DuBay and Witt range = 13.2-21.4 g/dl
...
# <35% suggests anemia; >55% suggests dehydration or erythrocytosis (polycythemia; increase in RBCs)
...
# Calculate percent total dropped:
round(3/1149*100,2) # 0.26%
...
# Campbell & Ellis note that 308 MCV is "large" & indicative of younger cells while 128 is smaller, suggests older cells
# I made upper limit cut-off 150 instead of 200 after looking at distributions again; makes hist and qqPlots look better

# Calculate percent total dropped:
round(29/790*100,2) # 3.67%
...
Note about outlier removal: Since dropping extreme values can result in more data lost (though I'd argue that 66 total observations isn't much), an alternative approach would be to only eliminate outliers for each response variable data subset, thereby retaining more subset-specific data; i.e., by just dropping MCV outliers in MCV dataset, I'd then keep normal Hb values associated with outlier MCV values); though this is is ultimately a bit more painstaking, makes it difficult to wrangle data downstream, and is unlikely to really get us too much in the end because our sampling is good.
...
Bursa lasts longer than anomalous juvenile blood characteristics, so presence/absence of bursa isn't 100% correlative with ages that would be important for blood analyses. SO: looking at a *combination* of characteristics is the best approach: 
* 1) First, eliminate anomalous blood values (see code chunks above)
* 2) Then, analyze distribution of blood values. If juvenile values look normal and fit in with others (namely: the adult distribution), don't remove them on the basis of having a bursa. But, if blood values look weird *AND* a bird has a "large" bursa, then remove value. 
...
# Wrangle "Stotz" (Parker et al. 1996 data)
...
```{r, message=FALSE, warning=FALSE}
# Note: this process could be simplified with a nice function like the one Ethan wrote for Linck et al. 2023 Am Nat blood paper
# *But* I committed to this clunky, "species-specific-personalized" method, so sticking with it at the mo.
...
Chris knows most of these from memory. Otherwise, look up/double check all clades in McGuire et al. 2014 (supplement has full, searchable trees). Some species/clades are listed in Projecto-Garcia et al. 2013.
...
# Commented out becuase we didn't use this for paper! It's an overly basic (and therefore somewhat problematic) classification for many reasons.
...
# We have a lot of missing blood data...this is because these parameters are hard to calculate!
# It's also not always possible to collect all blood trait data for each individual. 
...
final$b13[final$species=="Amazilia viridicauda" | 
          ...
          final$species=="Florisuga mellivora" |   # -------
...
Hb genotypes from Projecto-Garcia et al. 2013 data. Genotypes inferred from sister taxa for: 
Amazilia franciae
