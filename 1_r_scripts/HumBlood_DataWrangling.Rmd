...
qqPlot(full$mass) # looks bad (but would look ok w/out Patagona...)
qqPlot(full$elev) # not great but maybe hard to fix because of uneven sampling?
qqPlot(full$hb) # fine AF 
...
- Also NOTE: Any non-vouchered (i.e. tracked) Patagona whose age was scored by plumage shouldn't be eliminated, as juvenile plumage lasts much longer than anomalous juvenile blood values. 
```{r}
...
# This looks pretty good. If age-based outliers remained, we'd expect lots of errant green points
...
peru_alt <- raster("/Users/Jessie/Dropbox (MSBbirds)/Rdirectory/ComparativeHummingbirdBlood//peru_alt.grd") #read elevation raster
chile_alt <- raster("/Users/Jessie/Dropbox (MSBbirds)/Rdirectory/ComparativeHummingbirdBlood//chile_alt.grd") #read elevation raster
surrounding_elevs <- raster("/Users/Jessie/Dropbox (MSBbirds)/Rdirectory/ComparativeHummingbirdBlood//surrounding_elevs.grd") # surrounding South American country elevs
...
#write.csv(final, "AllHummingbirdBlood_Wrangled_withBioClim_11-09-20.csv")
```
...
write.csv(final, "AllHummingbirdBlood_Wrangled_withBioClim_andPCA_NoHbGenotypes_11-09-20.csv")
```
...
After doing this, we were still left with 15 species missing Hb genotypes. We made educated best guesses based on genotypes of sister taxa and elevational ranges. We think most of these are solidly-inferred, though we know this is not a foolproof method. 
...
write.csv(species.means, "AllHummingbird_SpeciesMeanBloodData_11-09-20.csv")
...
write.csv(final, "AllHummingbirdBlood_Wrangled_withBioClim_andPCA_11-09-20.csv")
...
write.csv(final.nopgig, "HumBlood.final.nopgig_11-09-20.csv")
```
