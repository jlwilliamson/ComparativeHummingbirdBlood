...
date: "11/23/2020; last revised 1/23/22"
output:
  md_document:
...
**This script includes**: processing of raw data, evaluating and eliminating outliers, creating new variables, combining data w/ Stotz and MSB wing loading, making individual elevational range adjustments, processing spatial data and generating sampling map, BioClim processing, reading out of final data file for modeling, plotting and downstream analyses.
...
# Convert all lat and lon values and add to data frame 
blood$lat_convert <- convert_lat(blood) 
blood$lon_convert <- convert_lon(blood) 
...
Note about lat/lons: A bunch of lat/lons were missing from July 2007 Tres Lagunas, Lambayeque birds at 3,200 m. This stemmed from processing errors. However, one bird, NK162815 (*C. iris*) from 12 July 2007 *does* have coords for this locality, elev, and date range. Chris and I decided to use the lat/lon for NK162815 to fill in coordinates for all missing-coord NKs.  

So, in summary: I added coords from NK162815 to the following NKs: 162691, 162758, 162720, 162705, 162757, 162690, 162725, 162592, 162759, 162776, 162782, 162689, 162591, 162783, 162704



# Subset the data by just values I'll use in analysis 
Previous code chunks increased dataset 'bloat'. Now, subset again to something more manageable. This also helps initiate removal of NAs, which is prudent for modeling. 
```{r, message=FALSE, warning=FALSE}
...
bsub <- subset(blood, select = c(rowID,
                                 ...
                                 species,
                                 sex,
                                 age_est, # See notes in Excel metadata about estimating age from bursa
                               # day,
                                 month,
                                 year,
                                 locality,
...
# The dplyr way that isn't working: 
...
The Patagona data we will include that is not in the all-hummingbird dataset are those data from Chilean birds for which we have full blood data (n=17). This list excludes tracked birds, which are the focus of forthcoming publications for Jessie's dissertation. Because this is a short list of 17, we will subset by the NKs of the specific individuals whose data we want. 
```{r, message=FALSE, warning=FALSE}
...
# subset pdat to include all the same columns that bsub has/to prepare for merge w/ bsub
...
# Note:  many 'wing' measurements are zero at this point; this will be fixed once we combine w/ Skandalis & impute
...
# Write out 'full' w/ ALL data - to be published in Witt et al. (In Prep)
*Full data for Williamson et al. and Linck et al. will be published in Witt et al., a large data paper describing blood measurements for hundreds of species from 2006-2020. 
```{r}
#write.csv(full, "AllHumBlood_full-dataset_notfinal_forreference_2022-01-23.csv") 
    # Note that this is NOT in Williamson et al.; will be published with Witt et al. 
...
# Transform  variables
```{r, echo=FALSE}
# Note: I now do this in modeling files! Made a lot more sense to combine all transformations prior to modeling. 
```
...
# "assessment of abnormal values of MCV, MCH, and MCHC has not been properly evaluated in birds"; let distribution guide!

# for MCV: drop any value <50 or >200
full <- full[-which(full$mcv < 50 | full$mcv > 150),] # 29 observations 
# Cambpell & Ellis note that 308 MCV is "large" & indicative of younger cells while 128 is smaller, suggests older cells
# I made upper limit cut-off 150 instead of 200 after looking at distributions again; makes hist and qqPlots look better
...
Bursa lasts longer than anomalous juvenile blood characteristics, so presence/absence of bursa isn't 100% correlative with ages that would be important for blood analyses. Jessie, Chris, and Ethan decided that looking at a *combination* of characteristics was the best approach: 
* 1) First, eliminate anomalous blood values (see code chunks above)
* 2) Then, analyze distribution of blood values. If these look normal and fit in with others, don't remove on the basis of having a bursa. But, if blood values look weird *AND* a bird has a "large" bursa, then remove value. 
...
wload$wing_area_cm2 <- wload$wing_area_mm2/100 # convert total wing area in mm^2 to cm^2
...
# What I want: number of rows should be same as `full`, wing loading data should exist for wload species = SUCCESS! 
...
# Number of rows should match 'full'
...
# Note: I previously called index column names (elev_min=[28], elev_max=[29], elev_midpt=[30]), BUT, I didn't like this: 
# Bc every time I updated df above I had to tediously update column numbers & things shuffled; BETTER TO CALL COL NAMES.
...
full1[full1$species==missing[10],][, "elev_min"] <- 0 # P. atrimentalis # Not in Stotz; adding Schulenberg 2007 ranges
...
full1[full1$species==missing[11],][, "elev_min"] <- 0 # Sephanoides sephaniodes # Sephanoides sephanOIdes in Stotz...OBVS
...
Stotz (Parker et al. 1996) is a good baseline for elevational ranges but is frequently inaccurate, and in some cases, outdated. Adjust species elevational ranges based on Schulenberg et al. 2007 Birds of Peru, museum records from MSB, other field guides (if necessary), and working knowledge. 
```{r, message=FALSE, warning=FALSE}
# Note: this process could be simplified with a nice function like the one Ethan wrote for Linck et al. all-species
# blood comparison. *But* I committed to this clunky, "species-specific-personalized" method, so sticking with it atm.
...
full1[full1$species=="Patagona gigas",][, "elev_min"] <- 0    # Min elev; based on Williamson & Witt 2021 (ENSM paper) 
full1[full1$species=="Patagona gigas",][, "elev_max"] <- 4830 # Max elev; based on Williamson & Witt 2021 (ENSM paper) 

# Rhodopis vesper 
full1[full1$species=="Rhodopis vesper",][, "elev_max"] <- 3800 # Max elev, determined from Schulenberg et al. 2007
...
# Polyonymus caroli
# Schulenberg et al. 2007 lists 2100-3400 m; we know they're common at 3,700-3,800 m in Lima Department
# We've collected up to 4,062 m in Potaga, so will use max elev of 4,100
...
full1[full1$species=="Doryfera ludovicae",][, "elev_max"] <- 2900 # Max elev; increasing to match Schulenberg & specimen records
...
# Discuss this one - seems to be found a lot higher than recorded 
...
Projecto-Garcia et al. 2013 lists many species and their associated clades. Best phylogeny is McGuire et al. 2014. 
...
# Commented out becuase we didn't use this for paper! Problematic basic classification for a handful of reasons.
...
# Calculate elevational range position, called `elevational position` in our paper
```{r, message=FALSE, warning=FALSE}
...
# Could have also done collection elev minus species midpoint; more analogous to intravar variables
```
...
# We have too much missing data to be able to impute well/confidently/as recommended. 
# It's clunkier, but  more reliable to create subsets for each blood parameter model set, then remove NAs from each.

# SECOND BUT: We can 'impute' mass values by substituting means across species and sex
...
# If this is wonky, try `summarise` spelling with an `s`; dplyr is really sensitive to `ise` vs `ize`

# Create a teeny subset with all mass=NA values 
nomasses <- final[-which(!is.na(final$mass)),] # 19 obs missing masses that you'll "impute" w/ species mean values
...
How we did this: We first added Hb genotypes from Projecto-Garcia et al. 2013. If any were missing, we looked through Hbba sequence files, assembled alignments with 5 GenBank Reference sequences from Projecto-Garcia, and determined Hb genotypes that way (see description of this workflow in Hbba-genotyping-workflow-hummingbird-comparative-data_08-27-20.doc). Then, if species were still missing, we took Hb genotypes from the McGuire et al .nex file that has a nice little alignment of b13-b83 genotypes for many species. 

After doing this, we were still left with 12 species missing Hb genotypes. We made educated best guesses based on genotypes of sister taxa and elevational ranges. We believe most of these are solidly-inferred, though we know this is not a foolproof method. 
...
Here are the list of 12 species for which we used "best guess" Hb genotype info: 
Amazilia franciae
...
# Note: I tried to switch order to merge polygons and THEN clip + mask but this never worked
