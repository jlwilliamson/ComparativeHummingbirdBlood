...
final <- read.csv("AllHummingbirdBlood_Wrangled_withBioClim_andPCA_12-01-20.csv", stringsAsFactors = TRUE)
...
p <- ggpairs(subset(final, select = c(hb, hct, trbc, mcv.log, mch.log, mchc, mass.log, wl, elev, elev_position, tempPC1, precipPC1)))
print(p)
```
...
final$intravar.elev.z <- standardize(final$intravar.elev) 
...
p <- ggpairs(subset(final, select = c(mass.z, wl.z, elev.z, elev.pos.z, temp.z, precip.z, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z)))
print(p)
```
...
# Hb subset - 991 observations
final.hb <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, hb, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z)) 
final.hb <- na.omit(final.hb)
str(final.hb) # looks good, nothing to coerce 

# Hct subset - 1092 observations
final.hct <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, hct, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z)) 
final.hct <- na.omit(final.hct)
str(final.hct) # looks good, nothing to coerce 

# trbc subset - 754 observations
final.trbc <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, trbc, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z)) 
final.trbc <- na.omit(final.trbc)
str(final.trbc) # looks good, nothing to coerce

# mcv subset - 738 observations
final.mcv <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mcv, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z, mcv.log)) 
final.mcv <- na.omit(final.mcv)
str(final.mcv) # looks good, nothing to coerce 

# mch subset - 706 observations
final.mch <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mch, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z, mch.log)) 
final.mch <- na.omit(final.mch)
str(final.mch) # looks good, nothing to coerce 

# mchc subset - 927 observations
final.mchc <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mchc, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.elev.z, intravar.elev.pos.z, intravar.mass.z, intravar.temp.z, intravar.precip.z)) 
...
phylosig(tree, x, test=TRUE) # K = 0.196, p = 0.622 = no signal 
...
phylosig(tree=tree.hb, x=just.hb, method="K", test=TRUE) # K=0.144, p=0.829
...
```{r}
# Correlation matrix 
cor(final[,c("elev.z", "elev.pos.z", "temp.z", "precip.z", "mass.z", "intravar.elev.z", "intravar.elev.pos.z", "intravar.mass.z", "intravar.temp.z", "intravar.precip.z", "wl.z")])
...
summary(hb.m3)
# Predictors whose 95% CIs do NOT overlap zero: elev.z, elev.pos.z
# Reduced hb.m3 will be: elev.z, elev.pos.z

# Model 5
#color_scheme_set("red") # nice default
hb.m5.p1 <- mcmc_plot(hb.m5, pars=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev.z", "b_intravar.elev.pos.z", "b_intravar.mass.z",
                                    "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
...
summary(hb.m5)
# Predictors whose 95% CIs do NOT overlap zero: elev.pos.z
# Reduced hb.m5 will be: elev.position.z

####

# Summary of reducing models: 
# m7 (reduced m3) will be: elev.z, elev.pos.z
# m8 (reduced m4) will be: elev.pos.z
```
...
hb.m7 <- brm(
  formula = bf(hb ~ 1 + elev.z + elev.pos.z),
  data = final.hb,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
hb.m9 <- brm(
  formula = bf(hb ~ 1 + elev.pos.z + (1|species)),
  data = final.hb,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
# Model 7 
color_scheme_set("blue") # nice default
hb.m7.p1 <- mcmc_plot(hb.m7, pars=c("b_elev.z", "b_elev.pos.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elev", "Elevation Position"))  
hb.m7.p1
...
# Model 9
#color_scheme_set("red") # nice default
hb.m9.p1 <- mcmc_plot(hb.m9, pars=c("b_elev.pos.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
**All models fit substantially better than the intercept-only model (m1) and the random-effects only model (m2)
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(hct.m5, type = "trace") 
plot(hct.m5, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(hct.m5, resp = "hct", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(hct.m5, type = "stat", stat = 'median', nsamples = 100)
...
# Pale bars = you want all heading towards zero to show they're not correlating w/ each other across the chains 
# fine to be pos and neg, jumping around to get to zero
mcmc_plot(hct.m5, type = "acf_bar")

# plot conditional effects for full model (note: you do this separately below)
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(hct.m5), points = TRUE) 

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(hct.m5)  # adjust prob to get 95% CI
...
# Model 5
#color_scheme_set("red") # nice default
hct.m5.p1 <- mcmc_plot(hct.m5, pars=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev.z", "b_intravar.elev.pos.z", "b_intravar.mass.z",
                                    "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
...
summary(hct.m5)
# Predictors whose 95% CIs do NOT overlap zero: elev.position.z + intravar.elev.pos.z
# Reduced hct.m5 will be: elev.pos.z + + intravar.elev.pos.z

####

# Summary of reducing models: 
# m7 (reduced m3) will be: elev.position.z 
# m9 (reduced m5) will be: elev.pos.z + + intravar.elev.pos.z
```
...
hct.waics <- cbind(hct.m1.waic, hct.m2.waic, hct.m3.waic, hct.m5.waic); hct.waics
...
# MODEL 9: REDUCED MODEL WITH ONLY SPECIES RANDOM EFFECT
hct.m9 <- brm(
  formula = bf(hct ~ 1 + elev.pos.z + intravar.elev.pos.z + (1|species)),
  data = final.hct,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(hct.m9, type = "trace") 
plot(hct.m9, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(hct.m9, resp = "hct", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(hct.m9, type = "stat", stat = 'median', nsamples = 100)
...
# Pale bars = you want all heading towards zero to show they're not correlating w/ each other across the chains 
# fine to be pos and neg, jumping around to get to zero
mcmc_plot(hct.m9, type = "acf_bar")

# plot conditional effects for full model (note: you do this separately below)
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(hct.m9), points = TRUE) 

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(hct.m9)  # adjust prob to get 95% CI
...
# Print group level variable (in log odds)
# ranef is still an array, so not an exponent on it, hard to change to probability estimates 
ranef(hct.m9, groups="species") #, probs = 0.95)
```
...
# Model 7 
color_scheme_set("blue") # nice default
hct.m7.p1 <- mcmc_plot(hct.m7, pars=c("b_elev.pos.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# Model 9
#color_scheme_set("red") # nice default
hct.m9.p1 <- mcmc_plot(hct.m9, pars=c("b_elev.pos.z", "b_intravar.elev.pos.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation Position", "Intravar: Elev Position"))   
hct.m9.p1
...
hct.waics.red <- cbind(hct.m7.waic, hct.m9.waic); hct.waics.red
...
LOO(hct.m1, hct.m2, hct.m3, hct.m5, hct.m7, hct.m9, reloo=FALSE) 
sink()
...
Quick summary of top 3 models in order of ranking: 
1) Model 5: Full model + (1|species). 
2) Model 9: Elevation position + (1|species). **only marginally worse than m5 
2) Model 2: RE only model with + (1|species).

**All models performed substantially better than the intercept-only model. 



NOTE ON 11/23/20 - I DIDN'T GET GET TO TRBC MODEL SET!!!
...
trbc.waics.red <- cbind(trbc.m7.waic, trbc.m9.waic); trbc.waics.red
...
LOO(trbc.m1, trbc.m2, trbc.m3, trbc.m5, trbc.m7, trbc.m9, reloo=FALSE) 
sink()
# According to LOO, model 8 is top model 
