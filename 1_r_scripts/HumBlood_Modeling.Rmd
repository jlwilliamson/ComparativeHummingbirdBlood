...
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(trbc.m4, resp = "trbc", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(trbc.m4, type = "stat", stat = 'median', nsamples = 100)
...
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(mcv.m4, resp = "mcv", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(mcv.m4, type = "stat", stat = 'median', nsamples = 100)
...
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(mcv.m6), points = TRUE) 
...
LOO(mcv.m1, mcv.m2, mcv.m3, mcv.m4, mcv.m5, mcv.m6, reloo=FALSE) 
sink()
# compare models # Just a shorter version of the above text
# loo_compare(en.loo) # Top ranked model includes no random effects 
...
# Calculate Bayesian R^2:
bayes_R2(mcv.m1)
bayes_R2(mcv.m2) # 0.091
bayes_R2(mcv.m3) # 0.052
bayes_R2(mcv.m4) # 0.112
...
1) Model 2: Null model + (1|species) **significantly better than most models, marginally better than M6 (reduced), 14% variance explained by species, R^2 = 0.093
2) Model 6: Reduced model w/ just temp.z and +1 species, 14% variation explained by species, R^2 = 0.09 
3) Model 4: Full + (1|species), 14% of variance explained by species, R^2 = 0.112 

**Null model fits the best, suggesting that no predictors are great for MCV. However, effect of species clearly important. Perhaps MCV is a blood trait best explained by other blood traits? Not quite sure how to interpret this. 
...
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(mch.m4, resp = "mch", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(mch.m4, type = "stat", stat = 'median', nsamples = 100)
...
# Calculate Bayesian R^2:
bayes_R2(mch.m1)
bayes_R2(mch.m2) # 0.09
bayes_R2(mch.m3) # 0.025
bayes_R2(mch.m4) # 0.115
bayes_R2(mch.m5) # 0.005
bayes_R2(mch.m6) # 0.09
...
1) Model 2: Null model + (1|species) **significantly better than most models, marginally better than M6 (reduced), 12% variance explained by species, R^2 = 0.09
2) Model 6: Reduced model w/ just temp.z and +1 species, 12% variation explained by species, R^2 = 0.09 
3) Model 4: Full + (1|species), 13% of variance explained by species, R^2 = 0.115 

**Null model fits the best, suggesting that no predictors are great for MCH However, effect of species clearly important. Perhaps MCH is a blood trait best explained by other blood traits? Not quite sure how to interpret this. 
...
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(mchc.m4, resp = "mchc", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(mchc.m4, type = "stat", stat = 'median', nsamples = 100)
...
bayes_R2(mchc.m3) # 0.048
bayes_R2(mchc.m4) # 0.133
bayes_R2(mchc.m5) # 0.036
bayes_R2(mchc.m6) # 0.116
...
1) Model 6: Reduced model w/ just intravar.temp and intravar.mass and +1 species, 15% variation explained by species, R^2 = 0.116 2) Model 4: Full + (1|species), 18% of variance explained by species, R^2 = 0.133. Only marginally lower than Model 6. 
...
hb.m4.p1 <- mcmc_plot(hb.m4, pars=c("b_elev.z", "b_elev.pos.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
