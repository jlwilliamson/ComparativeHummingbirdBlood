...
date: "11/09/2020; Last Updated: 2023-04-20"
output:
  md_document:
...
This script includes: Reading in pre-processed data from "HumBlood_DataWrangling.Rmd", standardizing predictors, making data subsets that match subsetted phylogenies (so there are no NAs in any dataset for modeling), and then running, checking, plotting sets of Bayesian hierarchical models. 

Note that elements of this script function as a notebook/codebook to document analyses that we didn't end up using in our paper (e.g., all transformations of wing loading data); see model structure and notes for information about what was/wasn't included in final paper analyses. 
...
# Troubleshooting rstan/brms issues: 
Pro tip: If brms stops running and throws weird error messages, in my experience it's related to updating R versions and something breaking along the way. Refer to those posts above about configuring hte toolchain and reinstalling R stan, restarting, and reinstalling packages. You can also run this test model: 
```{r}
...
source("/Users/Jessie/Dropbox (MSBbirds)/Rdirectory/ComparativeHummingbirdBlood/1_r_scripts/Rfunctions.R") 

# Final hummingbird blood dataset
# Unlike tree file, you want all categorical variables to be factors for modeling
final <- read.csv("AllHummingbirdBlood_Wrangled_withBioClim_andPCA_06-14-21.csv", stringsAsFactors = TRUE)
...
```{r}
# Since we confirmed above that the only missing data is currently related to missing blood data,  
...
# Calculate intraspecific variation variables (within-species metrics)
```{R}
# Do this before standardizing so calculations are done on raw values and standardizing happens AFTERWARDS
# (Otherwise you'll be using standardized values to calculate raw metrics = not what we want)
...
final$spec.mean.mass <- with(final, sapply(split(mass, species), mean)[species]) # RAW MASS, NOT LOG MASS (log wonky)

# MORE MASS STUFF (a little more complicated): 
# Since we know that males and females have substantially different body masses *and* blood characteristics are expected
# to differ by sex, calculate intraspecific variation for females w/ female mean mass, intraspecific variation for 
# males w/ male mean mass, and we'll use species mean mass for the n=6 unknown sexes. 
# I always love clunky n simple ways of doing things: split df by sex, calculate mean masses per sex, and rbind back together 
...
# Keep col header name the same for merging (& note that you'll write over original "spec.mean.mass" for each subset)
...
# NOW, Calculate individual diffs from species mean value (aka intraspecific variation, or 'intravar'); within-sp diffs
...
# HOWEVER (see below):
# After extensive prelim modeling, I figured out that it's much better to run MCV models with non-transformed MCV 
# and skew-normal distribution; fit is better than w/ log-transformed MCV and Gaussian or other dist
...
# MCHC: Distribution has semi-heavy tails. Log transforming shifts "fit" around tails; doesn't advance goal of normality.
# I think this would be better adjusted with priors to account for heavy/long tails. 
# See note for MCV above; relevant also to MCH
...
# All commented out below was not used in paper: 
...
# WING LOADING: original wing loading distribution is delightfully normal, but when each mean is "spread" across 
# our big dataset so that they can be used for modeling, distribution becomes wonky. 
# Would be best to get original wing loading data to be able to use these individual-level vs. mean values.
 
# Raw data checks from 'wload': Data normally distributed, look good in original data checks, i.e. 
# histogram, qqPlot, shapiro test checks:
# plotNormalHistogram(wload$wl) 
# qqPlot(wload$wl)
# shapiro.test(final$hb) # passes normality test, p > 0.05 
# 
# # but...as mentioned, when we "spread" mean values across individual dataset, bad stuff happens to normality: 
...
# Tukey's Ladder of Powers: performs iterative Shapiro–Wilk tests, finds lambda value to maximize W statistic from tests.  
...
# But since we didn't end up using wing loading data in within and among species models, this ultimately doesn't matter!
...
We're generating subsets now to be able to retain the max amount of data for each blood trait 
Versus...if we just ran na.omit() on `final`, we'd lose hundreds of Hb measurements that didn't e.g. have MCV values.
```{R}
...
Before moving on with pre-modeling verification and modeling itself, work up continuous trait maps for each of the 6 blood parameters to take a look at phylo signal and spread of values. Based on preliminary phylogenetic modeling efforts (not shown here and not included in our paper; see Methods and Supplement), we expect low phylogenetic signal (i.e., similar branch colors) aross the board. But, in a few cases, extreme values may be driven by low sampling (i.e., n=1 or n<3). When we re-ran tests of phylogenetic signal and prelim phylo models, and re-made these contmaps *without* these "sampling outliers" (see code chunk below), results were consistent and the outcome was the same: No to very low phylogenetic signal in all blood traits. 
```{r}
...
# Lamely, moosh all these together in Illustrator figure to make Figure S1
# AKA, open each pdf in Illustrator, remove clipping mask, paste into one artboard, manually add panel headers, write out as Figure S1
```
...
# Blomberg's K: computed w/ p-val by comparing real data to null distribution obtained by random permuations. 
    # Values of K range from 0 to inf; K=1 = Brownian motion evol; 
    # K>1 = spp more similar than expected under random drift; 
    # K<1 = spp less similar than expected under random drift. 
    # The larger the K, the stronger the phylo signal. 
...
# Verify that phylo signal and modeling are robust to "sampling outliers"
How much is phylo signal affected by poorly-sampled species? How much are modeling results affected by poorly-sampled species? (The latter is a topic that came up during Ecology Letters review). To get a sense for how these species might affect estimates, we dropped any species with n < 3 individuals, remade contmaps, and reran tests of phylogenetic signal. 

During review, our editor and a reviewer wondered how species with just a few records might affect model results. To test this, we removed singletons, n=2/species samples, and n=3/species samples, re-ran models, and compared output.
```{r}
# # The suffix ".nso" stands for "no sampling outliers"

# Change this to >3 if testing phylogenetic sample for min of 3 sampled.
# Change to >2 if dropping singletons
# Change to >4 if dropping species with 1-3 individuals per species sampled
# 
# Drop poorly sampled species 
final.hb.nso <- subset(final.hb, with(final.hb, species %in% names(which(table(species)>=4))))
hb.dropped.species <- subset(final.hb, with(final.hb, species %in% names(-which(table(species)<4)))) # pull those dropped
   # View(final.hb %>% group_by(species) %>% summarise(count = length(nk))) # 988 obs, 74 species
    View(final.hb.nso %>% group_by(species) %>% summarise(count = length(nk))) # 978 obs, 64 species
 
final.hct.nso <- subset(final.hct, with(final.hct, species %in% names(which(table(species)>=4))))
   # View(final.hct %>% group_by(species) %>% summarise(count = length(nk))) # 1086 obs, 76 species  
  #  View(final.hct.nso %>% group_by(species) %>% summarise(count = length(nk))) # 1078 obs, 68 species
    
final.trbc.nso <- subset(final.trbc, with(final.trbc, species %in% names(which(table(species)>=4))))
   # View(final.trbc %>% group_by(species) %>% summarise(count = length(nk))) # 748 obs, 72 species
   # View(final.trbc.nso %>% group_by(species) %>% summarise(count = length(nk))) # 736 obs, 60 species
    
final.mcv.nso <- subset(final.mcv, with(final.mcv, species %in% names(which(table(species)>=4))))
   # View(final.mcv %>% group_by(species) %>% summarise(count = length(nk))) # 732 obs, 72 species
   # View(final.mcv.nso %>% group_by(species) %>% summarise(count = length(nk))) # 719 obs, 59 species
    
final.mch.nso <- subset(final.mch, with(final.mch, species %in% names(which(table(species)>=4))))
   # View(final.mch %>% group_by(species) %>% summarise(count = length(nk))) # 703 obs, 69 species
   # View(final.mch.nso %>% group_by(species) %>% summarise(count = length(nk))) # 692 obs, 58 species
    
final.mchc.nso <- subset(final.mchc, with(final.mchc, species %in% names(which(table(species)>=4))))
   # View(final.mchc %>% group_by(species) %>% summarise(count = length(nk))) # 924 obs, 73 species
   # View(final.mchc.nso %>% group_by(species) %>% summarise(count = length(nk))) # 915 obs, 64 species

# # Set up tips to drop and drop tips from each dataset 
    ## Note that I dropped species with <3 for this phylo signal test but adjusted the treshold to 2 above for modeling 
    ## This allowed me to be consistent with species mean models (change made 2/19/23)
...
**Conclusion:** Even after removing sampling outliers (species with <3 individuals sampled), there is no phylogenetic signal in our data. This is confirmed visually w/ contmaps and with estimates of Pagel's lamda and Blomberg's K. (See more robust response on this topic in our response to reviewers)
...
# elevation and elev_position are correlated (0.63)
# However, we justify each as important and indicative of different patterns.   
# Elevation and tempPC1 are highly correlated (-0.86), but both are biologically important and encompass different things
# as described in McNew et al. 2021, PNAS
...
```{r}
#write.csv(final, "ComparativeHummingbirdBlood_FinalDataset_ForModeling_06-14-21.csv")
```


# Calculate means, standard deviations, ranges, and coefficients of variation for individual-level data 
These data are presented in SI Appendix Table S5. 
```{r}
# Parameter names 
col1 <- c("[Hb]", "Hct", "TRBC", "MCV", "MCH", "MCHC")

# Mean values
col2 <- c(
          round(mean(final.hb$hb),2), 
          round(mean(final.hct$hct),2), 
          round(mean(final.trbc$trbc),2), 
          round(mean(final.mcv$mcv),2), 
          round(mean(final.mch$mch),2), 
          round(mean(final.mchc$mchc),2) 
          ); col2

# Standard deviations 
col3 <- c(
          round(sd(final.hb$hb),2), 
          round(sd(final.hct$hct),2), 
          round(sd(final.trbc$trbc),2), 
          round(sd(final.mcv$mcv),2), 
          round(sd(final.mch$mch),2), 
          round(sd(final.mchc$mchc),2) 
          ); col3

# # Ranges
# col4 <- c(
#           c(round(min(final.hb$hb),2), round(max(final.hb$hb),2)),
#           c(round(min(final.hct$hct),2), round(max(final.hct$hct),2)),
#           c(round(min(final.trbc$trbc),2), round(max(final.trbc$trbc),2)),
#           c(round(min(final.mcv$mcv),2), round(max(final.mcv$mcv),2)),
#           c(round(min(final.mch$mch),2), round(max(final.mch$mch),2)),
#           c(round(min(final.mchc$mchc),2), round(max(final.mchc$mchc),2))
#           ); col4

# Range: Min values
col4 <- c(
          round(min(final.hb$hb),2), 
          round(min(final.hct$hct),2), 
          round(min(final.trbc$trbc),2), 
          round(min(final.mcv$mcv),2), 
          round(min(final.mch$mch),2), 
          round(min(final.mchc$mchc),2) 
          ); col4
    
# Range: Max values
col5 <- c(
          round(max(final.hb$hb),2), 
          round(max(final.hct$hct),2), 
          round(max(final.trbc$trbc),2), 
          round(max(final.mcv$mcv),2), 
          round(max(final.mch$mch),2), 
          round(max(final.mchc$mchc),2) 
          ); col5

# Coefficients of variation  
col6 <- c(
          round(cv(final.hb$hb),2), 
          round(cv(final.hct$hct),2), 
          round(cv(final.trbc$trbc),2), 
          round(cv(final.mcv$mcv),2), 
          round(cv(final.mch$mch),2), 
          round(cv(final.mchc$mchc),2) 
          ); col6

TableS5_IndividualLevelSummaryValues <- cbind(col1, col2, col3, col4, col5, col6); TableS5_IndividualLevelSummaryValues
colnames(TableS5_IndividualLevelSummaryValues) <- c("Parameter", "Mean", "Standard Deviation", "Min", "Max", "Coefficient of Variation")
# some weird character/string formatting going on, but I got the values I want for wrangling into Word format

# write.csv(TableS5_IndividualLevelSummaryValues, "TableS5_Individual-LevelSummaryValues.csv")
  # This file is called Table S3, but note that it became TAble S5 in paper
```


# Get correlation matrix for full, individual-level dataset 
We can't get a correlation matrix for the `final` dataset since it contains many NAs, and we can't use the strategic variable-specific subsets because they only contain values for each parameter of interest (ie., `final.hb` contains only hb information, not info for the 5 other parameters). To simplify, we will run na.omit() on `final` and just use what we can. 
```{r}
# First select only blood parameters (if we run na.omit on final w/out doing this, we're left with 14 lines of data)
final.for.corr <- subset(final, select = c(nk, hb, hct, trbc, mcv, mch, mchc))
final.for.corr <- na.omit(final.for.corr) # We're left with 688 observations for our correlation matrix

# Correlation matrix - individual-level data
cor(final.for.corr[,c("hb", "hct", "trbc", "mcv", "mch", "mchc")]) 

#             hb         hct        trbc         mcv         mch        mchc
# hb   1.0000000  0.76083476  0.24445721  0.13895495  0.25431313  0.33945216
# hct  0.7608348  1.00000000  0.30338019  0.19814442  0.07832426 -0.34899108
# trbc 0.2444572  0.30338019  1.00000000 -0.84807551 -0.84951531 -0.08432156
# mcv  0.1389550  0.19814442 -0.84807551  1.00000000  0.93990732 -0.09034342
# mch  0.2543131  0.07832426 -0.84951531  0.93990732  1.00000000  0.24927648
# mchc 0.3394522 -0.34899108 -0.08432156 -0.09034342  0.24927648  1.00000000

```


# Double-check Clade factor levels for data subsets 
```{r}
# Mountain-gem sticks around as a phantom level despite not being present for Hb, MCH, or MCHC; update factor # 
final.hb$Clade <- factor(final.hb$Clade)
final.mch$Clade <- factor(final.mch$Clade)
final.mchc$Clade <- factor(final.mchc$Clade)

nlevels(final.mchc$Clade) # Check to make sure levels are correct 
```
...
Note on 3/24/21: Is it appropriate to use individual-level data (referred to as "repeated measures" in the Bayesian world, implying multiple individuals sampled per species) as predictors in a model, since some Bayesian material suggests that predictors should be population-level? Brief answer = YES. 

See resources, like this excellent vignette about incorporating repeated measures into phylogenetic brms models: (https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html) 
This tutorial suggests taking individual-level, repeated measures data and constructing "species mean" and "within species variability" variables and using these two as predictors in lieu of individual-level data (aka raw data values). 
...
**MORE DETAILED CONCLUSION:** Yes, our model structure is ok, and it is fine to use individual-level data as predictors. See last comment by Jon Nations in the above stan forum post (aka, might affect those conditional probabilities plots because more error is involved in individual-level measurements). Alternatively, it is also totally fine to break the data into "species mean" and "within species variability" (raw value - mean), as described in the Buerkner phylo tutorial above, and Paul Buerkener says he did this following the phylogenetic textbook (Garamszagi original tutorial) from which the data originally come. So essentially: Paul modified Garamszagi's tutorial; now everyone uses Paul's tutorial, which is just one approach to a modeling challenge put forward by one person 
...
This is a good lesson in "there are multiple ways to skin a cat"; questioning is good, trusting intuition is good, exploring options is good. (Plus, I did end up doing a comparison between individual-level data and population-level data (w/ predictors of spec_mean and within_species_variability) and there was essentially no difference in results between models. Conditional probability plots did fit a bit better in population-level models, and R^2 was a bit higher in pop-level model, but diagnostics were all the same, and in loo model comparison, original hb.m4 emerged as the top model, marginally better than hb.popleveltest.m4)

TL;DR - We want individual-level data *and* it's totally acceptable to keep model structure this way.
...
I did a lot of research on priors and default priors specified by brms are ok. Models fit nearly identically with and without weakly informative priors. 
...
On a related note, it's better to run mcv models with non-transformed MCV and skew_normal distribution - fit is better than with log-transformed MCV and gaussian. 
...
save(hb.m1, file="Rdata_files/hb.m1_Intercept-Only.RData") # save model
#load("Rdata_files/hb.m1_Intercept-Only.Rdata") # If loading from pre-saved file and not re-running
...
save(hb.m2, file="Rdata_files/hb.m2_RandomEffectsOnly.RData") # save model
# load("Rdata_files/hb.m2_RandomEffectsOnly.RData") # If loading from pre-saved file and not re-running
...
save(hb.m3, file="Rdata_files/hb.m3_FullModel_PredictorsOnly.RData") # save model
# Run time: ~1 minute
load("Rdata_files/hb.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(hb.m4, file="Rdata_files/hb.m4_FullModel_SpeciesREOnly.RData") # save model
load("Rdata_files/hb.m4_FullModel_SpeciesREOnly.RData") 


#####
# Test whether results are sensitive to poorly-sampled species 
hb.wellsampled <- brm(
  formula = bf(hb ~ 1 + elev.z + elev.pos.z + mass.z + temp.z + precip.z + genotype +
               intravar.mass.z + intravar.temp.z + intravar.precip.z 
               + (1|species)),
...
save(hb.wellsampled, file="Rdata_files/hb.wellsampled_Min4Species.RData") # save model
#load("Rdata_files/hb.wellsampled.RData") 

# Test whether results are sensitive to poorly-sampled species 
# A test made during review; these results are not included in our paper
hb.wellsampled.randomslope <- brm(
  formula = bf(hb ~ 1 + elev.z + elev.pos.z + mass.z + temp.z + precip.z + genotype +
               intravar.mass.z + intravar.temp.z + intravar.precip.z 
               + (1 + elev.z |species)),
  data = final.hb.nso,
...
save(hb.wellsampled.randomslope, file="Rdata_files/hb.wellsampled.randomslope_SpeciesWithOnlyMoreThanThreeIndivs.RData") # save model
#load("Rdata_files/hb.wellsampled_SpeciesWithOnlyMoreThanThreeIndivs.RData.RData") 

length(final.hb.nso$species)
```
...
hb.m3.p1 <- mcmc_plot(hb.m3, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
...
hb.m4.p1 <- mcmc_plot(hb.m4, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# Model - well sampled
#color_scheme_set("red") # nice default
hb.wellsampled.p1 <- mcmc_plot(hb.wellsampled, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.wellsampled: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))   
hb.wellsampled.p1
ggsave(hb.wellsampled.p1, filename = "hb.wellsampled_Min4Individuals.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.wellsampled)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z and mass.z

# Model - well sampled w/ random slope
#color_scheme_set("red") # nice default
hb.wellsampled.randomslope.p1 <- mcmc_plot(hb.wellsampled.randomslope, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.wellsampled.randomslope: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))   
hb.wellsampled.randomslope.p1
ggsave(hb.wellsampled.randomslope.p1, filename = "hb.wellsampled.randomslope.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.wellsampled.randomslope)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z and mass.z
...
hb.wellsampled.waic <- waic(hb.wellsampled)
hb.wellsampled.randomslope.waic <- waic(hb.wellsampled.randomslope)
...
# m4 species variance 
hyp.sp.hb.wellsampled <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hb.wellsampled <- hypothesis(hb.wellsampled, hyp.sp.hb.wellsampled, class = NULL))
plot(hyp.sp.hb.wellsampled)
...
save(hb.m5, file="Rdata_files/hb.m5_ReducedModel_PredictorsOnly.RData") # save model
#load("Rdata_files/hb.m5_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(hb.m6, file="Rdata_files/hb.m6_ReducedModel_SpeciesREOnly.RData") # save model
# load("Rdata_files/hb.m6_ReducedModel_SpeciesREOnly.RData") 
```
...
# Model 5 
color_scheme_set("blue") # nice default
hb.m5.p1 <- mcmc_plot(hb.m5, variable=c("b_elev.z", "b_mass.z", "b_temp.z", "b_intravar.temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# Model 6
#color_scheme_set("red") # nice default
hb.m6.p1 <- mcmc_plot(hb.m6, variable=c("b_elev.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
bayes_R2(hb.m4) # 0.209s
bayes_R2(hb.m5) # 0.116
bayes_R2(hb.m6) # 0.192

bayes_R2(hb.wellsampled) # 0.192
...
hb.m4.unstandardized <- 
  update(hb.m4,
        ...
        chains = 4, cores = 4)
save(hb.m4.unstandardized, file="Rdata_files/hb.m4.unstandardized_Predictors_SpeciesRE.RData") # save model
})
#load("Rdata_files/hb.m4.unstandardized_Predictors_SpeciesRE.RData")
...
# lower & upper CIs
print(hb.m4.unstandardized, digits=10) # print model with 10 sig figs to pull full 95% CI estimates (this is lazy way)
round(0.0001157090*1000, 2) # lower CI (0.12 g/dl)
round(0.0006654191*1000, 2) # upper CI (0.67 g/dl)
```
...
# HB: POST HOC ANALYSIS OF CLADE AND ELEVATION RELATIONSHIPS 
Not really in the paper but explored here - one supplemental plot on this, Fig. S7
```{r}
# Load intercept only and top models (if not already loaded)
load("Rdata_files/hb.m4_FullModel_SpeciesREOnly.RData") 
load("Rdata_files/hb.m1_Intercept-Only.Rdata") 

# MODEL 4.c: TOP MODEL WITH CLADE RE ADDED 
hb.m4.c <- brm(
  formula = bf(hb ~ 1 + elev.z + elev.pos.z + mass.z + temp.z + precip.z + genotype +
               intravar.mass.z + intravar.temp.z + intravar.precip.z + (1|species) + (1|Clade)),
  data = final.hb,
  family = gaussian(), 
  cores = 4,
  chains = 4,
...
save(hb.m4.c, file="Rdata_files/hb.m4.c_TopModel_CladeREAdded.RData") # save model
#load("Rdata_files/hb.m4.c_TopModel_CladeREAdded.RData") 

# MODEL 4.ce: TOP MODEL WITH CLADE*ELEV INTERACTION
hb.m4.ce <- brm(
  formula = bf(hb ~ 1 + elev.z + elev.pos.z + mass.z + temp.z + precip.z + genotype +
               intravar.mass.z + intravar.temp.z + intravar.precip.z + Clade:elev.z + (1|species)),
  data = final.hb,
  family = gaussian(), 
  cores = 4,
  chains = 4,
...
save(hb.m4.ce, file="Rdata_files/hb.m4.ce_TopModel_CladeElevInteraction.RData") # save model
#load("Rdata_files/hb.m4.ce_TopModel_CladeElevInteraction.RData") 


# Fit assessment
bayesplot::pp_check(hb.m4.ce, resp = "hb", ndraws = 100)
mcmc_plot(hb.m4.c, type = "acf_bar")

# associated with each residual
final.hb %>%
  add_residual_draws(hb.m4.ce) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
final.hb %>%
  add_predicted_draws(hb.m4.ce) %>%
  summarise(
    p_residual = mean(.prediction < hb), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
# Model 4.c 
color_scheme_set("blue") # nice default
hb.m4.c.p1 <- mcmc_plot(hb.m4.c, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.m4.c: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))   
hb.m4.c.p1
ggsave(hb.m4.c.p1, filename = "hb.m4.c.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.m4.c)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z

# Model 4.ce
#color_scheme_set("red") # nice default
hb.m4.ce.p1 <- mcmc_plot(hb.m4.ce, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z",
                                    "b_elev.z:CladeBrilliant", "b_elev.z:CladeCoquette", "b_elev.z:CladeEmerald", 
                                    "b_elev.z:CladeHermit", "b_elev.z:CladeMango", "b_elev.z:CladePatagona", 
                                    "b_elev.z:CladeTopaz"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.m4.ce: Posterior mean estimates & 95% Credible Intervals") + 
...
hb.m4.ce.p1
ggsave(hb.m4.ce.p1, filename = "hb.m4.ce.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.m4.ce)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
# *Bee is the reference clade

# Plot interactions
plot(conditional_effects(hb.m4.ce_factor ,
                      effects="elev.z:Clade", 
                      spaghetti=TRUE, ndraws=150),
                      points=TRUE,
                      point_args=c(alpha=2/3, size=1), mean=FALSE)

interact_plot(hb.m4.ce, pred = elev.z, modx = Clade,
              colors=c("#F4CD0B", "#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F", "#EE4024", "#03446B"),
              plot.points=TRUE, partial.residuals = FALSE, interval = FALSE)
              # Order: Bee, Brilliant, Coquette, Emerald, Hermit, Mango, Patagona, Topaz
              # Manually specify colors because package doesn't handle more than 7 automatically
              # If modx is a factor, all levels will be plotted
# Using partial.residuals = TRUE, what is plotted instead is the observed data with the effects of all the control variables accounted for. In other words, the value cty for the observed data is based only on the values of displ, cyl, and the model error. Let’s take a look
# Can only plot partial.residuals with all clades specified since that's what the model estimated.

# Repeate interaction plot with just well-sampled clades
interact_plot(hb.m4.ce, pred=elev.z, modx=Clade, 
              plot.points=TRUE,
              modx.values=c("Brilliant", "Coquette", "Emerald", "Hermit", "Mango"),
              colors=c("#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F"),
              )
...
# Print model summaries 
sink("hb_CladeAnalysis_brms_model_summaries.txt",append = TRUE)
summary(hb.m1, waic=TRUE)
summary(hb.m4, waic=TRUE)
summary(hb.m4.c, waic=TRUE)
summary(hb.m4.ce, waic=TRUE)
sink()
# print(hb.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%
...
hb.m1.waic <- waic(hb.m1)
hb.m4.waic <- waic(hb.m4)
hb.m4.c.waic <- waic(hb.m4.c)
hb.m4.ce.waic <- waic(hb.m4.ce)
hb.clade.waics.red <- cbind(hb.m1.waic, hb.m4.waic, hb.m4.c.waic, hb.m4.ce.waic); hb.clade.waics.red
write.csv(hb.clade.waics.red, "hb_CladeAnalysis_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# m6 species variance 
hyp.sp.hb.m4.c <- paste(
  "sd_Clade__Intercept^2 /", 
  "(sd_Clade__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.sp.hb.m4.c <- hypothesis(hb.m4.c, hyp.sp.hb.m4.c, class = NULL))
plot(hyp.sp.hb.m4.c)
# 13% variance explained by Clade

# hyp.sp.hb.m4.c <- "sd_Clade__Intercept^2 / (sd_Clade__Intercept^2 + sigma^2) = 0"
# (hyp.sp.hb.m4.c <- hypothesis(hb.m4.c, hyp.sp.hb.m4.c, class = NULL))
#  
...
library(loo)
sink("hb_CladeAnalysis_brms_models_looic.txt", append=FALSE)
LOO(hb.m1, hb.m4, hb.m4.c, hb.m4.ce, reloo=FALSE) 
sink()
...
bayes_R2(hb.m4.c) # 0.214
bayes_R2(hb.m4.ce) # 0.227
```

Quick summary of top models from Clade analysis:  
1) Model 4.c: Full model + (1|Clade) *marginally better than m4.ce, 13% variance explained by clade.  
2) Model 4.ce: Full model w/ clade:elev.z. 
3) Model 4: Full model. 
4) All performed substantially better than the intercept-only null model. 


Interpreting interactions: 
In a simple linear regression with no interactions, each coefficient says how much the average outcome, μ, changes when the predictor changes by one unit. And since all of the parameters have independent influences on the outcome, there’s no trouble in interpreting each parameter separately. Each slope parameter gives us a direct measure of each predictor variable’s influence. Interaction models ruin this paradise
(pg. 220)


Out of curiosity, what does elev.z:Clade alone look like? 
```{r}
# Simple elev*clade model
hb.elevclade <- brm(
  formula = bf(hb ~ 1 + elev.z + Clade:elev.z + (1|species)),
  data = final.hb,
  family = gaussian(), 
  cores = 4,
  chains = 4,
...
save(hb.elevclade, file="Rdata_files/hb.elevclade_SimpleElevCladeModel.RData") # save model
#load("Rdata_files/hb.elevclade_SimpleElevCladeModel.RData") 

#color_scheme_set("red") # nice default
hb.elevclade.p1 <- mcmc_plot(hb.elevclade, variable=c("b_elev.z", 
                                    "b_elev.z:CladeBrilliant", "b_elev.z:CladeCoquette", "b_elev.z:CladeEmerald", 
                                    "b_elev.z:CladeHermit", "b_elev.z:CladeMango", "b_elev.z:CladePatagona", 
                                    "b_elev.z:CladeTopaz"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.elevclade: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation", "Elev*Brilliant", "Elev*Coquette", "Elev*Emerald", "Elev*Hermit", "Elev*Mango", "Elev*Patagona", "Elev*Topaz"))   
hb.elevclade.p1
ggsave(hb.elevclade.p1, filename = "hb.elevclade.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.elevclade)

# m4 species variance 
hyp.sp.hb.elevclade <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hb.elevclade <- hypothesis(hb.elevclade, hyp.sp.hb.elevclade, class = NULL))
plot(hyp.sp.hb.elevclade)
...
# Simple clade model
hb.clade <- brm(
  formula = bf(hb ~ 1 + elev.z + Clade + (1|species)),
  data = final.hb,
  family = gaussian(), 
  cores = 4,
  chains = 4,
...
save(hb.clade, file="Rdata_files/hb.clade_SimpleCladeModel.RData") # save model
#load("Rdata_files/hb.clade_SimpleElevCladeModel.RData") 

#color_scheme_set("red") # nice default
hb.clade.p1 <- mcmc_plot(hb.clade, variable=c("b_elev.z", 
                                    "b_CladeBrilliant", "b_CladeCoquette", "b_CladeEmerald", 
                                    "b_CladeHermit", "b_CladeMango", "b_CladePatagona", 
                                    "b_CladeTopaz"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.clade: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation", "Brilliant", "Coquette", "Emerald", "Hermit", "Mango", "Patagona", "Topaz"))   
hb.clade.p1
ggsave(hb.clade.p1, filename = "hb.clade.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.clade)

# m4 species variance 
hyp.sp.hb.clade <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hb.clade <- hypothesis(hb.clade, hyp.sp.hb.clade, class = NULL))
plot(hyp.sp.hb.clade)
# 18% variance explained by species
```
...
save(hct.m1, file="Rdata_files/hct.m1_Intercept-Only.RData") # save model
# load("Rdata_files/hct.m1_Intercept-Only.Rdata") 
...
save(hct.m2, file="Rdata_files/hct.m2_RandomEffectsOnly.RData") # save model
#load("Rdata_files/hct.m2_RandomEffectsOnly.RData") # If loading from pre-saved file and not re-running
...
save(hct.m3, file="Rdata_files/hct.m3_FullModel_PredictorsOnly.RData") # save model
# Run time: ~1 minute
# load("Rdata_files/hct.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(hct.m4, file="Rdata_files/hct.m4_FullModel_SpeciesREOnly.RData") # save model
...
#####
# MODEL 4: FULL MODEL WITH ONLY SPECIES RANDOM EFFECT ON WELL SAMPLED DATA
hct.wellsampled <- brm(
  formula = bf(hct ~ 1 + elev.z + elev.pos.z + mass.z + temp.z + precip.z + genotype +
               intravar.mass.z + intravar.temp.z + intravar.precip.z 
               + (1|species)),
...
save(hct.wellsampled, file="Rdata_files/hct.wellsampled_FullModel_SpeciesREOnly.RData") # save model
# load("Rdata_files/hct.m4_FullModel_SpeciesREOnly.RData") 
```
...
# Model 3 
color_scheme_set("blue") # nice default
hct.m3.p1 <- mcmc_plot(hct.m3, variable=c("b_elev.z", "b_elev.pos.z", "b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# Model 4
#color_scheme_set("red") # nice default
hct.m4.p1 <- mcmc_plot(hct.m4, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# Model 4
#color_scheme_set("red") # nice default
hct.wellsampled.p1 <- mcmc_plot(hct.wellsampled, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.wellsampled: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position", "Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))   
hct.wellsampled.p1
#ggsave(hct.m4.p1, filename = "hct.m4.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hct.wellsampled)
# Predictors whose 95% CIs do NOT overlap zero (that will be in reduced model): elev.z + elev.pos.z + intravar.mass.z
```
...
save(hct.m5, file="Rdata_files/hct.m5_ReducedModel_PredictorsOnly.RData") # save model
# load("Rdata_files/hct.m5_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(hct.m6, file="Rdata_files/hct.m6_ReducedModel_SpeciesREOnly.RData") # save model
# load("Rdata_files/hct.m6_ReducedModel_SpeciesREOnly.RData") 
```
...
# Model 5 
color_scheme_set("blue") # nice default
hct.m5.p1 <- mcmc_plot(hct.m5, variable=c("b_elev.z", "b_elev.pos.z", "b_mass.z", "b_intravar.mass.z", "b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# Model 6
#color_scheme_set("red") # nice default
hct.m6.p1 <- mcmc_plot(hct.m6, variable=c("b_elev.z", "b_elev.pos.z", "b_intravar.mass.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
hct.m4.unstandardized <- 
  update(hct.m4,
        ...
        formula = hct ~ 1 + elev + elev_position + mass.log + tempPC1 + precipPC1 + genotype + # use non .z names!!!!
               intravar.mass + intravar.temp + intravar.precip + (1|species),
        chains = 4, cores = 4)
save(hct.m4.unstandardized, file="Rdata_files/hct.m4.unstandardized_Predictors_SpeciesRE.RData") # save model
})
#load("Rdata_files/hct.m4.unstandardized_Predictors_SpeciesRE.RData")
...
# lower & upper CIs
print(hct.m4.unstandardized, digits=10) # print model with 10 sig figs to pull full 95% CI estimates (this is lazy way)
round(0.0005537224*1000, 2) # lower CI (0.55 percentage points)
round(0.0022943449*1000, 2) # upper CI (2.29 percentage points)
```
...
# hct: POST HOC ANALYSIS OF CLADE AND ELEVATION RELATIONSHIPS 
Not in paper but explored as a supplemental analysis.
```{r}
# Load intercept only and top models (if not already loaded)
load("Rdata_files/hct.m4_FullModel_SpeciesREOnly.RData") 
load("Rdata_files/hct.m1_Intercept-Only.Rdata") 
...
save(hct.m4.c, file="Rdata_files/hct.m4.c_TopModel_CladeREAdded.RData") # save model
#load("Rdata_files/hct.m4.c_TopModel_CladeREAdded.RData") 
...
save(hct.m4.ce, file="Rdata_files/hct.m4.ce_TopModel_CladeElevInteraction.RData") # save model
#load("Rdata_files/hct.m4.ce_TopModel_CladeElevInteraction.RData") 


# Fit assessment
bayesplot::pp_check(hct.m4.ce, resp = "hct", ndraws = 100)
mcmc_plot(hct.m4.c, type = "acf_bar")

# associated with each residual
final.hct %>%
  add_residual_draws(hct.m4.ce) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
final.hct %>%
  add_predicted_draws(hct.m4.ce) %>%
  summarise(
    p_residual = mean(.prediction < hct), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
# Model 4.c 
color_scheme_set("blue") # nice default
hct.m4.c.p1 <- mcmc_plot(hct.m4.c, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.m4.c: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))   
hct.m4.c.p1
ggsave(hct.m4.c.p1, filename = "hct.m4.c.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hct.m4.c)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z

# Model 6
#color_scheme_set("red") # nice default
hct.m4.ce.p1 <- mcmc_plot(hct.m4.ce, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z",
                                    "b_elev.z:CladeBrilliant", "b_elev.z:CladeCoquette", "b_elev.z:CladeEmerald", 
                                    "b_elev.z:CladeHermit", "b_elev.z:CladeMango", "b_elev.z:CladeMountainMgem",  
                                    "b_elev.z:CladePatagona", "b_elev.z:CladeTopaz"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.m4.ce: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip", "Elev*Brilliant", "Elev*Coquette", "Elev*Emerald", "Elev*Hermit", "Elev*Mango", "Elev*MtnGem", "Elev*Patagona", "Elev*Topaz"))   
hct.m4.ce.p1
ggsave(hct.m4.ce.p1, filename = "hct.m4.ce.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hct.m4.ce)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
# *Bee is the reference clade

# Plot interactions
plot(conditional_effects(hct.m4.ce,
                      effects="elev.z:Clade", 
                      spaghetti=TRUE, ndraws=150),
                      points=TRUE,
                      point_args=c(alpha=2/3, size=1), mean=FALSE)

interact_plot(hct.m4.ce, pred = elev.z, modx = Clade,
              colors=c("#F4CD0B", "#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F", "#E786B7", "#EE4024", "#03446B"),
              plot.points=TRUE, partial.residuals = FALSE, interval = FALSE)
              # Order: Bee, Brilliant, Coquette, Emerald, Hermit, Mango, Mountain-Gem, Patagona, Topaz
              # Manually specify colors because package doesn't handle more than 7 automatically
              # If modx is a factor, all levels will be plotted
# Using partial.residuals = TRUE, what is plotted instead is the observed data with the effects of all the control variables accounted for. In other words, the value cty for the observed data is based only on the values of displ, cyl, and the model error. Let’s take a look
# Can only plot partial.residuals with all clades specified since that's what the model estimated.

# Repeat interaction plot with just well-sampled clades
interact_plot(hct.m4.ce, pred=elev.z, modx=Clade, 
              plot.points=TRUE,
              modx.values=c("Brilliant", "Coquette", "Emerald", "Hermit", "Mango"),
              colors=c("#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F"),
...
# Print model summaries 
sink("hct_CladeAnalysis_brms_model_summaries.txt",append = TRUE)
summary(hct.m1, waic=TRUE)
summary(hct.m4, waic=TRUE)
summary(hct.m4.c, waic=TRUE)
summary(hct.m4.ce, waic=TRUE)
sink()
# print(hct.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%
...
hct.m1.waic <- waic(hct.m1)
hct.m4.waic <- waic(hct.m4)
hct.m4.c.waic <- waic(hct.m4.c)
hct.m4.ce.waic <- waic(hct.m4.ce)
hct.clade.waics.red <- cbind(hct.m1.waic, hct.m4.waic, hct.m4.c.waic, hct.m4.ce.waic); hct.clade.waics.red
write.csv(hct.clade.waics.red, "hct_CladeAnalysis_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Variance explained by Clade
hyp.sp.hct.m4.c <- paste(
  "sd_Clade__Intercept^2 /", 
  "(sd_Clade__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.sp.hct.m4.c <- hypothesis(hct.m4.c, hyp.sp.hct.m4.c, class = NULL))
plot(hyp.sp.hct.m4.c)
# 8% variance explained by Clade

# hyp.sp.hct.m4.c <- "sd_Clade__Intercept^2 / (sd_Clade__Intercept^2 + sigma^2) = 0"
# (hyp.sp.hct.m4.c <- hypothesis(hct.m4.c, hyp.sp.hct.m4.c, class = NULL))
#  
...
# if 2*standard error > delta LOOIC, models aren't necessarily distinguishable
library(loo)
sink("hct_CladeAnalysis_brms_models_looic.txt", append=FALSE)
LOO(hct.m1, hct.m4, hct.m4.c, hct.m4.ce, reloo=FALSE) 
sink()
# According to LOO, model 5 is top model 
...
# Calculate Bayesian R^2:
bayes_R2(hct.m4) # 0.17
bayes_R2(hct.m4.c) # 0.177
bayes_R2(hct.m4.ce) # 0.187
```

Quick summary of top models from Clade analysis:  
1) Model 4.c: Full model + (1|Clade) *marginally better than m4, 8% variance explained by clade.  
2) Model 4: Full model  
3) Model 4.ce: Full model w/ clade:elev.z.
4) All performed substantially better than the intercept-only null model. 
...
save(trbc.m1, file="Rdata_files/trbc.m1_Intercept-Only.RData") # save model
#load("Rdata_files/trbc.m1_Intercept-Only.Rdata") 
...
save(trbc.m2, file="Rdata_files/trbc.m2_RandomEffectsOnly.RData") # save model
#load("Rdata_files/trbc.m2_RandomEffectsOnly.RData") # If loading from pre-saved file and not re-running
...
save(trbc.m3, file="Rdata_files/trbc.m3_FullModel_PredictorsOnly.RData") # save model
# Run time: ~1 minute
load("Rdata_files/trbc.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(trbc.m4, file="Rdata_files/trbc.m4_FullModel_SpeciesREOnly.RData") # save model
...
# Model 3 
color_scheme_set("blue") # nice default
trbc.m3.p1 <- mcmc_plot(trbc.m3, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# Model 4
#color_scheme_set("red") # nice default
trbc.m4.p1 <- mcmc_plot(trbc.m4, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
...
save(trbc.m5, file="Rdata_files/trbc.m5_ReducedModel_PredictorsOnly.RData") # save model
# load("Rdata_files/trbc.m5_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(trbc.m6, file="Rdata_files/trbc.m6_ReducedModel_SpeciesREOnly.RData") # save model
# load("Rdata_files/trbc.m6_ReducedModel_SpeciesREOnly.RData") 
```
...
# Model 7 
color_scheme_set("blue") # nice default
trbc.m5.p1 <- mcmc_plot(trbc.m5, variable=c("b_elev.z", "b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# Model 9
#color_scheme_set("red") # nice default
trbc.m6.p1 <- mcmc_plot(trbc.m6, variable=c("b_elev.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
trbc.m4.unstandardized <- 
  update(trbc.m4,
        ...
        formula = trbc ~ 1 + elev + elev_position + mass.log + tempPC1 + precipPC1 + genotype + # use non .z names!!!!
               intravar.mass + intravar.temp + intravar.precip + (1|species),
        chains = 4, cores = 4)
save(trbc.m4.unstandardized, file="Rdata_files/trbc.m4.unstandardized_Predictors_SpeciesRE.RData") # save model
})
load("Rdata_files/trbc.m4.unstandardized_Predictors_SpeciesRE.RData")
...
round(fixef(trbc.m4.unstandardized)[2]*1000, 2)*(10^6) # Convert to raw # cells per microliter (can also *1000000)
# SO: For every 1,000 m increase in elev, TRBC will increase by 310,000 cells per microliter 

# lower & upper CIs
print(trbc.m4.unstandardized, digits=10) # print model with 10 sig figs to pull full 95% CI estimates (this is lazy way)
round(0.0000604009*1000, 2)*(10^6) # lower CI (60,000 cells per microliter)
round(0.0005456582*1000, 2)*(10^6) # upper CI (550,000 cells per microliter)
```
...
# TRBC: POST HOC ANALYSIS OF CLADE AND ELEVATION RELATIONSHIPS 
Not in paper but explored as a supplemental analysis.
```{r}
# Load intercept only and top models (if not already loaded)
load("Rdata_files/trbc.m4_FullModel_SpeciesREOnly.RData") 
...
save(trbc.m4.c, file="Rdata_files/trbc.m4.c_TopModel_CladeREAdded.RData") # save model
#load("Rdata_files/trbc.m4.c_TopModel_CladeREAdded.RData") 
...
save(trbc.m4.ce, file="Rdata_files/trbc.m4.ce_TopModel_CladeElevInteraction.RData") # save model
#load("Rdata_files/trbc.m4.ce_TopModel_CladeElevInteraction.RData") 


# Fit assessment
bayesplot::pp_check(trbc.m4.ce, resp = "trbc", ndraws = 100)
mcmc_plot(trbc.m4.c, type = "acf_bar")

# associated with each residual
final.trbc %>%
  add_residual_draws(trbc.m4.ce) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
final.trbc %>%
  add_predicted_draws(trbc.m4.ce) %>%
  summarise(
    p_residual = mean(.prediction < trbc), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
# Model 4.c 
color_scheme_set("blue") # nice default
trbc.m4.c.p1 <- mcmc_plot(trbc.m4.c, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [trbc]", title = "trbc.m4.c: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))   
trbc.m4.c.p1
ggsave(trbc.m4.c.p1, filename = "trbc.m4.c.p1_trbc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(trbc.m4.c)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z

# Model 4.ce
#color_scheme_set("red") # nice default
mcmc_plot(trbc.m4.ce)


trbc.m4.ce.p1 <- mcmc_plot(trbc.m4.ce, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z", "b_intravar.temp.z", "b_intravar.precip.z",
                                    "b_elev.z:CladeBrilliant", "b_elev.z:CladeCoquette", "b_elev.z:CladeEmerald", 
                                    "b_elev.z:CladeHermit", "b_elev.z:CladeMango", "b_elev.z:CladeMountainMgem", 
                                    ...
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [trbc]", title = "trbc.m4.ce: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Elevation Position","Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip", "Elev*Brilliant", "Elev*Coquette", "Elev*Emerald", "Elev*Hermit", "Elev*Mango", "Elev*MtnGem", "Elev*Patagona", "Elev*Topaz"))   
trbc.m4.ce.p1
ggsave(trbc.m4.ce.p1, filename = "trbc.m4.ce.p1_trbc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(trbc.m4.ce)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
# *Bee is the reference clade

# Plot interactions
# plot(conditional_effects(trbc.m4.ce,
#                       effects="elev.z:Clade", 
#                       spaghetti=TRUE, ndraws=150),
#                       points=TRUE,
#                       point_args=c(alpha=2/3, size=1), mean=FALSE)

interact_plot(trbc.m4.ce, pred = elev.z, modx = Clade,
              colors=c("#F4CD0B", "#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F", "#E786B7", "#EE4024", "#03446B"),
              plot.points=TRUE, partial.residuals = FALSE, interval = FALSE)
              # Order: Bee, Brilliant, Coquette, Emerald, Hermit, Mango, Mountain-Gem, Patagona, Topaz
              # Manually specify colors because package doesn't handle more than 7 automatically
...
# Print model summaries 
sink("trbc_CladeAnalysis_brms_model_summaries.txt",append = TRUE)
summary(trbc.m1, waic=TRUE)
summary(trbc.m4, waic=TRUE)
summary(trbc.m4.c, waic=TRUE)
summary(trbc.m4.ce, waic=TRUE)
sink()
# print(trbc.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
trbc.m1.waic <- waic(trbc.m1)
trbc.m4.waic <- waic(trbc.m4)
trbc.m4.c.waic <- waic(trbc.m4.c)
trbc.m4.ce.waic <- waic(trbc.m4.ce)
trbc.clade.waics.red <- cbind(trbc.m1.waic, trbc.m4.waic, trbc.m4.c.waic, trbc.m4.ce.waic); trbc.clade.waics.red
write.csv(trbc.clade.waics.red, "trbc_CladeAnalysis_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Variance explained by Clade
hyp.sp.trbc.m4.c <- paste(
  "sd_Clade__Intercept^2 /", 
  "(sd_Clade__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.sp.trbc.m4.c <- hypothesis(trbc.m4.c, hyp.sp.trbc.m4.c, class = NULL))
plot(hyp.sp.trbc.m4.c)
# 4% variance explained by Clade

# hyp.sp.trbc.m4.c <- "sd_Clade__Intercept^2 / (sd_Clade__Intercept^2 + sigma^2) = 0"
# (hyp.sp.trbc.m4.c <- hypothesis(trbc.m4.c, hyp.sp.trbc.m4.c, class = NULL))
#  
...
# if 2*standard error > delta LOOIC, models aren't necessarily distinguishable
library(loo)
sink("trbc_CladeAnalysis_brms_models_looic.txt", append=FALSE)
LOO(trbc.m1, trbc.m4, trbc.m4.c, trbc.m4.ce, reloo=FALSE) 
sink()
# According to LOO, model 5 is top model 
...
# Calculate Bayesian R^2:
bayes_R2(trbc.m4) # 0.169
bayes_R2(trbc.m4.c) # 0.171
bayes_R2(trbc.m4.ce) # 0.185
```

Quick summary of top models from Clade analysis:  
1) Model 4: Full model  
2) Model 4: Full model + (1|Clade) *marginally better than m4, 8% variance explained by clade.
3) Model 4.ce: Full model w/ clade:elev.z.
4) All performed substantially better than the intercept-only null model. 
...
save(mcv.m1, file="Rdata_files/Rdata_files/mcv.m1_Intercept-Only.RData") # save model
#load("Rdata_files/Rdata_files/mcv.m1_Intercept-Only.Rdata") 
...
save(mcv.m2, file="Rdata_files/mcv.m2_RandomEffectsOnly.RData") # save model
load("Rdata_files/mcv.m2_RandomEffectsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mcv.m3, file="Rdata_files/mcv.m3_FullModel_PredictorsOnly.RData") # save model
load("Rdata_files/mcv.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mcv.m4, file="Rdata_files/mcv.m4_FullModel_SpeciesREOnly.RData") # save model
load("Rdata_files/mcv.m4_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
})
...
#   sample_prior = TRUE # save priors 
# )
# save(mcv.m4.quad, file="Rdata_files/mcv.m4.quad_FullModel_SpeciesREOnly.RData") # save model
# #load("Rdata_files/mcv.m4.quad_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
```
...
# Model 3 
color_scheme_set("blue") # nice default
mcv.m3.p1 <- mcmc_plot(mcv.m3, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
...
# Model 4
#color_scheme_set("red") # nice default
mcv.m4.p1 <- mcmc_plot(mcv.m4, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z","b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# # Model 4 - quadratic
# #color_scheme_set("red") # nice default
# mcv.m4.p1.quad <- mcmc_plot(mcv.m4.quad, variable=c("b_elev.z", "b_Ielev.zE2", 
...
save(mcv.m5, file="Rdata_files/mcv.m5_ReducedModel_PredictorsOnly.RData") # save model
load("Rdata_files/mcv.m5_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mcv.m6, file="Rdata_files/mcv.m6_ReducedModel_SpeciesREOnly.RData") # save model
load("Rdata_files/mcv.m6_ReducedModel_SpeciesREOnly.RData") 
...
#   sample_prior = TRUE # save priors 
# )
# save(mcv.m6.quad, file="Rdata_files/mcv.m6.quad_ReducedModel_SpeciesREOnly.RData") # save model
#load("Rdata_files/mcv.m6.quad_ReducedModel_SpeciesREOnly.RData") 
```
...
# Model 5 
color_scheme_set("blue") # nice default
mcv.m5.p1 <- mcmc_plot(mcv.m5, variable=c("b_elev.z", "b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
mcv.m6.p1 <- mcmc_plot(mcv.m6, variable=c("b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI
                                    prob=0.50, # 50% inner CI
                                    point_est="mean") + # mean point est; default is median
...
# # Model 6.quad 
# color_scheme_set("blue") # nice default
# mcv.m6.quad.p1 <- mcmc_plot(mcv.m6.quad, variable=c("b_elev.z", "b_temp.z"),
#                                     prob_outer=0.95, # 95% outer CI 
#                                     prob=0.50, # 50% inner CI 
#                                     point_est="mean") + # mean point est; default is median 
...
# you can also use something called moment_match=TRUE in loo, but to do this need save_variable=TRUE in models and that 
...
mcv.m6.unstandardized <- 
  update(mcv.m6,
        ...
        chains = 4, cores = 4)
save(mcv.m6.unstandardized, file="Rdata_files/mcv.m6.unstandardized_Predictors_SpeciesRE.RData") # save model
})
# load("Rdata_files/mcv.m6.unstandardized_Predictors_SpeciesRE.RData")
...
# MCV: POST HOC ANALYSIS OF CLADE AND ELEVATION RELATIONSHIPS 
Not in paper but explored as a supplemental analysis.
```{r}
# Load intercept only and top models (if not already loaded)
load("Rdata_files/mcv.m6_ReducedModel_SpeciesREOnly.RData") 
load("Rdata_files/mcv.m1_Intercept-Only.Rdata") 

# MODEL 6.c: TOP MODEL WITH CLADE RE ADDED 
mcv.m6.c <- brm(
  formula = bf(mcv ~ 1 + temp.z + (1|species) + (1|Clade)),
  data = final.mcv,
  family = skew_normal(), 
  cores = 4,
  chains = 4,
...
save(mcv.m6.c, file="Rdata_files/mcv.m6.c_TopModel_CladeREAdded.RData") # save model
#load("Rdata_files/mcv.m6.c_TopModel_CladeREAdded.RData") 

# MODEL 6.ce: TOP MODEL WITH CLADE*ELEV INTERACTION
mcv.m6.ce <- brm(
  formula = bf(mcv ~ 1 + temp.z + Clade:elev.z + (1|species)),
  data = final.mcv,
  family = skew_normal(), 
  cores = 4,
  chains = 4,
...
save(mcv.m6.ce, file="Rdata_files/mcv.m6.ce_TopModel_CladeElevInteraction.RData") # save model
#load("Rdata_files/mcv.m6.ce_TopModel_CladeElevInteraction.RData") 


# Fit assessment
bayesplot::pp_check(mcv.m6.ce, resp = "mcv", ndraws = 100)
mcmc_plot(mcv.m6.c, type = "acf_bar")

# associated with each residual
final.mcv %>%
  add_residual_draws(mcv.m6.ce) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
final.mcv %>%
  add_predicted_draws(mcv.m6.ce) %>%
  summarise(
    p_residual = mean(.prediction < mcv), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
# Model 6.c 
color_scheme_set("blue") # nice default
mcv.m6.c.p1 <- mcmc_plot(mcv.m6.c, variable=c("b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mcv]", title = "mcv.m6.c: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Temp"))   
mcv.m6.c.p1
ggsave(mcv.m6.c.p1, filename = "mcv.m6.c.p1_mcv_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mcv.m6.c)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
...
#color_scheme_set("red") # nice default
#mcmc_plot(mcv.m6.ce)
mcv.m6.ce.p1 <- mcmc_plot(mcv.m6.ce, variable=c("b_temp.z", "b_CladeBee:elev.z",
                                    "b_CladeBrilliant:elev.z", "b_CladeCoquette:elev.z", "b_CladeEmerald:elev.z", 
                                    "b_CladeHermit:elev.z", "b_CladeMango:elev.z", "b_CladeMountainMgem:elev.z", 
                                    "b_CladePatagona:elev.z", "b_CladeTopaz:elev.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mcv]", title = "mcv.m6.ce: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Temp", "Elev*Bee", "Elev*Brilliant", "Elev*Coquette", "Elev*Emerald", "Elev*Hermit", "Elev*Mango", "Elev*MtnGem", "Elev*Patagona", "Elev*Topaz"))   
mcv.m6.ce.p1
ggsave(mcv.m6.ce.p1, filename = "mcv.m6.ce.p1_mcv_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mcv.m6.ce)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
# *Bee is the reference clade

# Plot interactions
plot(conditional_effects(mcv.m6.ce,
                      effects="elev.z:Clade", 
                      spaghetti=TRUE, ndraws=150),
                      points=TRUE,
                      point_args=c(alpha=2/3, size=1), mean=FALSE)

interact_plot(mcv.m6.ce, pred = elev.z, modx = Clade,
              colors=c("#F4CD0B", "#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F", "#E786B7", "#EE4024", "#03446B"),
              plot.points=TRUE, partial.residuals = FALSE, interval = FALSE)
              # Order: Bee, Brilliant, Coquette, Emerald, Hermit, Mango, Mountain-Gem, Patagona, Topaz
              # Manually specify colors because package doesn't handle more than 7 automatically
...
# Print model summaries 
sink("mcv_CladeAnalysis_brms_model_summaries.txt",append = TRUE)
summary(mcv.m1, waic=TRUE)
summary(mcv.m6, waic=TRUE)
summary(mcv.m6.c, waic=TRUE)
summary(mcv.m6.ce, waic=TRUE)
sink()
# print(mcv.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%
...
mcv.m1.waic <- waic(mcv.m1)
mcv.m6.waic <- waic(mcv.m6)
mcv.m6.c.waic <- waic(mcv.m6.c)
mcv.m6.ce.waic <- waic(mcv.m6.ce)
mcv.clade.waics.red <- cbind(mcv.m1.waic, mcv.m6.waic, mcv.m6.c.waic, mcv.m6.ce.waic); mcv.clade.waics.red
write.csv(mcv.clade.waics.red, "mcv_CladeAnalysis_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Variance explained by Clade
hyp.sp.mcv.m6.c <- paste(
  "sd_Clade__Intercept^2 /", 
  "(sd_Clade__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.sp.mcv.m6.c <- hypothesis(mcv.m6.c, hyp.sp.mcv.m6.c, class = NULL))
plot(hyp.sp.mcv.m6.c)
# 2% variance explained by Clade

# hyp.sp.mcv.m6.c <- "sd_Clade__Intercept^2 / (sd_Clade__Intercept^2 + sigma^2) = 0"
# (hyp.sp.mcv.m6.c <- hypothesis(mcv.m6.c, hyp.sp.mcv.m6.c, class = NULL))
#  
...
# if 2*standard error > delta LOOIC, models aren't necessarily distinguishable
library(loo)
sink("mcv_CladeAnalysis_brms_models_looic.txt", append=FALSE)
LOO(mcv.m1, mcv.m6, mcv.m6.c, mcv.m6.ce, reloo=FALSE) 
sink()
# According to LOO, model 5 is top model 
...
# Calculate Bayesian R^2:
bayes_R2(mcv.m6) # 0.094
bayes_R2(mcv.m6.c) # 0.0.096
bayes_R2(mcv.m6.ce) # 0.118
```

Quick summary of top models from Clade analysis:  
1) Model 6.ce: Full model + clade*elev interaction (though this essentially ranked the same as original best model)  
1) Model 6: Full model  
3) Model 6.c: Full model w/ + (1|Clade); 2% of the variation explained by Clade. 
4) All performed substantially better than the intercept-only null model. 
...
save(mch.m1, file="Rdata_files/mch.m1_Intercept-Only.RData") # save model
#load("Rdata_files/mch.m1_Intercept-Only.Rdata") 
...
save(mch.m2, file="Rdata_files/mch.m2_RandomEffectsOnly.RData") # save model
load("Rdata_files/mch.m2_RandomEffectsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mch.m3, file="Rdata_files/mch.m3_FullModel_PredictorsOnly.RData") # save model
# Run time: ~1 minute
#load("Rdata_files/mch.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mch.m4, file="Rdata_files/mch.m4_FullModel_SpeciesREOnly.RData") # save model
# ~3 mins
...
# Model 3 
color_scheme_set("blue") # nice default
mch.m3.p1 <- mcmc_plot(mch.m3, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass","b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# Model 4
#color_scheme_set("red") # nice default
mch.m4.p1 <- mcmc_plot(mch.m4, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass","b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
save(mch.m5, file="Rdata_files/mch.m5_ReducedModel_PredictorsOnly.RData") # save model
#load("Rdata_files/mch.m5_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mch.m6, file="Rdata_files/mch.m6_ReducedModel_PhyloREOnly.RData") # save model
load("Rdata_files/mch.m6_ReducedModel_PhyloREOnly.RData") 
```
...
# Model 5 
color_scheme_set("blue") # nice default
mch.m5.p1 <- mcmc_plot(mch.m5, variable=c("b_elev.z", "b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# # Model 6
# color_scheme_set("teal") # nice default
mch.m6.p1 <- mcmc_plot(mch.m6, variable=c("b_elev.z", "b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI
                                    prob=0.50, # 50% inner CI
                                    point_est="mean") + # mean point est; default is median
...
mch.m6.unstandardized <- 
  update(mch.m6,
        ...
        chains = 4, cores = 4)
save(mch.m6.unstandardized, file="Rdata_files/mch.m6.unstandardized_Predictors_SpeciesRE.RData") # save model
})
#load("Rdata_files/mch.m6.unstandardized_Predictors_SpeciesRE.RData")
...
# lower & upper CIs
print(mch.m6.unstandardized, digits=10) # print model with 10 sig figs to pull full 95% CI estimates (this is lazy way)
round(-0.0011951528*1000, 2) # lower CI (-1.2 pg)
round(0.0002678837*1000, 2) # upper CI (550,000 cells per microliter)
```
...
# MCH: POST HOC ANALYSIS OF CLADE AND ELEVATION RELATIONSHIPS 
Not in paper but explored as a supplemental analysis.
```{r}
# Load intercept only and top models (if not already loaded)
load("Rdata_files/mch.m6_ReducedModel_SpeciesREOnly.RData") 
load("Rdata_files/mch.m1_Intercept-Only.Rdata") 

# MODEL 6.c: TOP MODEL WITH CLADE RE ADDED 
mch.m6.c <- brm(
  formula = bf(mch ~ 1 + elev.z + temp.z + (1|species) + (1|Clade)),
  data = final.mch,
  family = skew_normal(), 
  cores = 4,
  chains = 4,
...
save(mch.m6.c, file="Rdata_files/mch.m6.c_TopModel_CladeREAdded.RData") # save model
#load("Rdata_files/mch.m6.c_TopModel_CladeREAdded.RData") 

# MODEL 6.ce: TOP MODEL WITH CLADE*ELEV INTERACTION
mch.m6.ce <- brm(
  formula = bf(mch ~ 1 + elev.z + temp.z + elev.z:Clade + (1|species)),
  data = final.mch,
  family = skew_normal(), 
  cores = 4,
  chains = 4,
...
save(mch.m6.ce, file="Rdata_files/mch.m6.ce_TopModel_CladeElevInteraction.RData") # save model
#load("Rdata_files/mch.m6.ce_TopModel_CladeElevInteraction.RData") 


# Fit assessment
bayesplot::pp_check(mch.m6.ce, resp = "mch", ndraws = 100)
mcmc_plot(mch.m6.c, type = "acf_bar")

# associated with each residual
final.mch %>%
  add_residual_draws(mch.m6.ce) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
final.mch %>%
  add_predicted_draws(mch.m6.ce) %>%
  summarise(
    p_residual = mean(.prediction < mch), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
# Model 6.c 
color_scheme_set("blue") # nice default
mch.m6.c.p1 <- mcmc_plot(mch.m6.c, variable=c("b_elev.z", "b_temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mch]", title = "mch.m6.c: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Temp"))   
mch.m6.c.p1
ggsave(mch.m6.c.p1, filename = "mch.m6.c.p1_mch_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mch.m6.c)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
...
#color_scheme_set("red") # nice default
mcmc_plot(mch.m6.ce)
mch.m6.ce.p1 <- mcmc_plot(mch.m6.ce, variable=c("b_elev.z", "temp.z",
                                    "b_elev.z:CladeBrilliant", "b_elev.z:CladeCoquette", "b_elev.z:CladeEmerald", 
                                    "b_elev.z:CladeHermit", "b_elev.z:CladeMango", "b_elev.z:CladePatagona", 
                                    "b_elev.z:CladeTopaz"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mch]", title = "mch.m6.ce: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation","Temp", "Elev*Brilliant", "Elev*Coquette", "Elev*Emerald", "Elev*Hermit", "Elev*Mango", "Elev*Patagona", "Elev*Topaz"))   
mch.m6.ce.p1
ggsave(mch.m6.ce.p1, filename = "mch.m6.ce.p1_mch_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mch.m6.ce)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
# *Bee is the reference clade
levels(final.mch$Clade)

# Plot interactions
plot(conditional_effects(mch.m6.ce,
                      effects="elev.z:Clade", 
                      spaghetti=TRUE, ndraws=150),
                      points=TRUE,
                      point_args=c(alpha=2/3, size=1), mean=FALSE)

interact_plot(mch.m6.ce, pred = elev.z, modx = Clade,
              colors=c("#F4CD0B", "#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F", "#EE4024", "#03446B"),
              plot.points=TRUE, partial.residuals = FALSE, interval = FALSE)
              # Order: Bee, Brilliant, Coquette, Emerald, Hermit, Mango, Patagona, Topaz
              # Manually specify colors because package doesn't handle more than 7 automatically
...
# Print model summaries 
sink("mch_CladeAnalysis_brms_model_summaries.txt",append = TRUE)
summary(mch.m1, waic=TRUE)
summary(mch.m6, waic=TRUE)
summary(mch.m6.c, waic=TRUE)
summary(mch.m6.ce, waic=TRUE)
sink()
# print(mch.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%
...
mch.m1.waic <- waic(mch.m1)
mch.m6.waic <- waic(mch.m6)
mch.m6.c.waic <- waic(mch.m6.c)
mch.m6.ce.waic <- waic(mch.m6.ce)
mch.clade.waics.red <- cbind(mch.m1.waic, mch.m6.waic, mch.m6.c.waic, mch.m6.ce.waic); mch.clade.waics.red
write.csv(mch.clade.waics.red, "mch_CladeAnalysis_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Variance explained by Clade
hyp.sp.mch.m6.c <- paste(
  "sd_Clade__Intercept^2 /", 
  "(sd_Clade__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.sp.mch.m6.c <- hypothesis(mch.m6.c, hyp.sp.mch.m6.c, class = NULL))
plot(hyp.sp.mch.m6.c)
# 1% variance explained by Clade

# hyp.sp.mch.m6.c <- "sd_Clade__Intercept^2 / (sd_Clade__Intercept^2 + sigma^2) = 0"
# (hyp.sp.mch.m6.c <- hypothesis(mch.m6.c, hyp.sp.mch.m6.c, class = NULL))
#  
...
# if 2*standard error > delta LOOIC, models aren't necessarily distinguishable
library(loo)
sink("mch_CladeAnalysis_brms_models_looic.txt", append=FALSE)
LOO(mch.m1, mch.m6, mch.m6.c, mch.m6.ce, reloo=FALSE) 
sink()
# According to LOO, model 5 is top model 
...
# Calculate Bayesian R^2:
bayes_R2(mch.m6) # 0.092
bayes_R2(mch.m6.c) # 0.095
bayes_R2(mch.m6.ce) # 0.112
```

Quick summary of top models from Clade analysis:  
1) Model 6: Full model 
2) Model 6: Full model + (1|Clade) *marginally worse than m6*; 1% variance explained by clade.  
3) Model 6.ce: Full model w/ clade:elev.z.
4) All performed substantially better than the intercept-only null model. 
...
save(mchc.m1, file="Rdata_files/mchc.m1_Intercept-Only.RData") # save model
#load("Rdata_files/mchc.m1_Intercept-Only.Rdata") 
...
save(mchc.m2, file="Rdata_files/mchc.m2_RandomEffectsOnly.RData") # save model
#load("Rdata_files/mchc.m2_RandomEffectsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mchc.m3, file="Rdata_files/mchc.m3_FullModel_PredictorsOnly.RData") # save model
#load("Rdata_files/mchc.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mchc.m4, file="Rdata_files/mchc.m4_FullModel_SpeciesREOnly.RData") # save model
#load("Rdata_files/mchc.m4_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
```
...
# Model 3 
color_scheme_set("blue") # nice default
mchc.m3.p1 <- mcmc_plot(mchc.m3, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z","b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
# Model 4
# color_scheme_set("teal") # nice default
mchc.m4.p1 <- mcmc_plot(mchc.m4, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass","b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
...
save(mchc.m5, file="Rdata_files/mchc.m5_ReducedModel_PredictorsOnly.RData") # save model
#load("Rdata_files/mchc.m5_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mchc.m6, file="Rdata_files/mchc.m6_ReducedModel_SpeciesREOnly.RData") # save model
load("Rdata_files/mchc.m6_ReducedModel_SpeciesREOnly.RData") 
```
...
# Model 5 
color_scheme_set("blue") # nice default
mchc.m5.p1 <- mcmc_plot(mchc.m5, variable=c("b_mass.z","b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.mass.z","b_intravar.temp.z", "b_intravar.precip.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
...
# # Model 6
# color_scheme_set("teal") # nice default
mchc.m6.p1 <- mcmc_plot(mchc.m6, variable=c("b_intravar.mass.z", "b_intravar.temp.z"),
                                    prob_outer=0.95, # 95% outer CI
                                    prob=0.50, # 50% inner CI
                                    point_est="mean") + # mean point est; default is median
...
mchc.m6.unstandardized <- 
  update(mchc.m6,
        ...
        chains = 4, cores = 4)
save(mchc.m6.unstandardized, file="Rdata_files/mchc.m6.unstandardized_Predictors_SpeciesRE.RData") # save model
})
# load("Rdata_files/mchc.m6.unstandardized_Predictors_SpeciesRE.RData")
...
1) Model 6: Reduced model w/ just intravar.temp and intravar.mass and +1 species; 15% variation explained by species; R^2 = 0.115 
2) Model 4: Full + (1|species); 18% of variance explained by species; R^2 = 0.134. Only *marginally* lower than Model 6. 
...
# MCHC: POST HOC ANALYSIS OF CLADE AND ELEVATION RELATIONSHIPS 
Not in paper but explored as a supplemental analysis.
```{r}
# Load intercept only and top models (if not already loaded)
load("Rdata_files/mchc.m6_ReducedModel_SpeciesREOnly.RData") 
...
# MODEL 6.c: TOP MODEL WITH CLADE RE ADDED 
mchc.m6.c <- brm(
  formula = bf(mchc ~ 1 + intravar.mass.z + intravar.temp.z + (1|species) + (1|Clade)),
  data = final.mchc,
  family = student(), 
  cores = 4,
  chains = 4,
...
save(mchc.m6.c, file="Rdata_files/mchc.m6.c_TopModel_CladeREAdded.RData") # save model
#load("Rdata_files/mchc.m6.c_TopModel_CladeREAdded.RData") 

# MODEL 6.ce: TOP MODEL WITH CLADE*ELEV INTERACTION
mchc.m6.ce <- brm(
  formula = bf(mchc ~ 1 + intravar.mass.z + intravar.temp.z + elev.z:Clade + (1|species)),
  data = final.mchc,
  family = student(), 
  cores = 4,
  chains = 4,
...
save(mchc.m6.ce, file="Rdata_files/mchc.m6.ce_TopModel_CladeElevInteraction.RData") # save model
#load("Rdata_files/mchc.m6.ce_TopModel_CladeElevInteraction.RData") 


# Fit assessment
bayesplot::pp_check(mchc.m6.ce, resp = "mchc", ndraws = 100)
mcmc_plot(mchc.m6.c, type = "acf_bar")

# associated with each residual
final.mchc %>%
  add_residual_draws(mchc.m6.ce) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

# qqPlots
final.mchc %>%
  add_predicted_draws(mchc.m6.ce) %>%
  summarise(
    p_residual = mean(.prediction < mchc), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
# Model 6.c 
color_scheme_set("blue") # nice default
mchc.m6.c.p1 <- mcmc_plot(mchc.m6.c, variable=c("b_intravar.mass.z", "b_intravar.temp.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mchc]", title = "mchc.m6.c: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("IntraVar: Mass","IntraVar: Temp"))   
mchc.m6.c.p1
ggsave(mchc.m6.c.p1, filename = "mchc.m6.c.p1_mchc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mchc.m6.c)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z

# Model 6
color_scheme_set("blue") # nice default
mchc.m6.ce.p1 <- mcmc_plot(mchc.m6.ce, variable=c("b_intravar.mass.z", "intravar.temp.z", "b_elev.z:CladeBee",
                                    "b_elev.z:CladeBrilliant", "b_elev.z:CladeCoquette", "b_elev.z:CladeEmerald", 
                                    "b_elev.z:CladeHermit", "b_elev.z:CladeMango", "b_elev.z:CladePatagona", 
                                    "b_elev.z:CladeTopaz"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mchc]", title = "mchc.m6.ce: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("IntraVar: Mass","IntraVar: Temp", "Elev*Bee", "Elev*Brilliant", "Elev*Coquette", "Elev*Emerald", "Elev*Hermit", "Elev*Mango", "Elev*Patagona", "Elev*Topaz"))   
mchc.m6.ce.p1
ggsave(mchc.m6.ce.p1, filename = "mchc.m6.ce.p1_mchc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mchc.m6.ce)
# Predictors whose 95% CIs do NOT overlap zero (what reduced model will be): elev.z
# *Bee is the reference clade

# Plot interactions
plot(conditional_effects(mchc.m6.ce,
                      effects="elev.z:Clade", 
                      spaghetti=TRUE, ndraws=150),
                      points=TRUE,
                      point_args=c(alpha=2/3, size=1), mean=FALSE)

interact_plot(mchc.m6.ce, pred = elev.z, modx = Clade,
              colors=c("#F4CD0B", "#9E9285", "#04A5A5", "#97B25B", "#916EF9", "#F79E0F", "#EE4024", "#03446B"),
              plot.points=TRUE, partial.residuals = FALSE, interval = FALSE)
              # Order: Bee, Brilliant, Coquette, Emerald, Hermit, Mango, Patagona, Topaz
              # Manually specify colors because package doesn't handle more than 7 automatically
...
# Print model summaries 
sink("mchc_CladeAnalysis_brms_model_summaries.txt",append = TRUE)
summary(mchc.m1, waic=TRUE)
summary(mchc.m6, waic=TRUE)
summary(mchc.m6.c, waic=TRUE)
summary(mchc.m6.ce, waic=TRUE)
sink()
# print(mchc.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%
...
mchc.m1.waic <- waic(mchc.m1)
mchc.m6.waic <- waic(mchc.m6)
mchc.m6.c.waic <- waic(mchc.m6.c)
mchc.m6.ce.waic <- waic(mchc.m6.ce)
mchc.clade.waics.red <- cbind(mchc.m1.waic, mchc.m6.waic, mchc.m6.c.waic, mchc.m6.ce.waic); mchc.clade.waics.red
write.csv(mchc.clade.waics.red, "mchc_CladeAnalysis_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Variance explained by Clade
hyp.sp.mchc.m6.c <- paste(
  "sd_Clade__Intercept^2 /", 
  "(sd_Clade__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.sp.mchc.m6.c <- hypothesis(mchc.m6.c, hyp.sp.mchc.m6.c, class = NULL))
plot(hyp.sp.mchc.m6.c)
# 4% variance explained by Clade

# hyp.sp.mchc.m6.c <- "sd_Clade__Intercept^2 / (sd_Clade__Intercept^2 + sigma^2) = 0"
# (hyp.sp.mchc.m6.c <- hypothesis(mchc.m6.c, hyp.sp.mchc.m6.c, class = NULL))
#  
...
# if 2*standard error > delta LOOIC, models aren't necessarily distinguishable
library(loo)
sink("mchc_CladeAnalysis_brms_models_looic.txt", append=FALSE)
LOO(mchc.m1, mchc.m6, mchc.m6.c, mchc.m6.ce, reloo=FALSE) 
sink()
# According to LOO, model 5 is top model 
...
# Calculate Bayesian R^2:
bayes_R2(mchc.m6) # 0.115
bayes_R2(mchc.m6.c) # 0.118
bayes_R2(mchc.m6.ce) # 0.132
```

Quick summary of top models from Clade analysis:  
1) Model 6.c: Full model + (1|Clade) *marginally better than m6, 4% variance explained by clade.  
2) Model 6: Full model  
3) Model 6.ce: Full model w/ clade:elev.z.
4) All performed substantially better than the intercept-only null model. 
...
###### TUTORIALS and other helpful pages that I referenced during analysis below this line #######
...
Cat*con interactions in brms: https://ibecav.netlify.app/post/exploring-vaccine-effectiveness-through-bayesian-regression-part-4/


**Note that I pasted some tests w/ Hb in tidybayes into CUT script - didn't end up using them, partly becuase I don't think aesthetics looked as good. 
