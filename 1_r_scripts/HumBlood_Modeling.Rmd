...
(Dropping latitude as a predictor)
...
source("Rfunctions.R") # Nora's functions
...
final <- read.csv("AllHummingbirdBlood_Wrangled_withBioClim_andPCA_10-27-20.csv", stringsAsFactors = TRUE)
...
Further clean for modeling, impute missing mass values, and generate response variable-specific subets that you'll read into the phylogeny file to create trees needed for each model set. 

# Subset for modeling
```{r}
# Drop variables you won't need for modeling, which reduces 'clunk' in dataset and problems associated w/ NA values 
# final <- subset(final, select = -c(prep_num, # 2.64% missing values 
#                                    cat_owner,
#                                    elev_min,
#                                    elev_max,
#                                    elev_midpt,
#                                    elev_range
#                                     ) )
# Might be able to include more things to drop here 
```


# Make Model-specific subsets that would be dropped from big dataset with na.omit 
```{r}
...
# Calculate intraspecific variation 
...
# Calculate individual differences from species mean value (aka intraspecific variation, or 'intravar'); within-sp diffs
final$intravar.elev <- final$elev - final$spec.mean.elev 
final$intravar.elev.pos <- final$elev_position - final$spec.mean.elev.pos
...
final$elev.position.z <- standardize(final$elev_position)
final$intravar.elev.z <- standardize(final$intravar.elev) # Now standardize intraspecific variation 
final$intravar.elev.pos.z <- standardize(final$intravar.elev.pos)
...
# Hb subset - 1085 observations
final.hb <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, hb, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.position.z, temp.z, precip.z, b13, b83, genotype, intravar.elev, intravar.elev.pos, intravar.mass, intravar.temp, intravar.precip)) 
final.hb <- na.omit(final.hb)
str(final.hb) # looks good, nothing to coerce 

# Hct subset - 1138 observations
final.hct <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, hct, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.position.z, temp.z, precip.z, b13, b83, genotype, intravar.elev, intravar.elev.pos, intravar.mass, intravar.temp, intravar.precip)) 
final.hct <- na.omit(final.hct)
str(final.hct) # looks good, nothing to coerce 

# trbc subset - 789 observations
final.trbc <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, trbc, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.position.z, temp.z, precip.z, b13, b83, genotype, intravar.elev, intravar.elev.pos, intravar.mass, intravar.temp, intravar.precip)) 
final.trbc <- na.omit(final.trbc)
str(final.trbc) # looks good, nothing to coerce

# mcv subset - 773 observations
final.mcv <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mcv, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.position.z, temp.z, precip.z, b13, b83, genotype, intravar.elev, intravar.elev.pos, intravar.mass, intravar.temp, intravar.precip)) 
final.mcv <- na.omit(final.mcv)
str(final.mcv) # looks good, nothing to coerce 

# mch subset - 744 observations
final.mch <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mch, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.position.z, temp.z, precip.z, b13, b83, genotype, intravar.elev, intravar.elev.pos, intravar.mass, intravar.temp, intravar.precip)) 
final.mch <- na.omit(final.mch)
str(final.mch) # looks good, nothing to coerce 

# mchc subset - 976 observations
final.mchc <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mchc, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.position.z, temp.z, precip.z, b13, b83, genotype, intravar.elev, intravar.elev.pos, intravar.mass, intravar.temp, intravar.precip)) 
...
# Check contMaps for trait and sampling outliers
NOTE: We may cut this chunk since we might not be removing these sampling value "outliers". 

Before moving on with pre-modeling verification steps and modeling itself, work up continuous trait maps for each of the 6 blood parameters to take a look at spread of values. Based on preliminary modeling efforts, we expect low phylogenetic signal (i.e. similar branch colors) aross the board. But, in a few cases, extreme values may be driven by low sampling (i.e. n=1 or n<3). We want to carefully consider and possibly exclude these cases. 
```{r}
...
View(just.hb); class(just.hb); typeof(just.hb) # Check data format and structure 

# Create hemoglobin continuous trait phylo object 
hb.cont.tree <- contMap(tree.hb, just.hb, plot=TRUE) 

# Save file to pdf and adjust aesthetics 
pdf(file="hemoglobin.contmap.pdf", width=6, height=10)
plot(hb.cont.tree, lwd=3, fsize=0.8) # setting xlim changes actual plot dimensions, NOT trait value scale 
# Use outline=FALSE to remove black outlines from branches 
dev.off()
#errorbar.contMap(obj)

# Based on this, consider dropping from Hb dataset: 
...
View(just.hct); class(just.hct); typeof(just.hct) 
hct.cont.tree <- contMap(tree.hct, just.hct, plot=TRUE) # Make contmap tree 
pdf(file="hematocrit.contmap.pdf", width=6, height=10) # Write plot to pdf 
plot(hct.cont.tree, lwd=3, fsize=0.8) 
dev.off()
...
View(just.trbc); class(just.trbc); typeof(just.trbc) 
trbc.cont.tree <- contMap(tree.trbc, just.trbc, plot=TRUE) # Make contmap tree 
pdf(file="trbc.contmap.pdf", width=6, height=10) # Write plot to pdf 
plot(trbc.cont.tree, lwd=3, fsize=0.8) 
dev.off()
...
View(just.mcv); class(just.mcv); typeof(just.mcv) 
mcv.cont.tree <- contMap(tree.mcv, just.mcv, plot=TRUE) # Make contmap tree 
pdf(file="mcv.contmap.pdf", width=6, height=10) # Write plot to pdf 
plot(mcv.cont.tree, lwd=3, fsize=0.8) 
dev.off()
...
View(just.mch); class(just.mch); typeof(just.mch) 
mch.cont.tree <- contMap(tree.mch, just.mch, plot=TRUE) # Make contmap tree 
pdf(file="mch.contmap.pdf", width=6, height=10) # Write plot to pdf 
plot(mch.cont.tree, lwd=3, fsize=0.8) 
dev.off()
...
View(just.mchc); class(just.mchc); typeof(just.mchc) 
mchc.cont.tree <- contMap(tree.mchc, just.mchc, plot=TRUE) # Make contmap tree 
pdf(file="mchc.contmap.pdf", width=6, height=10) # Write plot to pdf 
plot(mchc.cont.tree, lwd=3, fsize=0.8) 
dev.off()
...
```{r}
# Correlation matrix 
cor(final.hb[,c("elev", "elev_position", "tempPC1", "precipPC1", "lat", "mass", "intravar.elev", "intravar.elev.pos", "intravar.lat", "intravar.mass", "intravar.temp", "intravar.precip")])
...
p <- ggpairs(subset(final, select = c(elev.z, elev.position.z, mass.z, temp.z, precip.z, hb, hct, trbc, mcv, mch, mchc))) 
print(p)
```
...
# MODELING 
Remember: you standardized all predictor variables in HumBlood_DataWrangling.Rmd before making subsets, so all variables you need are already standardized. 
...
#final.hb$phylo <- final.hb$species
...
get_prior(hb ~ 1 + elev.z + elev.position.z + mass.z + temp.z + precip.z + intravar.elev + intravar.elev.pos + intravar.mass +  intravar.precip + intravar.temp + (1|species), data = final.hb) # removed + (1|gr(phylo, cov=A)) 
...
hb.m3 <- brm(
  formula = bf(hb ~ 1 + elev.z + elev.position.z + mass.z + temp.z + precip.z + genotype +
               intravar.elev + intravar.elev.pos + intravar.mass + intravar.temp + intravar.precip),
  data = final.hb,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
# hb.m4 <- brm(
#   formula = bf(hb ~ 1 + elev.z + elev.position.z + lat.z + mass.z + temp.z + precip.z + genotype +
...
#   # )
# )
# save(hb.m4, file="hb.m4_FullModel_PhyloREOnly.RData") # save model 
# #  ~3 mins run time 
# load("hb.m4_FullModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
...
save(hb.m5, file="hb.m5_FullModel_SpeciesREOnly.RData") # save model
# ~3 mins
load("hb.m5_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
...
#   # )
# )
# save(hb.m6, file="hb.m6_FullModel_Phylo&SpeciesRE.RData") # save model
# # ~ 3 mins run time 
# load("hb.m6_FullModel_Phylo&SpeciesRE.RData") # If loading from pre-saved file and not re-running
...
mcmc_plot(hb.m5, type = "trace") 
plot(hb.m5, N = 2, ask = FALSE)
...
pp_check(hb.m5, resp = "hb", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(hb.m5, type = "stat", stat = 'median', nsamples = 100)
...
plot(conditional_effects(hb.m5), points = TRUE) 

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(hb.m5)  # adjust prob to get 95% CI
...
ranef(hb.m5, groups="species") #, probs = 0.95)
```
...
hb.m3.p1 <- mcmc_plot(hb.m3, pars=c("b_elev.z", "b_elev.position.z","b_mass.z","b_temp.z","b_precip.z",
                                    ...
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.mass",
...
summary(hb.m3)
# Predictors whose 95% CIs do NOT overlap zero: elev.z, mass.z, genotype (Gly-Ser)
# Reduced hb.m3 will be: elev.z + mass.z + genotype
...
# Reduced hb.m4 will be: elev.z

# Model 5
#color_scheme_set("red") # nice default
hb.m5.p1 <- mcmc_plot(hb.m5, pars=c("b_elev.z", "b_elev.position.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
# Reduced hb.m5 will be: elev.position.z

# # Model 6
# #color_scheme_set("gray") # nice default
# hb.m6.p1 <- mcmc_plot(hb.m6, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
#                                     "b_genotypeGlyMSer", "b_genotypeSerMSer",
#                                     "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
#                                     "b_intravar.temp", "b_intravar.precip"),
#                                     prob_outer=0.95, # 95% outer CI 
...
# Reduced hb.m6 will be: NA - can't reduced .m6 further 

####

# Summary of reducing models: 
# m7 (reduced m3) will be: elev.position.z + genotype
# m8 (reduced m4) will be: elev.z
# m9 (reduced m5) will be: elev.position.z
# m10 (reduced m6) will be: NA - m6 can't be further reduced as all 95% CIs of predictors overlap zero 
```
...
hb.waics <- cbind(hb.m1.waic, hb.m2.waic, hb.m3.waic, hb.m5.waic); hb.waics
...
bayes_R2(hb.m3) # 0.15
#bayes_R2(hb.m4) # 0.21
bayes_R2(hb.m5) # 0.227
#bayes_R2(hb.m6) # 0.229
```


# Hb: Calculate phylogenetic signal for big model sets for full models (m1-m6)
...
# Reduced models that have phylogenetic grouping variable: m8, m10

# CALCULATE PHYLOGENETIC SIGNAL 

# m2 phylo signal   
# hyp.hb.m2 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
# (hyp.hb.m2 <- hypothesis(hb.m2, hyp.hb.m2, class = NULL))
# plot(hyp.hb.m2)
# # Phylo signal = 0 
# 
# # m4 phylo signal   
# hyp.hb.m4 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE 
# (hyp.hb.m4 <- hypothesis(hb.m4, hyp.hb.m4, class = NULL))
# plot(hyp.hb.m4)
# # Phylo signal = 0.01 = ~1%
# 
# # m6 phylo signal   
# hyp.hb.m6 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
# (hyp.hb.m6 <- hypothesis(hb.m6, hyp.hb.m6, class = NULL))
# plot(hyp.hb.m6)
# k = 0.0 
...
# m5 species variance 
hyp.sp.hb.m5 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hb.m5 <- hypothesis(hb.m5, hyp.sp.hb.m5, class = NULL))
plot(hyp.sp.hb.m5)
...
# plot(hyp.sp.hb.m6)
# ~9% variance explained by species 
```


# Hb: RUN REDUCED MODELS (m7-m10)
```{r}
# MODEL 7: REDUCED MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
hb.m7 <- brm(
  formula = bf(hb ~ 1 + elev.z + mass.z + genotype),
  data = final.hb,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(hb.m7, file="hb.m7_ReducedModel_PredictorsOnly.RData") # save model
load("hb.m7_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running


# MODEL 8: REDUCED MODEL WITH ONLY PHYLO RANDOM EFFECT
# hb.m8 <- brm(
#   formula = bf(hb ~ 1 + elev.z + (1|gr(phylo, cov=A))),
#   data = final.hb,
#   family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
#   data2 = list(A = A), # Phylogeny
#   cores = 4,
...
hb.m9 <- brm(
  formula = bf(hb ~ 1 + elev.position.z + (1|species)),
  data = final.hb,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(hb.m9, file="hb.m9_ReducedModel_SpeciesREOnly.RData") # save model
load("hb.m9_ReducedModel_SpeciesREOnly.RData") 

##### NO MODEL 10 FOR HB BECAUSE FULL MODEL CAN'T BE REDUCED 
# MODEL 10: REDUCED MODEL WITH PHYLO AND SPECIES RANDOM EFFECTS
# hb.m10 <- brm(
#   formula = bf(hb ~ 1 + elev.z + elev.position.z + temp.z + precip.z +
#                      intravar.lat + intravar.mass + intravar.tempPC1 + 
#                      intravar.precipPC1 + (1|gr(phylo, cov = A)) + (1|species)),
#   data = final.hb,
#   family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
#   data2 = list(A = A), # Phylogeny
#   cores = 4,
...
#   sample_prior = TRUE # default priors 
# )
# save(hb.m10, file="hb.m10_ReducedModel_Phylo&SpeciesRE.RData") # save model
#load("hb.m10_ReducedModel_Phylo&SpeciesRE.RData") # If loading from pre-saved file and not re-running
```


# HB: Summarize and check fit for reduced models (m7-m10)
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(hb.m9, type = "trace") 
plot(hb.m9, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(hb.m9, resp = "hb", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(hb.m9, type = "stat", stat = 'median', nsamples = 100)
...
# Pale bars = you want all heading towards zero to show they're not correlating w/ each other across the chains 
# fine to be pos and neg, jumping around to get to zero
mcmc_plot(hb.m9, type = "acf_bar")

# plot conditional effects for full model (note: you do this separately below)
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(hb.m9), points = TRUE) 

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(hb.m9)  # adjust prob to get 95% CI
...
# Print group level variable (in log odds)
# ranef is still an array, so not an exponent on it, hard to change to probability estimates 
ranef(hb.m8, groups="species") #, probs = 0.95)
```
...
# Model 7 
color_scheme_set("blue") # nice default
hb.m7.p1 <- mcmc_plot(hb.m7, pars=c("b_elev.z", "b_mass.z", "b_genotypeGlyMSer", "b_genotypeSerMSer"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elev", "Mass", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)"))  
hb.m7.p1
ggsave(hb.m7.p1, filename = "hb.m7.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.m7)


# # Model 8
# # color_scheme_set("teal") # nice default
# hb.m8.p1 <- mcmc_plot(hb.m8, pars=c("b_elev.z"),
#                                     prob_outer=0.95, # 95% outer CI 
#                                     prob=0.50, # 50% inner CI 
#                                     point_est="mean") + # mean point est; default is median 
# labs(x="Effect on [Hb]", title = "hb.m8: Posterior mean estimates & 95% Credible Intervals") + 
# scale_y_discrete(labels=c("Elevation")) 
# hb.m8.p1
# ggsave(hb.m8.p1, filename = "hb.m8.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
# summary(hb.m8)


# Model 9
#color_scheme_set("red") # nice default
hb.m9.p1 <- mcmc_plot(hb.m9, pars=c("b_elev.position.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "hb.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation Position"))   
hb.m9.p1
ggsave(hb.m9.p1, filename = "hb.m9.p1_Hb_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.m9)

# No model 10 because model 6 couldn't be further reduced 

```


# Hb: Collate model summaries and WAIC scores for reduced models (m7-m10)
```{r}
# Print model summaries 
sink("hb_reduced_brms_model_summaries.txt",append = TRUE)
summary(hb.m7, waic=TRUE)
#summary(hb.m8, waic=TRUE)
summary(hb.m9, waic=TRUE)
# summary(hb.m10, waic=TRUE)
sink()
# print(hb.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
hb.m7.waic <- waic(hb.m7)
#hb.m8.waic <- waic(hb.m8)
hb.m9.waic <- waic(hb.m9)
# hb.m10.waic <- waic(hb.m10)
hb.waics.red <- cbind(hb.m7.waic, hb.m9.waic); hb.waics.red
...
# Hb: Calculate phylogenetic signal for big model sets for reduced models (m7-m10)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m8 phylo signal   
# hyp.hb.m8 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE 
# (hyp.hb.m8 <- hypothesis(hb.m8, hyp.hb.m8, class = NULL))
# plot(hyp.hb.m8)
# # Phylo signal = 0.01 = ~1%

# m10 phylo signal   
# hyp.hb.m10 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
# (hyp.hb.m10 <- hypothesis(hb.m10, hyp.hb.m10, class = NULL))
# plot(hyp.hb.m10)
# Phylo signal = 0 
...
# m9 species variance 
hyp.sp.hb.m9 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hb.m9 <- hypothesis(hb.m9, hyp.sp.hb.m9, class = NULL))
plot(hyp.sp.hb.m9)
...
LOO(hb.m1, hb.m2, hb.m3, hb.m5, hb.m7, hb.m9, reloo=FALSE) 
sink()
...
1) Model 5: Full model + (1|species) **significantly better than other models 
2) Model 9: Elevation position + (1|species) 
3) Model 3: Full model w/ predictors only  
...
```{r}
# Copy species column and name it 'phylo' for estimation of phylo variance 
final.hct$phylo <- final.hct$species
...
# See what prior choices are being made for us. 
get_prior(hct ~ 1 + elev.z + elev.position.z + lat.z + mass.z + temp.z + precip.z + intravar.elev + intravar.elev.pos + intravar.mass + intravar.lat + intravar.precip + intravar.temp + (1|gr(phylo, cov=A)) + (1|species), data = final.hct)
# I see why the tutorial calls one "phlyo" and one species; it's because you can't have both 
...
hct.m2 <- brm(
  formula = bf(hct ~ 1 + (1|gr(phylo, cov=A)) + (1|species)),
  data = final.hct,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
# MODEL 3: FULL MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
hct.m3 <- brm(
  formula = bf(hct ~ 1 + elev.z + elev.position.z + lat.z + mass.z + temp.z + precip.z + genotype +
               intravar.elev + intravar.elev.pos + intravar.lat + intravar.mass + intravar.temp + intravar.precip),
  data = final.hct,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
...
save(hct.m4, file="hct.m4_FullModel_PhyloREOnly.RData") # save model 
#  ~3 mins run time 
load("hct.m4_FullModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
...
save(hct.m5, file="hct.m5_FullModel_SpeciesREOnly.RData") # save model
# ~3 mins
load("hct.m5_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
...
save(hct.m6, file="hct.m6_FullModel_Phylo&SpeciesRE.RData") # save model
# ~ 3 mins run time 
load("hct.m6_FullModel_Phylo&SpeciesRE.RData") # If loading from pre-saved file and not re-running
...
# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(hct.m3)  # adjust prob to get 95% CI

# Changes the fixed effects (i.e. random variable) to odd's ratio 
exp(fixef(hct.test)[,-2])

# Print group level variable (in log odds)
# ranef is still an array, so not an exponent on it, hard to change to probability estimates 
ranef(hct.test, groups="species") #, probs = 0.95)
```
...
# Model 3 
color_scheme_set("blue") # nice default
hct.m3.p1 <- mcmc_plot(hct.m3, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
summary(hct.m3)
# Predictors whose 95% CIs do NOT overlap zero: elev.position.z 
# Reduced hct.m3 will be: elev.position.z 

# Model 4
# color_scheme_set("teal") # nice default
hct.m4.p1 <- mcmc_plot(hct.m4, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
# Model 6
#color_scheme_set("gray") # nice default
hct.m6.p1 <- mcmc_plot(hct.m6, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
summary(hct.m6)
# Predictors whose 95% CIs do NOT overlap zero: elev.position.z
# Reduced hct.m6 will be: elev.position.z

####

# Summary of reducing models: 
# m7 (reduced m3) will be: elev.position.z 
# m8 (reduced m4) will be: NA
# m9 (reduced m5) will be: elev.position.z
# m10 (reduced m6) will be: elev.position.z
```


#### quick plot for Chris Brown Bag of model 6 posterior probs #####
```{r}
# Model 6
#color_scheme_set("gray") # nice default
bbag_hct_m6 <- mcmc_plot(hct.m6, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
scale_y_discrete(labels=c("Elevation","Elevation Position", "Latitude", "Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Elev", "Intra Var: Elev pos", "Intra Var: Lat", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip"))     
bbag_hct_m6
ggsave(bbag_hct_m6, filename = "bbag_hct.m6.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=3, width=4.5, units="in")
summary(hct.m6)
# Predictors whose 95% CIs do NOT overlap zero: elev.position.z
# Reduced hct.m6 will be: elev.position.z
...
hct.waics <- cbind(hct.m1.waic, hct.m2.waic, hct.m3.waic, hct.m4.waic, hct.m5.waic, hct.m6.waic); hct.waics
write.csv(hct.waics, "hct_brms_models_waics.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons 

# Calculate Bayesian R^2:
bayes_R2(hct.m1)
bayes_R2(hct.m2) # 0.11
bayes_R2(hct.m3) # 0.08
bayes_R2(hct.m4) # 0.15
bayes_R2(hct.m5) # 0.167
bayes_R2(hct.m6) # 0.019
```


# hct: Calculate phylogenetic signal for big model sets for full models (m1-m6)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m2 phylo signal   
hyp.hct.m2 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.hct.m2 <- hypothesis(hct.m2, hyp.hct.m2, class = NULL))
plot(hyp.hct.m2)
# Phylo signal = 0 

# m4 phylo signal   
hyp.hct.m4 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE 
(hyp.hct.m4 <- hypothesis(hct.m4, hyp.hct.m4, class = NULL))
plot(hyp.hct.m4)
# Phylo signal = 0.01 = ~1%

# m6 phylo signal   
hyp.hct.m6 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.hct.m6 <- hypothesis(hct.m6, hyp.hct.m6, class = NULL))
plot(hyp.hct.m6)
# Phylo signal = 0 
...
# m5 species variance 
hyp.sp.hct.m5 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hct.m5 <- hypothesis(hct.m5, hyp.sp.hct.m5, class = NULL))
plot(hyp.sp.hct.m5)
...
plot(hyp.sp.hct.m6)
# ~10% variance explained by species 
```


# hct: RUN REDUCED MODELS (m7-m10)
```{r}
# MODEL 7: REDUCED MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
hct.m7 <- brm(
  formula = bf(hct ~ 1 + elev.position.z),
  data = final.hct,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(hct.m7, file="hct.m7_ReducedModel_PredictorsOnly.RData") # save model
load("hct.m7_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running

# NA - model m4 can't be reduced further 
# MODEL 8: REDUCED MODEL WITH ONLY PHYLO RANDOM EFFECT
# hct.m8 <- brm(
#   formula = bf(hct ~ 1 + elev.z + (1|gr(phylo, cov=A))),
#   data = final.hct,
#   family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
#   data2 = list(A = A), # Phylogeny
#   cores = 4,
...
#   sample_prior = TRUE # default priors 
# )
# save(hct.m8, file="hct.m8_ReducedModel_PhyloREOnly.RData") # save model
# #load("hct.m8_ReducedModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running


# MODEL 9: REDUCED MODEL WITH ONLY SPECIES RANDOM EFFECT
hct.m9 <- brm(
  formula = bf(hct ~ 1 + elev.position.z + (1|species)),
  data = final.hct,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(hct.m9, file="hct.m9_ReducedModel_SpeciesREOnly.RData") # save model
load("hct.m9_ReducedModel_SpeciesREOnly.RData") 

#MODEL 10: REDUCED MODEL WITH PHYLO AND SPECIES RANDOM EFFECTS
hct.m10 <- brm(
  formula = bf(hct ~ 1 + elev.position.z + (1|gr(phylo, cov = A)) + (1|species)),
  data = final.hct,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(hct.m10, file="hct.m10_ReducedModel_Phylo&SpeciesRE.RData") # save model
load("hct.m10_ReducedModel_Phylo&SpeciesRE.RData") 
```


# hct: Summarize and check fit for reduced models (m7-m10)
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(hct.m10, type = "trace") 
plot(hct.m10, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(hct.m10, resp = "hct", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(hct.m10, type = "stat", stat = 'median', nsamples = 100)
...
# Pale bars = you want all heading towards zero to show they're not correlating w/ each other across the chains 
# fine to be pos and neg, jumping around to get to zero
mcmc_plot(hct.m8, type = "acf_bar")

# plot conditional effects for full model (note: you do this separately below)
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(hct.m10), points = TRUE) 

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(hct.m8)  # adjust prob to get 95% CI
...
# Print group level variable (in log odds)
# ranef is still an array, so not an exponent on it, hard to change to probability estimates 
ranef(hct.m8, groups="species") #, probs = 0.95)
```
...
# Model 7 
color_scheme_set("blue") # nice default
hct.m7.p1 <- mcmc_plot(hct.m7, pars=c("b_elev.position.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation Position"))  
hct.m7.p1
ggsave(hct.m7.p1, filename = "hct.m7.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hct.m7)

# # Model 8
# # color_scheme_set("teal") # nice default
# hct.m8.p1 <- mcmc_plot(hct.m8, pars=c("b_elev.position.z"),
#                                     prob_outer=0.95, # 95% outer CI 
#                                     prob=0.50, # 50% inner CI 
#                                     point_est="mean") + # mean point est; default is median 
# labs(x="Effect on [hct]", title = "hct.m8: Posterior mean estimates & 95% Credible Intervals") + 
# scale_y_discrete(labels=c("Elevation position")) 
# hct.m8.p1
# ggsave(hct.m8.p1, filename = "hct.m8.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
# summary(hct.m8)

# Model 9
#color_scheme_set("red") # nice default
hct.m9.p1 <- mcmc_plot(hct.m9, pars=c("b_elev.position.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation Position"))   
hct.m9.p1
ggsave(hct.m9.p1, filename = "hct.m9.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hct.m9)

# Model 10
#color_scheme_set("red") # nice default
hct.m10.p1 <- mcmc_plot(hct.m10, pars=c("b_elev.position.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [hct]", title = "hct.m10: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Elevation Position"))   
hct.m10.p1
ggsave(hct.m10.p1, filename = "hct.m10.p1_hct_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(hct.m10)

```


# hct: Collate model summaries and WAIC scores for reduced models (m7-m10)
```{r}
# Print model summaries 
sink("hct_reduced_brms_model_summaries.txt",append = TRUE)
summary(hct.m7, waic=TRUE)
#summary(hct.m8, waic=TRUE)
summary(hct.m9, waic=TRUE)
summary(hct.m10, waic=TRUE)
sink()
# print(hct.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
hct.m7.waic <- waic(hct.m7)
#hct.m8.waic <- waic(hct.m8)
hct.m9.waic <- waic(hct.m9)
hct.m10.waic <- waic(hct.m10)
hct.waics.red <- cbind(hct.m7.waic, hct.m9.waic, hct.m10.waic); hct.waics.red
write.csv(hct.waics.red, "hct_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Calculate Bayesian R^2:
bayes_R2(hct.m7) # 0.05
# bayes_R2(hct.m8) # 0.13
bayes_R2(hct.m9) # 0.14
bayes_R2(hct.m10) # 0.14

```


# hct: Calculate phylogenetic signal for big model sets for reduced models (m7-m10)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m8 phylo signal   
# hyp.hct.m8 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE 
# (hyp.hct.m8 <- hypothesis(hct.m8, hyp.hct.m8, class = NULL))
# plot(hyp.hct.m8)
# Phylo signal = 0.01 = ~1%

# m10 phylo signal   
hyp.hct.m10 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.hct.m10 <- hypothesis(hct.m10, hyp.hct.m10, class = NULL))
plot(hyp.hct.m10)
# Phylo signal = 0 
...
# m9 species variance 
hyp.sp.hct.m9 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hct.m9 <- hypothesis(hct.m9, hyp.sp.hct.m9, class = NULL))
plot(hyp.sp.hct.m9)
# ~10% variance explained by species 

# m10 
hyp.sp.hct.m10 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.hct.m10 <- hypothesis(hct.m10, hyp.sp.hct.m10, class = NULL))
plot(hyp.sp.hct.m10)
# ~9% variance explained by species 
```
...
LOO(hct.m1, hct.m2, hct.m3, hct.m4, hct.m5, hct.m6, hct.m7, hct.m9, hct.m10, reloo=FALSE) 
sink()
...
Quick summary of top 3 models in order of ranking: 
1) Model 9: Elevation position + (1|species). Phylo signal NA. 
2) Model 10: Elevation position + (1|phylogeny) + (1|species). Phylo signal is 0. 
...
```{r}
# Copy species column and name it 'phylo' for estimation of phylo variance 
final.trbc$phylo <- final.trbc$species
...
# See what prior choices are being made for us. 
get_prior(trbc ~ 1 + elev.z + elev.position.z + lat.z + mass.z + temp.z + precip.z + intravar.elev + intravar.elev.pos + intravar.mass + intravar.lat + intravar.precip + intravar.temp + (1|gr(phylo, cov=A)) + (1|species), data = final.trbc)
# I see why the tutorial calls one "phlyo" and one species; it's because you can't have both 
...
# MODEL 2: RANDOM EFFECTS-ONLY MODEL (INTERCEPT + PHYLO & SPECIES RANDOM EFFECTS)
trbc.m2 <- brm(
  formula = bf(trbc ~ 1 + (1|gr(phylo, cov=A)) + (1|species)),
  data = final.trbc,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
# MODEL 3: FULL MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
trbc.m3 <- brm(
  formula = bf(trbc ~ 1 + elev.z + elev.position.z + lat.z + mass.z + temp.z + precip.z + genotype +
               intravar.elev + intravar.elev.pos + intravar.lat + intravar.mass + intravar.temp + intravar.precip),
  data = final.trbc,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
...
save(trbc.m4, file="trbc.m4_FullModel_PhyloREOnly.RData") # save model 
#  ~3 mins run time 
load("trbc.m4_FullModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
...
save(trbc.m5, file="trbc.m5_FullModel_SpeciesREOnly.RData") # save model
# ~3 mins
load("trbc.m5_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
...
save(trbc.m6, file="trbc.m6_FullModel_Phylo&SpeciesRE.RData") # save model
# ~ 3 mins run time 
load("trbc.m6_FullModel_Phylo&SpeciesRE.RData") # If loading from pre-saved file and not re-running
...
# Model 4
# color_scheme_set("teal") # nice default
trbc.m4.p1 <- mcmc_plot(trbc.m4, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
summary(trbc.m4)
# Predictors whose 95% CIs do NOT overlap zero: intravar.elev, intravar.elev.pos
# Reduced trbc.m4 will be: intravar.elev, intravar.elev.pos
...
# Model 6
#color_scheme_set("gray") # nice default
trbc.m6.p1 <- mcmc_plot(trbc.m6, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
summary(trbc.m6)
# Predictors whose 95% CIs do NOT overlap zero: intravar.elev
# Reduced trbc.m6 will be: intravar.elev

####

# Summary of reducing models: 
# m7 (reduced m3) will be: lat.z, intravar.elev, intravar.elev.pos  
# m8 (reduced m4) will be: intravar.elev, intravar.elev.pos
# m9 (reduced m5) will be: intravar.elev
# m10 (reduced m6) will be: intravar.elev
```
...
trbc.waics <- cbind(trbc.m1.waic, trbc.m2.waic, trbc.m3.waic, trbc.m4.waic, trbc.m5.waic, trbc.m6.waic); trbc.waics
...
bayes_R2(trbc.m4) # 0.17
bayes_R2(trbc.m5) # 0.186
bayes_R2(trbc.m6) # 0.186
```
...
# m2 phylo signal   
hyp.trbc.m2 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.trbc.m2 <- hypothesis(trbc.m2, hyp.trbc.m2, class = NULL))
plot(hyp.trbc.m2)
# Phylo signal = 0.01 = ~1%

# m4 phylo signal   
hyp.trbc.m4 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE 
(hyp.trbc.m4 <- hypothesis(trbc.m4, hyp.trbc.m4, class = NULL))
plot(hyp.trbc.m4)
# Phylo signal = 0.02 = ~2%

# m6 phylo signal   
hyp.trbc.m6 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.trbc.m6 <- hypothesis(trbc.m6, hyp.trbc.m6, class = NULL))
plot(hyp.trbc.m6)
# Phylo signal = 0.01 = ~1% 
...
# trbc: RUN REDUCED MODELS (m7-m10)
```{r}
# MODEL 7: REDUCED MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
trbc.m7 <- brm(
...
save(trbc.m7, file="trbc.m7_ReducedModel_PredictorsOnly.RData") # save model
load("trbc.m7_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running

# NA - model m4 can't be reduced further 
# MODEL 8: REDUCED MODEL WITH ONLY PHYLO RANDOM EFFECT
trbc.m8 <- brm(
  formula = bf(trbc ~ 1 + intravar.elev + intravar.elev.pos + (1|gr(phylo, cov=A))),
  data = final.trbc,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(trbc.m8, file="trbc.m8_ReducedModel_PhyloREOnly.RData") # save model
load("trbc.m8_ReducedModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
...
save(trbc.m9, file="trbc.m9_ReducedModel_SpeciesREOnly.RData") # save model
load("trbc.m9_ReducedModel_SpeciesREOnly.RData") 

#MODEL 10: REDUCED MODEL WITH PHYLO AND SPECIES RANDOM EFFECTS
trbc.m10 <- brm(
  formula = bf(trbc ~ 1 + intravar.elev + (1|gr(phylo, cov = A)) + (1|species)),
  data = final.trbc,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(trbc.m10, file="trbc.m10_ReducedModel_Phylo&SpeciesRE.RData") # save model
load("trbc.m10_ReducedModel_Phylo&SpeciesRE.RData") 
```


# trbc: Summarize and check fit for reduced models (m7-m10)
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(trbc.m10, type = "trace") 
plot(trbc.m10, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(trbc.m10, resp = "trbc", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(trbc.m10, type = "stat", stat = 'median', nsamples = 100)
...
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(trbc.m7), points = TRUE) 
...
# Model 7 
color_scheme_set("blue") # nice default
trbc.m7.p1 <- mcmc_plot(trbc.m7, pars=c("b_lat.z", "b_intravar.elev", "b_intravar.elev.pos"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [trbc]", title = "trbc.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude", "Intra Var: Elev", "Intra Var: Elev Pos"))  
trbc.m7.p1
ggsave(trbc.m7.p1, filename = "trbc.m7.p1_trbc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(trbc.m7)

# # Model 8
# color_scheme_set("teal") # nice default
trbc.m8.p1 <- mcmc_plot(trbc.m8, pars=c("b_intravar.elev", "b_intravar.elev.pos"),
                                    prob_outer=0.95, # 95% outer CI
                                    prob=0.50, # 50% inner CI
                                    point_est="mean") + # mean point est; default is median
labs(x="Effect on [trbc]", title = "trbc.m8: Posterior mean estimates & 95% Credible Intervals") +
scale_y_discrete(labels=c("Intra Var: Elev", "Intra Var: Elev Pos"))
trbc.m8.p1
ggsave(trbc.m8.p1, filename = "trbc.m8.p1_trbc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(trbc.m8)

# Model 9
#color_scheme_set("red") # nice default
trbc.m9.p1 <- mcmc_plot(trbc.m9, pars=c("b_intravar.elev"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [trbc]", title = "trbc.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Intra Var: Elev"))   
trbc.m9.p1
ggsave(trbc.m9.p1, filename = "trbc.m9.p1_trbc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(trbc.m9)

# Model 10
#color_scheme_set("red") # nice default
trbc.m10.p1 <- mcmc_plot(trbc.m10, pars=c("b_intravar.elev"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on trbc", title = "trbc.m10: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Intra Var: Elev"))   
trbc.m10.p1
ggsave(trbc.m10.p1, filename = "trbc.m10.p1_trbc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(trbc.m10)

```


# trbc: Collate model summaries and WAIC scores for reduced models (m7-m10)
```{r}
# Print model summaries 
sink("trbc_reduced_brms_model_summaries.txt",append = TRUE)
summary(trbc.m7, waic=TRUE)
summary(trbc.m8, waic=TRUE)
summary(trbc.m9, waic=TRUE)
summary(trbc.m10, waic=TRUE)
sink()
# print(trbc.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
trbc.m7.waic <- waic(trbc.m7)
trbc.m8.waic <- waic(trbc.m8)
trbc.m9.waic <- waic(trbc.m9)
trbc.m10.waic <- waic(trbc.m10)
trbc.waics.red <- cbind(trbc.m7.waic, trbc.m8.waic, trbc.m9.waic, trbc.m10.waic); trbc.waics.red
...
# Calculate Bayesian R^2:
bayes_R2(trbc.m7) # 0.06
bayes_R2(trbc.m8) # 0.13
bayes_R2(trbc.m9) # 0.14
bayes_R2(trbc.m10) # 0.14
```


# trbc: Calculate phylogenetic signal for big model sets for reduced models (m7-m10)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m8 phylo signal   
hyp.trbc.m8 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE
(hyp.trbc.m8 <- hypothesis(trbc.m8, hyp.trbc.m8, class = NULL))
plot(hyp.trbc.m8)
# Phylo signal = 0.02 = ~2%

# m10 phylo signal   
hyp.trbc.m10 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.trbc.m10 <- hypothesis(trbc.m10, hyp.trbc.m10, class = NULL))
plot(hyp.trbc.m10)
# Phylo signal = 0.0 
...
# m9 species variance 
hyp.sp.trbc.m9 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.trbc.m9 <- hypothesis(trbc.m9, hyp.sp.trbc.m9, class = NULL))
plot(hyp.sp.trbc.m9)
# ~21% variance explained by species 

# m10 
hyp.sp.trbc.m10 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.trbc.m10 <- hypothesis(trbc.m10, hyp.sp.trbc.m10, class = NULL))
plot(hyp.sp.trbc.m10)
...
LOO(trbc.m1, trbc.m2, trbc.m3, trbc.m4, trbc.m5, trbc.m6, trbc.m7, trbc.m8, trbc.m9, trbc.m10, reloo=FALSE) 
sink()
# According to LOO, model 8 is top model 
...
```{r}
# Copy species column and name it 'phylo' for estimation of phylo variance 
#final.mcv$phylo <- final.mcv$species
...
# MODEL 3: FULL MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
mcv.m3 <- brm(
  formula = bf(mcv ~ 1 + elev.z + elev.position.z + mass.z + temp.z + precip.z + genotype +
               intravar.elev + intravar.elev.pos + intravar.mass + intravar.temp + intravar.precip),
  data = final.mcv,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
 # data2 = list(A = A), # Phylogeny
...
# MODEL 4: FULL MODEL WITH ONLY PHYLO RANDOM EFFECT
# mcv.m4 <- brm(
#   formula = bf(mcv ~ 1 + elev.z + elev.position.z + lat.z + mass.z + temp.z + precip.z + genotype +
#                intravar.elev + intravar.elev.pos + intravar.lat + intravar.mass + intravar.temp + intravar.precip 
#                + (1|gr(phylo, cov=A))),
#   data = final.mcv,
#   family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
...
#   # )
# )
# save(mcv.m4, file="mcv.m4_FullModel_PhyloREOnly.RData") # save model 
# #  ~3 mins run time 
# load("mcv.m4_FullModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
...
#   # )
# )
# save(mcv.m6, file="mcv.m6_FullModel_Phylo&SpeciesRE.RData") # save model
# # ~ 3 mins run time 
# load("mcv.m6_FullModel_Phylo&SpeciesRE.RData") # If loading from pre-saved file and not re-running
...
# Model 3 
color_scheme_set("blue") # nice default
mcv.m3.p1 <- mcmc_plot(mcv.m3, pars=c("b_elev.z", "b_elev.position.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos","b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
# Model 4
# color_scheme_set("teal") # nice default
# mcv.m4.p1 <- mcmc_plot(mcv.m4, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
#                                     "b_genotypeGlyMSer", "b_genotypeSerMSer",
#                                     "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
#                                     "b_intravar.temp", "b_intravar.precip"),
#                                     prob_outer=0.95, # 95% outer CI 
...
# scale_y_discrete(labels=c("Elevation","Elevation Position", "Latitude", "Mass", "Temp", "Precip", "Genotype (Gly-Ser)", "Genotype (Ser-Ser)", "Intra Var: Elev", "Intra Var: Elev pos", "Intra Var: Lat", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip")) 
# mcv.m4.p1
# ggsave(mcv.m4.p1, filename = "mcv.m4.p1_mcv_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
# summary(mcv.m4)
# Predictors whose 95% CIs do NOT overlap zero: latitude 
# Reduced mcv.m4 will be: lat.z

# Model 5
#color_scheme_set("red") # nice default
mcv.m5.p1 <- mcmc_plot(mcv.m5, pars=c("b_elev.z", "b_elev.position.z","b_mass.z","b_temp.z","b_precip.z",
                                    "b_genotypeGlyMSer", "b_genotypeSerMSer",
                                    "b_intravar.elev", "b_intravar.elev.pos","b_intravar.mass",
                                    "b_intravar.temp", "b_intravar.precip"),
                                    prob_outer=0.95, # 95% outer CI 
...
# Model 6
#color_scheme_set("gray") # nice default
# mcv.m6.p1 <- mcmc_plot(mcv.m6, pars=c("b_elev.z", "b_elev.position.z","b_lat.z","b_mass.z","b_temp.z","b_precip.z",
#                                     "b_genotypeGlyMSer", "b_genotypeSerMSer",
#                                     "b_intravar.elev", "b_intravar.elev.pos", "b_intravar.lat", "b_intravar.mass",
#                                     "b_intravar.temp", "b_intravar.precip"),
#                                     prob_outer=0.95, # 95% outer CI 
...
# summary(mcv.m6)
# Predictors whose 95% CIs do NOT overlap zero: latitude
# Reduced mcv.m6 will be: lat.z

####

# Summary of reducing models: 
# m7 (reduced m3) will be: intravar.elev, intravar.elev.pos, lat.z, elev.pos 
# m8 (reduced m4) will be: lat.z
# m9 (reduced m5) will be: lat.z
# m10 (reduced m6) will be: lat.z
```
...
mcv.waics <- cbind(mcv.m1.waic, mcv.m2.waic, mcv.m3.waic, mcv.m4.waic, mcv.m5.waic, mcv.m6.waic); mcv.waics
...
bayes_R2(mcv.m4) # 0.139
bayes_R2(mcv.m5) # 0.156
bayes_R2(mcv.m6) # 0.157
```
...
# m2 phylo signal   
hyp.mcv.m2 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.mcv.m2 <- hypothesis(mcv.m2, hyp.mcv.m2, class = NULL))
plot(hyp.mcv.m2)
# Phylo signal = 0.0 = ~0%

# m4 phylo signal   
hyp.mcv.m4 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sigma^2) = 0") # only phylo RE 
(hyp.mcv.m4 <- hypothesis(mcv.m4, hyp.mcv.m4, class = NULL))
plot(hyp.mcv.m4)
# Phylo signal = 0.01 = ~1%
...
# Summary of reducing models: 
# m7 (reduced m3) will be: lat.z, elev.pos, intravar.elev, intravar.elev.pos  
# m8 (reduced m4) will be: intravar.elev, intravar.elev.pos
# m9 (reduced m5) will be: intravar.elev
# m10 (reduced m6) will be: intravar.elev


# mcv: RUN REDUCED MODELS (m7-m10)
```{r}
# MODEL 7: REDUCED MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
mcv.m7 <- brm(
...
save(mcv.m7, file="mcv.m7_ReducedModel_PredictorsOnly.RData") # save model
load("mcv.m7_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running

# MODEL 8: REDUCED MODEL WITH ONLY PHYLO RANDOM EFFECT
mcv.m8 <- brm(
  formula = bf(mcv ~ 1 + lat.z + (1|gr(phylo, cov=A))),
  data = final.mcv,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(mcv.m8, file="mcv.m8_ReducedModel_PhyloREOnly.RData") # save model
load("mcv.m8_ReducedModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
...
save(mcv.m9, file="mcv.m9_ReducedModel_SpeciesREOnly.RData") # save model
load("mcv.m9_ReducedModel_SpeciesREOnly.RData") 

#MODEL 10: REDUCED MODEL WITH PHYLO AND SPECIES RANDOM EFFECTS
mcv.m10 <- brm(
  formula = bf(mcv ~ 1 + lat.z + (1|gr(phylo, cov = A)) + (1|species)),
  data = final.mcv,
  family = gaussian(), # estimates of  mean and sd in t dist are robust to outliers relative to normal dist
  data2 = list(A = A), # Phylogeny
  cores = 4,
...
save(mcv.m10, file="mcv.m10_ReducedModel_Phylo&SpeciesRE.RData") # save model
load("mcv.m10_ReducedModel_Phylo&SpeciesRE.RData") 
```


# mcv: Summarize and check fit for reduced models (m7-m10)
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(mcv.m10, type = "trace") 
plot(mcv.m10, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(mcv.m10, resp = "mcv", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(mcv.m10, type = "stat", stat = 'median', nsamples = 100)
...
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(mcv.m7), points = TRUE) 
...
# Model 7 
color_scheme_set("blue") # nice default
mcv.m7.p1 <- mcmc_plot(mcv.m7, pars=c("b_lat.z", "b_elev.position.z", "b_intravar.elev", "b_intravar.elev.pos"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mcv]", title = "mcv.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude", "Elev Position", "Intra Var: Elev", "Intra Var: Elev Pos"))  
mcv.m7.p1
ggsave(mcv.m7.p1, filename = "mcv.m7.p1_mcv_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mcv.m7)
...
# Model 9
#color_scheme_set("red") # nice default
mcv.m9.p1 <- mcmc_plot(mcv.m9, pars=c("b_lat.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mcv]", title = "mcv.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude"))   
mcv.m9.p1
ggsave(mcv.m9.p1, filename = "mcv.m9.p1_mcv_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mcv.m9)

# Model 10
#color_scheme_set("red") # nice default
mcv.m10.p1 <- mcmc_plot(mcv.m10, pars=c("b_lat.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on mcv", title = "mcv.m10: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude"))   
mcv.m10.p1
ggsave(mcv.m10.p1, filename = "mcv.m10.p1_mcv_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mcv.m10)

```


# mcv: Collate model summaries and WAIC scores for reduced models (m7-m10)
```{r}
# Print model summaries 
sink("mcv_reduced_brms_model_summaries.txt",append = TRUE)
summary(mcv.m7, waic=TRUE)
summary(mcv.m8, waic=TRUE)
summary(mcv.m9, waic=TRUE)
summary(mcv.m10, waic=TRUE)
sink()
# print(mcv.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
mcv.m7.waic <- waic(mcv.m7)
mcv.m8.waic <- waic(mcv.m8)
mcv.m9.waic <- waic(mcv.m9)
mcv.m10.waic <- waic(mcv.m10)
mcv.waics.red <- cbind(mcv.m7.waic, mcv.m8.waic, mcv.m9.waic, mcv.m10.waic); mcv.waics.red
write.csv(mcv.waics.red, "mcv_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons

# Calculate Bayesian R^2:
bayes_R2(mcv.m7) # 0.05
bayes_R2(mcv.m8) # 0.10
bayes_R2(mcv.m9) # 0.12
bayes_R2(mcv.m10) # 0.12
```


# mcv: Calculate phylogenetic signal for big model sets for reduced models (m7-m10)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m10 phylo signal   
hyp.mcv.m10 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.mcv.m10 <- hypothesis(mcv.m10, hyp.mcv.m10, class = NULL))
plot(hyp.mcv.m10)
# Phylo signal = 0.0 
...
# m9 species variance 
hyp.sp.mcv.m9 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.mcv.m9 <- hypothesis(mcv.m9, hyp.sp.mcv.m9, class = NULL))
plot(hyp.sp.mcv.m9)
# ~16% variance explained by species 

# m10 
hyp.sp.mcv.m10 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.mcv.m10 <- hypothesis(mcv.m10, hyp.sp.mcv.m10, class = NULL))
plot(hyp.sp.mcv.m10)
# ~13% variance explained by species
...
LOO(mcv.m1, mcv.m2, mcv.m3, mcv.m4, mcv.m5, mcv.m6, mcv.m7, mcv.m8, mcv.m9, mcv.m10, reloo=FALSE) 
sink()
# According to LOO, model 8 is top model 
...
# Summary of reducing models: 
# m7 (reduced m3) will be: lat.z, intravar.elev, elev.pos
# m8 (reduced m4) will be: lat.z
# m9 (reduced m5) will be: lat.z
# m10 (reduced m6) will be: lat.z
```
...
save(mch.m7, file="mch.m7_ReducedModel_PredictorsOnly.RData") # save model
load("mch.m7_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mch.m9, file="mch.m9_ReducedModel_SpeciesREOnly.RData") # save modell
load("mch.m9_ReducedModel_SpeciesREOnly.RData") 
...
save(mch.m10, file="mch.m10_ReducedModel_Phylo&SpeciesRE.RData") # save model
load("mch.m10_ReducedModel_Phylo&SpeciesRE.RData") 
```


# MCH: Summarize and check fit for reduced models (m7-m10)
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(mch.m10, type = "trace") 
plot(mch.m10, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(mch.m10, resp = "mch", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(mch.m10, type = "stat", stat = 'median', nsamples = 100)
...
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(mch.m7), points = TRUE) 
...
# Model 7 
color_scheme_set("blue") # nice default
mch.m7.p1 <- mcmc_plot(mch.m7, pars=c("b_lat.z", "b_elev.position.z", "b_intravar.elev"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mch]", title = "mch.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude", "Elev Position", "Intra Var: Elev"))  
mch.m7.p1
ggsave(mch.m7.p1, filename = "mch.m7.p1_mch_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mch.m7)
...
# Model 9
#color_scheme_set("red") # nice default
mch.m9.p1 <- mcmc_plot(mch.m9, pars=c("b_lat.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mch]", title = "mch.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude"))   
mch.m9.p1
ggsave(mch.m9.p1, filename = "mch.m9.p1_mch_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mch.m9)

# Model 10
#color_scheme_set("red") # nice default
mch.m10.p1 <- mcmc_plot(mch.m10, pars=c("b_lat.z"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on mch", title = "mch.m10: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Latitude"))   
mch.m10.p1
ggsave(mch.m10.p1, filename = "mch.m10.p1_mch_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mch.m10)

```


# mch: Collate model summaries and WAIC scores for reduced models (m7-m10)
```{r}
# Print model summaries 
sink("mch_reduced_brms_model_summaries.txt",append = TRUE)
summary(mch.m7, waic=TRUE)
summary(mch.m8, waic=TRUE)
summary(mch.m9, waic=TRUE)
summary(mch.m10, waic=TRUE)
sink()
# print(mch.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
mch.m7.waic <- waic(mch.m7)
mch.m8.waic <- waic(mch.m8)
mch.m9.waic <- waic(mch.m9)
mch.m10.waic <- waic(mch.m10)
mch.waics.red <- cbind(mch.m7.waic, mch.m8.waic, mch.m9.waic, mch.m10.waic); mch.waics.red
write.csv(mch.waics.red, "mch_brms_models_waics.red.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons 

# Calculate Bayesian R^2:
bayes_R2(mch.m7) # 0.055
bayes_R2(mch.m8) # .13
bayes_R2(mch.m9) # 0.14
bayes_R2(mch.m10) # 0.14
```


# mch: Calculate phylogenetic signal for big model sets for reduced models (m7-m10)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m10 phylo signal   
hyp.mch.m10 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.mch.m10 <- hypothesis(mch.m10, hyp.mch.m10, class = NULL))
plot(hyp.mch.m10)
# Phylo signal = 0.0 
...
# m9 species variance 
hyp.sp.mch.m9 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.mch.m9 <- hypothesis(mch.m9, hyp.sp.mch.m9, class = NULL))
plot(hyp.sp.mch.m9)
# ~18% variance explained by species 

# m10 
hyp.sp.mch.m10 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.mch.m10 <- hypothesis(mch.m10, hyp.sp.mch.m10, class = NULL))
plot(hyp.sp.mch.m10)
# ~16% variance explained by species
```
...
LOO(mch.m1, mch.m2, mch.m3, mch.m4, mch.m5, mch.m6, mch.m7, mch.m8, mch.m9, mch.m10, reloo=FALSE) 
sink()
# According to LOO, model 8 is top model 
...
# Summary of reducing models: 
# m7 (reduced m3) will be: intravar.temp, temp, mass
# m8 (reduced m4) will be: intravar.mass, intravar.temp
# m9 (reduced m5) will be: intravar.mass
# m10 (reduced m6) will be: intravar.mass
```
...
# mchc: RUN REDUCED MODELS (m7-m10)
```{r}
# MODEL 7: REDUCED MODEL WITH NO RANDOM EFFECTS (JUST PREDICTORS)
mchc.m7 <- brm(
...
save(mchc.m7, file="mchc.m7_ReducedModel_PredictorsOnly.RData") # save model
load("mchc.m7_ReducedModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(mchc.m9, file="mchc.m9_ReducedModel_SpeciesREOnly.RData") # save model
load("mchc.m9_ReducedModel_SpeciesREOnly.RData") 
...
save(mchc.m10, file="mchc.m10_ReducedModel_Phylo&SpeciesRE.RData") # save model
load("mchc.m10_ReducedModel_Phylo&SpeciesRE.RData") 
```


# MCHC: Summarize and check fit for reduced models (m7-m10)
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
...
# Beginning will look crappy because of burn-in, but we want output to look like a "fuzzy caterpillar", aka noise is good
# Any plateaus/flat horizontal lines are bad, suggest we're sampling the same spot in the distribution repeatedly 
mcmc_plot(mchc.m10, type = "trace") 
plot(mchc.m10, N = 2, ask = FALSE)

# y to y-rep fit plot 
# Generate new y values given the model and compare to actual y from the data. 
# You want the y rep line to be somewhat close to dark line, signifying that model fits the data
pp_check(mchc.m10, resp = "mchc", nsamples = 100) # pull 100 draws from model and cast back to original data 
pp_check(mchc.m10, type = "stat", stat = 'median', nsamples = 100)
...
# Note: this used to be called marginal_effects, but that's now deprecated; replaced w/ conditional_effects
# nu = degrees of freedom
plot(conditional_effects(mchc.m7), points = TRUE) 
...
# Model 7 
color_scheme_set("blue") # nice default
mchc.m7.p1 <- mcmc_plot(mchc.m7, pars=c("b_mass.z", "b_temp.z", "b_intravar.temp"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mchc]", title = "mchc.m7: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Mass", "Temp", "Intra Var: Temp"))  
mchc.m7.p1
ggsave(mchc.m7.p1, filename = "mchc.m7.p1_mchc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mchc.m7)
...
# Model 9
#color_scheme_set("red") # nice default
mchc.m9.p1 <- mcmc_plot(mchc.m9, pars=c("b_intravar.mass"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [mchc]", title = "mchc.m9: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Intra Var: Mass"))   
mchc.m9.p1
ggsave(mchc.m9.p1, filename = "mchc.m9.p1_mchc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mchc.m9)

# Model 10
#color_scheme_set("red") # nice default
mchc.m10.p1 <- mcmc_plot(mchc.m10, pars=c("b_intravar.mass"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on mchc", title = "mchc.m10: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("Intra Var: Mass"))   
mchc.m10.p1
ggsave(mchc.m10.p1, filename = "mchc.m10.p1_mchc_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(mchc.m10)

```


# mchc: Collate model summaries and WAIC scores for reduced models (m7-m10)
```{r}
# Print model summaries 
sink("mchc_reduced_brms_model_summaries.txt",append = TRUE)
summary(mchc.m7, waic=TRUE)
summary(mchc.m8, waic=TRUE)
summary(mchc.m9, waic=TRUE)
summary(mchc.m10, waic=TRUE)
sink()
# print(mchc.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
mchc.m7.waic <- waic(mchc.m7)
mchc.m8.waic <- waic(mchc.m8)
mchc.m9.waic <- waic(mchc.m9)
mchc.m10.waic <- waic(mchc.m10)
mchc.waics.red <- cbind(mchc.m7.waic, mchc.m8.waic, mchc.m9.waic, mchc.m10.waic); mchc.waics.red
...
# Calculate Bayesian R^2:
bayes_R2(mchc.m7) # 0.03
bayes_R2(mchc.m8) # 0.11
bayes_R2(mchc.m9) # 0.11
bayes_R2(mchc.m10) # 0.11
```


# mchc: Calculate phylogenetic signal for big model sets for reduced models (m7-m10)
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# Big models that have phylogenetic grouping variable: m2, m4, m6
...
# m10 phylo signal   
hyp.mchc.m10 <- paste("sd_phylo__Intercept^2 /", "(sd_phylo__Intercept^2 + sd_species__Intercept^2 + sigma^2) = 0")
(hyp.mchc.m10 <- hypothesis(mchc.m10, hyp.mchc.m10, class = NULL))
plot(hyp.mchc.m10)
# Phylo signal = 0.0 
...
# m9 species variance 
hyp.sp.mchc.m9 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.mchc.m9 <- hypothesis(mchc.m9, hyp.sp.mchc.m9, class = NULL))
plot(hyp.sp.mchc.m9)
# ~13% variance explained by species 

# m10 
hyp.sp.mchc.m10 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.mchc.m10 <- hypothesis(mchc.m10, hyp.sp.mchc.m10, class = NULL))
plot(hyp.sp.mchc.m10)
# ~12% variance explained by species
```
...
LOO(mchc.m1, mchc.m2, mchc.m3, mchc.m4, mchc.m5, mchc.m6, mchc.m7, mchc.m8, mchc.m9, mchc.m10, reloo=FALSE) 
sink()
# According to LOO, model 8 is top model 
...
```{r}
get_variables(hb.m10) # get variables that appear in the model 
