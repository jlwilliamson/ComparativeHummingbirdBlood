...
final.mcv <- subset(final, select=c(species, rowID, nk, sex, age, month, year, locality, elev, department, lat, lon, mass, mcv, Clade, elev_position, tempPC1, precipPC1, mass.z, elev.z, elev.pos.z, temp.z, precip.z, b13, b83, genotype, intravar.mass.z, intravar.temp.z, intravar.precip.z, mcv.log, spec.mean.elev.z, spec.mean.temp.z, spec.mean.precip.z, spec.mean.mass.z)) 
...
mcv.waics <- cbind(mcv.m1.waic, mcv.m2.waic, mcv.m3.waic, mcv.m4.waic, mcv.m4.quad.waic); mcv.waics
...
mcv.m6.quad.waic <- waic(mcv.m6.quad)
mcv.waics.red <- cbind(mcv.m5.waic, mcv.m6.waic, mcv.m6.quad.waic); mcv.waics.red
...
LOO(mcv.m1, mcv.m2, mcv.m3, mcv.m4, mcv.m5, mcv.m6, mcv.m4.quad, mcv.m6.quad, reloo=TRUE) 
sink()
...
bayes_R2(mcv.m6.quad) # 0.099

# Model comparisons - original comparison without quadratic models:
#        elpd_diff se_diff
# mcv.m6   0.0       0.0  
# mcv.m2  -0.5       1.0  
# mcv.m4  -0.9       3.6  
...
# Model comparisons- WITH QUADRATIC MODELS:
#             elpd_diff se_diff
# mcv.m6.quad   0.0       0.0  ; 14% explained by species
# mcv.m6       -1.8       2.5  
# mcv.m2       -2.4       2.7  
# mcv.m4       -2.6       1.9  
# mcv.m4.quad  -3.4       2.2  
...
# ~3 mins
#load("mch.m4_FullModel_SpeciesREOnly.RData") # If loading from pre-saved file and not re-running
...
#load("mch.m6_ReducedModel_PhyloREOnly.RData") # If loading from pre-saved file and not re-running
```
...
# Note that if you use font_family "Arial" ggsave will have trouble writing this to a pdf because it makes the plot w/ Arial 
...
hct.m4.fig4b <- mcmc_plot(hct.m4, pars=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
            ...
            # scale_y_discrete(labels=c("Elevation","Elevation Position", "Mass", "Temp", "Precip", "Genotype (Gly-Ser)", 
            #                         "Genotype (Ser-Ser)", "Intra Var: Mass", "Intra Var: Temp", "Intra Var: Precip")) +  
            scale_y_discrete(labels=c("","", "", "", "", "", "", "", "", "")) + # Replace labels w/ blanks for multipanel fig
            theme(plot.title = element_text(family = "Helvetica", color="black",size=12)) + # font = panel header size
            theme(text = element_text(family="Helvetica",size=9)) + # y axis labels
...
mcv.m6.fig4d.inset <- mcmc_plot(mcv.m6, pars=c("b_temp.z"),
                   ...
                   theme(text = element_text(family="Helvetica",size=6)) + # y axis labels
                   # ggtitle("C") + # Assign panel number/header
                   theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
                   plot.title = element_text(face="bold"))  # This makes panel header bold 
                   # This is good for labeling figure panels! Avoids having to manually toy w/ hjust and vjust
...
ggsave(Fig4_PosteriorProbs_6panel, filename = "Fig4_PosteriorProbs_6panel_03-24-21.pdf", bg="transparent", height=7, width=11, units="in")
...
converting to unstandardized coefficients?
```{r}
...
# 
# coef(hb.m4)[1]*sd(final.hb$elev.z)+mean(final.hb$elev.z)-  
#   (coef(hb.m4)['x1']*sd(final.hb$elev.z)*mean(final.hb[['x1']])/sd(final.hb[['x1']]) +
#    coef(hb.m4)['x2']*sd(final.hb$elev.z)*mean(final.hb[['x2']])/sd(final.hb[['x2']]))
...
This post seems to have answers: https://github.com/ASKurz/Doing-Bayesian-Data-Analysis-in-brms-and-the-tidyverse/blob/master/17.Rmd

From this Stack Exchange page: https://stats.stackexchange.com/questions/235057/convert-standardized-coefficients-to-unstandardized-metric-coefficients-for-li

I've been searching: "convert standardized model coefficients to unstandardized brms r"

THIS ALSO MIGHT BE SIMPLER AND HAVE THE ANSWER: https://bookdown.org/ajkurz/recoding_Hayes_2018/multiple-linear-regression.html
...
# PCA of morphological variables 
Tutorial: https://tgmstat.wordpress.com/2013/11/28/computing-and-visualizing-pca-in-r/
Nice refresher on PCA interpretation: https://online.stat.psu.edu/stat505/lesson/11/11.4

We want to use PCA to visualize relationships among MCV, TRBC, Hb, and elev, so let's start there with those 4. We can always add more later. 
```{R}
# Load PCA plotting packages
library(ggfortify)
library(devtools)

# Install ggbiplot from GitHub; might need to restart R console for devtools update to recognize install_github()
#install_github("vqv/ggbiplot") # download ggbiplot from GitHub page 
library(ggbiplot)

# Alternatively, load functions you'll need from source 
...
# PCA of MCV, TRBC, Hb
# [rows, cols]
pca.sub <- final[ , c("species", "Clade", "elev", "mass", "hb", "mcv", "trbc")] # subset desired variables 
pca.sub <- na.omit(pca.sub) # remove all NAs

# We'll apply PCA to continuous variables and use categorical variables for visualization
pca.sub.log <- log(pca.sub[, c("hb", "mcv", "trbc")]) # Apply log transform as recommended
pca.species <- pca.sub[, "species"]
pca.clade <- pca.sub[, "Clade"]
 
# Apply PCA
cens.pca <- prcomp(pca.sub.log, center = TRUE, scale. = TRUE) # Centering and scaling variables is highly advisable 
print(cens.pca) # get loadings; also achieved w/ pg.pca$rotation
summary(cens.pca) # Get importance of components (look at cumulative proportion); 
# First 2 PCs explain 98% of variance; first three explain 100%

# visualize how much variance is explained by top PCs
screeplot(cens.pca)
plot(cens.pca, type = "l")
...
# Plot output w/ elipses & biplot arrows. Colors = haplotype, shape = country 
g <- ggbiplot(cens.pca, obs.scale = 1, var.scale = 1, 
              groups = pca.clade, ellipse = TRUE, # groups = grouping variable for PCA output
              circle = FALSE, varname.size=2, labels.size=2) 
#g <- g + scale_color_manual(name="Haplotype", values=c("#F39040", "#853FCD", "gainsboro")) # orange, purple
#g <- g + scale_shape_manual(name="Country", values=c(17, 19))
#g <- g + geom_point(aes(colour=pg.haplo, shape=pg.country), size = 3)
...
g <- g + theme(panel.grid.major = element_blank(),
              ...
              panel.background = element_blank())
#g <- g + geom_text(aes(label = pca.sub$hb), hjust = -0.1, alpha = 0.2) # labels for MCV to verify x-axis
g <- g + theme(axis.text=element_text(size=11, face="bold"))
g <- g + ggtitle("CeNs PCA")
# g <- g + xlim(-3, 3) + ylim(-3, 3)
print(g)
ggsave(g, filename = "PCA_TRBCxMCVxHb_CladeCircles_04-22-21.pdf", bg="transparent", height=7, width=9, units="in")
# ggsave(g, filename = "PCA_TRBCxMCVxHbxElev_CladeCircles_04-22-21.pdf", bg="transparent", height=7, width=9, units="in")

# Extract individual loadings
cens.loadings <- cens.pca$x # this is all PCs
datawithload <- cbind(pca.sub, cens.loadings)
plot(PC2 ~ PC1, data=datawithload) #PC1 v. PC2 

(p <- ggplot(datawithload, aes(x=PC1, y=PC2)) + # uncenter Tamin values
  geom_point() +
...
ggsave(p, filename="CeNS_Scatterplot_PC1xPC2.pdf", height=7, width=9, units="in")


plot(PC1 ~ hb, data=datawithload)
(p1 <- ggplot(datawithload, aes(x=hb, y=PC1)) + # uncenter Tamin values
  geom_point() +
 # scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="darkturquoise", size=1.2) + 
  theme_classic() + 
  labs(x="[Hb] (g/dl)", y="PC1") +
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") +
  # ggtitle("(a)") + # Assign panel number/header; this will be (a) because first in series of 3
  # theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
...
ggsave(p1, filename="CeNS_Scatterplot_HbxPC1.pdf", height=7, width=9, units="in")

# Try to make CeNS index w/ individual data

# Normalize data on scale from 0 to 1
library(scales)
datawithload$PC1.s <- rescale(datawithload$PC1, to=c(0,1))

# PC1 vs. Hb but rescaled
(p100 <- ggplot(datawithload, aes(x=hb, y=PC1.s, color=PC1.s)) + 
  geom_point(size=1.5) +
  scale_color_viridis(option="viridis", discrete=FALSE) +
  #scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="black", size=1.2) + 
  theme_classic() + 
  labs(x="[Hb] (g/dl)", y="CeNS Index \n (0 = large MCV, 1 = high TRBC)") +
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") +
 # geom_text(aes(label = species), hjust = -0.1, alpha = 0.2) + # labels for MCV to verify x-axis
  # ggtitle("(a)") + # Assign panel number/header; this will be (a) because first in series of 3
  # theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
  #       plot.title = element_text(face="bold")) + # This makes panel header bold 
  #     # This is good for labeling figure panels! Avoids having to manually toy w/ hjust and vjust
...
#ggsave(p100, filename="CeNS_Scatterplot_SpeciesMeans_HbxCeNSIndex.pdf", height=7, width=9, units="in")
# Values closer to 1 indicate increased TRBC; values closer to 0 indicate bigger cells 



#############

# AND WHAT IF WE USE SPECIES MEAN DATA? 

# Species mean PCA dataset

# PCA of MCV, TRBC, Hb
# [rows, cols]
pca.sub.spm <- final.spm[ , c("species", "Clade", "elev", "mass", "hb", "mcv", "trbc")] # subset desired variables 
# Note that I'm using 'final.spm' made in species means file to keep patagona as 2 functional groups
pca.sub.spm <- subset(pca.sub.spm, with(pca.sub.spm, species %in% names(which(table(species)>=3))))
pca.sub.spm <- na.omit(pca.sub.spm) # remove all NAs or PCAs and means won't work
# Tho I suppose on second thought the dplyr pipes don't take means from 1 individual...

pca.sub.spm <- pca.sub.spm %>% group_by(species) %>% summarise(hb.mean = mean(hb), # Species means for 60 species 
                                                  elev.mean = mean(elev),   
                                                  mcv.mean = mean(mcv),
                                                  trbc.mean = mean(trbc)
                                                  )
...
# We'll apply PCA to continuous variables and use categorical variables for visualization
pca.sub.spm.log <- log(pca.sub.spm[, c("hb.mean", "mcv.mean", "trbc.mean")]) # Apply log transform as recommended
pca.species.spm <- pca.sub.spm[, "species"]
 
# Apply PCA
cens.pca.spm <- prcomp(pca.sub.spm.log, center = TRUE, scale. = TRUE) # Centering and scaling variables = highly advisable
print(cens.pca.spm) # get loadings; also achieved w/ pg.pca$rotation
summary(cens.pca.spm) # Get importance of components (look at cumulative proportion); 
# First 2 PCs explain 98% of variance; first three explain 100%

# visualize how much variance is explained by top PCs
screeplot(cens.pca.spm)
plot(cens.pca.spm, type = "l")
...
# Plot data w/ biplot arrow and elipses 
g1 <- ggbiplot(cens.pca.spm, obs.scale = 1, var.scale = 1, 
             # groups = pca.species.spm, ellipse = TRUE, # groups = grouping variable for PCA output
              circle = FALSE, varname.size=3, labels.size=3) 
g1 <- g1 + theme(legend.direction = "vertical", 
              legend.position = "right")
g1 <- g1 + theme(panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(),
              panel.border = element_rect(colour = "black", fill=NA, size=0.5),
              panel.background = element_blank())
g1 <- g1 + geom_text(aes(label = pca.sub.spm$species), hjust = -0.1, alpha = 0.2) # labels for MCV to verify x-axis
g1 <- g1 + theme(axis.text=element_text(size=11, face="bold"))
g1 <- g1 + ggtitle("CeNs PCA - Species Means")
g1 <- g1 + xlim(-4, 6) + ylim(-3, 2)
print(g1)
ggsave(g1, filename = "PCA_SpeciesMeans_TRBCxMCVxHb_04-22-21.pdf", bg="transparent", height=7, width=9, units="in")
# ggsave(g, filename = "PCA_TRBCxMCVxHbxElev_CladeCircles_04-22-21.pdf", bg="transparent", height=7, width=9, units="in")


# AS WE DID ABOVE, EXTRACT LOADINGS AND EXAMINE RELATIONSHIPS: 
cens.spm.loadings <- cens.pca.spm$x # this is all PCs
cens.pca.spm.all <- cbind(pca.sub.spm, cens.spm.loadings)

(p3 <- ggplot(cens.pca.spm.all, aes(x=PC1, y=PC2)) + # uncenter Tamin values
  geom_point() +
 # scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="darkturquoise", size=1.2) + 
  theme_classic() + 
...
ggsave(p3, filename="CeNS_Scatterplot_SpeciesMeans_PC1xPC2.pdf", height=7, width=9, units="in")

# PC1 v. hb
(p4 <- ggplot(cens.pca.spm.all, aes(x=hb.mean, y=PC1)) + 
  geom_point(size=3) +
 # scale_color_viridis() +
 # scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="darkturquoise", size=1.2) + 
...
ggsave(p4, filename="CeNS_Scatterplot_SpeciesMeans_HbxPC1.pdf", height=7, width=9, units="in")

# Normalize data on scale from 0 to 1
library(scales)
cens.pca.spm.all$PC1.s <- rescale(cens.pca.spm.all$PC1, to=c(0,1))
write.csv(cens.pca.spm.all, "cens.pca.spm.all.csv")

# PC1 vs. Hb but rescaled
(p5 <- ggplot(cens.pca.spm.all, aes(x=hb.mean, y=PC1.s, color=elev.mean)) + 
  geom_point(size=4.5) +
  scale_color_viridis(option="rocket", discrete=FALSE, name="Mean\nElevation(m)") + # name= argument is for LEGEND TITLE
  #scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="black", size=1.2) + 
  theme_classic() + 
  labs(x="[Hb] (g/dl)", y="Cell size vs. Number (CeNS) Index \n (0 = large MCV, 1 = high TRBC)") +
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") +
 # theme(legend.title = element_blank()) + # Remove legend title; change title in scale_color_viridis, weirdly
 # geom_text(aes(label = species), hjust = -0.1, alpha = 0.2) + # labels for MCV to verify x-axis
  # ggtitle("(a)") + # Assign panel number/header; this will be (a) because first in series of 3
  # theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
  #       plot.title = element_text(face="bold")) + # This makes panel header bold 
...
ggsave(p5, filename="CeNS_Scatterplot_SpeciesMeans_HbxCeNSIndex.pdf", height=7, width=9, units="in")
# Values closer to 1 indicate increased TRBC; values closer to 0 indicate bigger cells 

# PC1 vs. Hb but rescaled - FLIPPED AXES 
(p6 <- ggplot(cens.pca.spm.all, aes(x=PC1.s, y=hb.mean, color=elev.mean)) + 
  geom_point(size=5.5) +
  scale_color_viridis(option="rocket", discrete=FALSE, name="Mean\nElevation (m)") + # name= argument is for LEGEND TITLE
  #scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="black", size=1.2) + 
  theme_classic() + 
  labs(x="Cell size vs. Number (CeNS) Index \n (0 = larger MCV, 1 = higher TRBC)", y="[Hb] (g/dl)") +
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") +
 # theme(legend.title = element_blank()) + # Remove legend title; change title in scale_color_viridis, weirdly
 # geom_text(aes(label = species), hjust = -0.1, alpha = 0.2) + # labels for MCV to verify x-axis
...
ggsave(p6, filename="CeNS_Scatterplot_SpeciesMeans_HbxCeNSIndex.pdf", height=7, width=9, units="in")
# Values closer to 1 indicate increased TRBC; values closer to 0 indicate bigger cells 


# PC1 vs. Elev but rescaled - ELEV ON X AXIS, CENS ON Y
(p7 <- ggplot(cens.pca.spm.all, aes(x=PC1.s, y=elev.mean, color=hb.mean)) + 
  geom_point(size=5.5) +
  scale_color_viridis(option="rocket", discrete=FALSE) +
  #scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="black", size=1.2) + 
  theme_classic() + 
  labs(x="CeNS Index \n (0 = large MCV, 1 = high TRBC)", y="Elevation (m)") +
       # theme(axis.text.x = element_text(hjust = 1)) +
  theme(legend.position = "right") +
 # geom_text(aes(label = species), hjust = -0.1, alpha = 0.2) + # labels for MCV to verify x-axis
  # ggtitle("(a)") + # Assign panel number/header; this will be (a) because first in series of 3
...
# Based on this plot is *kind of* looks like high and low elevation species have different strategies. Let's try to pull
# out this relationship: 
# PC1 vs. Elev but rescaled - ELEV ON X AXIS, CENS ON Y
(p8 <- ggplot(cens.pca.spm.all, aes(x=elev.mean, y=hb.mean, color=PC1.s)) + 
  geom_point(size=5.5) +
  scale_color_viridis(option="plasma", discrete=FALSE, name="CeNS\nIndex") +
  #scale_colour_viridis(discrete=TRUE) +
  geom_smooth(method = "lm", colour="black", size=1.2) + 
  theme_classic() + 
...
# Plasma is best for this because it shows differentiation of middle values best 
...
##### OLD CODE BELOW HERE, HAVEN'T CLEANED OR ANTYHIGN YET




# # Plot output w/ elipses & biplot arrows. Colors = haplotype, shape = sex 
# g <- ggbiplot(pg.pca, obs.scale = 1, var.scale = 1, 
#               groups = pg.haplo, ellipse = TRUE, # groups = grouping variable for PCA output
#               circle = FALSE, varname.size=3, labels.size=3) 
# g <- g + scale_color_manual(name="Haplotype", values=c("#F39040", "#853FCD", "gainsboro")) # orange, purple
# g <- g + scale_shape_manual(name="Sex", values=c(17,19,18))
# g <- g + geom_point(aes(colour=pg.haplo, shape=pg.sex), size = 3)
# # g <- g + scale_color_discrete(name = '')
# g <- g + theme(legend.direction = 'vertical', 
#                legend.position = 'right')
# g <- g + theme(
...
#     panel.background = element_blank())
# g <- g + theme(axis.text=element_text(size=11, face="bold"))
# g <- g + ggtitle("PCA w/ Morphological Measurements (bill, bill + head, wing chord, tail length, tarsus)")
# # g <- g + xlim(-3, 3) + ylim(-3, 3)
# print(g)
# 
# 
# # just plot by sex 
# g <- ggbiplot(pg.pca, obs.scale = 1, var.scale = 1, 
#               groups = pg.sex, ellipse = TRUE, # groups = grouping variable for PCA output
#               circle = FALSE, varname.size=3, labels.size=3) 
# g <- g + scale_color_manual(name="Sex", values=c("magenta2", "mediumblue", "gainsboro")) 
# #g <- g + geom_point(aes(colour=pg.haplo, shape=pg.sex), size = 3)
# # g <- g + scale_color_discrete(name = '')
# g <- g + theme(legend.direction = 'vertical', 
#                legend.position = 'right')
...
#     panel.background = element_blank())
# g <- g + theme(axis.text=element_text(size=11, face="bold"))
# g <- g + ggtitle("PCA w/ Morphological Measurements (grouped by sex)")
# # g <- g + xlim(-3, 3) + ylim(-3, 3)
# print(g)


# Try subsetting by clade and *then* examining species differences: 
coq <- final[which(final$Clade == "Coquette"),] # Just Coquettes for starters

# PCA of MCV, TRBC, Hb # [rows, cols]
coq.sub <- coq[ , c("species", "Clade", "elev", "mass", "hb", "mcv", "trbc")] # subset desired variables 
coq.sub <- na.omit(coq.sub) # remove all NAs

# We'll apply PCA to continuous variables and use categorical variables for visualization
coq.log <- log(coq.sub[, c("hb", "mcv", "trbc")]) # Apply log transform as recommended
coq.sp <- coq.sub[, "species"]
# pca.clade <- pca.sub[, "Clade"]
 
# Apply PCA
coq.pca <- prcomp(coq.log, center = TRUE, scale. = TRUE) # Centering and scaling variables is highly advisable 
print(coq.pca) # get loadings; also achieved w/ pg.pca$rotation
summary(coq.pca) # Get importance of components (look at cumulative proportion); 
# First 2 PCs explain 98% of variance; first three explain 100%
...
# predict new PC values with new observed data
# predict(coq.pca, newdata=tail(coq.sub, 100))
biplot(coq.pca)
autoplot(coq.pca)

# Plot output w/ elipses & biplot arrows. Colors = haplotype, shape = country 
g <- ggbiplot(coq.pca, obs.scale = 1, var.scale = 1, 
              groups = coq.sp, ellipse = TRUE, # groups = grouping variable for PCA output
              circle = FALSE, varname.size=3, labels.size=3) 
#g <- g + scale_color_manual(name="Haplotype", values=c("#F39040", "#853FCD", "gainsboro")) # orange, purple
#g <- g + scale_shape_manual(name="Country", values=c(17, 19))
#g <- g + geom_point(aes(colour=pg.haplo, shape=pg.country), size = 3)
...
g <- g + theme(
    ...
    panel.background = element_blank())
g <- g + theme(axis.text=element_text(size=11, face="bold"))
g <- g + ggtitle("Coquette CeNS PCA")
# g <- g + xlim(-3, 3) + ylim(-3, 3)
print(g)


# Now try subsetting by Mangos since it's a small clade and we might be able to pull out differences: 
# Try subsetting by clade and *then* examining species differences: 
mango <- final[which(final$Clade == "Mango"),] # Just Mangos for starters

# PCA of MCV, TRBC, Hb # [rows, cols]
mango.sub <- mango[ , c("species", "Clade", "elev", "mass", "hb", "mcv", "trbc")] # subset desired variables 
mango.sub <- na.omit(mango.sub) # remove all NAs

# We'll apply PCA to continuous variables and use categorical variables for visualization
mango.log <- log(mango.sub[, c("hb", "mcv", "trbc", "elev")]) # Apply log transform as recommended
mango.sp <- mango.sub[, "species"]
# pca.clade <- pca.sub[, "Clade"]
 
# Apply PCA
mango.pca <- prcomp(mango.log, center = TRUE, scale. = TRUE) # Centering and scaling variables is highly advisable 
print(mango.pca) # get loadings; also achieved w/ pg.pca$rotation
summary(mango.pca) # Get importance of components (look at cumulative proportion); 
# First 2 PCs explain 98% of variance; first three explain 100%

# visualize how much variance is explained by top PCs
screeplot(mango.pca)
plot(mango.pca, type = "l")

# predict new PC values with new observed data
# predict(mango.pca, newdata=tail(mango.sub, 100))
biplot(mango.pca)
autoplot(mango.pca)

# Plot output w/ elipses & biplot arrows. Colors = haplotype, shape = country 
g <- ggbiplot(mango.pca, obs.scale = 1, var.scale = 1, 
              groups = mango.sp, ellipse = TRUE, # groups = grouping variable for PCA output
              circle = FALSE, varname.size=3, labels.size=3) 
#g <- g + scale_color_manual(name="Haplotype", values=c("#F39040", "#853FCD", "gainsboro")) # orange, purple
#g <- g + scale_shape_manual(name="Country", values=c(17, 19))
#g <- g + geom_point(aes(colour=pg.haplo, shape=pg.country), size = 3)
...
g <- g + theme(
    ...
    panel.background = element_blank())
g <- g + theme(axis.text=element_text(size=11, face="bold"))
g <- g + ggtitle("Mango CeNS PCA")
# g <- g + xlim(-3, 3) + ylim(-3, 3)
print(g)

# This is kind of the same of arrows plotted on PCA; tells you about direction of influence of variables
require(ggplot2)
theta <- seq(0,2*pi,length.out = 100)
circle <- data.frame(x = cos(theta), y = sin(theta))
p <- ggplot(circle,aes(x,y)) + geom_path()

loadings <- data.frame(mango.pca$rotation, 
                       .names = row.names(mango.pca$rotation))
p + geom_text(data=loadings, 
              mapping=aes(x = PC1, y = PC2, label = .names, colour = .names)) +
  coord_fixed(ratio=1) +
  labs(x = "PC1", y = "PC2")

# Another idea to pull out patterns might be to slim down the dataset to a few well-sampled species and then see
# whether there are species differences
# MCV and TRBC go in opposite directions in a PCA - could it be possible that some birds use MCV, some TRBC? 
# Can you only do one or the other? Are some species intermediate? 
# Try shading plots by elevation, body size, sex, etc - try to pull out differences

``` 

# CeNS PCA interpretation

**PCA Interpretation:**  
- The first 2 principal components explain `r signif(sum(summary(cens.pca)$sdev[1:2]^2) / sum(summary(cens.pca)$sdev^2), 4)` percent of the variance. This is good because it's easiest to plot dimensions in 2D. 

**PC1**
- PC`r i.print <- 1; signif(i.print,1)` explains `r signif(100*cens.pca$sdev[i.print]^2/sum(cens.pca$sdev^2), 3)`% 
of the total variation.
As PC`r signif(i.print,1)` increases, hb and trbc **increase**, while mcv **decreases**. 

**PC2**  
- PC`r i.print <- 2; signif(i.print,1)` explains `r signif(100*cens.pca$sdev[i.print]^2/sum(cens.pca$sdev^2), 3)`%
of the total variation.
As PC`r signif(i.print,2)` increases, hb, trbc, and mcv **decrease**. 





# Quick model with Hb response, MCV and TRBC as predictors
```{r}
# bi = blood index 

# In order to run hb ~ MCV + trbc, need to: 
# Use 'final' dataset; models will automatically remove NAs (alternatively, make a hb, mcv, trbc subset w/ no NAs)
# Models run ok w/out transformed MCV, but may be worth trying to run with transformed MCV 
# Models look ok without standardizing variables becuase numbers are relatively on the same scale
# But I might try re=running with standardized predictors to see if it improves fit
# Interaction (mcv*trbc) isn't important and overlaps zero
...
save(bi.1, file="bi.1_FullModel_PredictorsOnly.RData") # save model
# load("hb.m3_FullModel_PredictorsOnly.RData") # If loading from pre-saved file and not re-running
...
save(bi.m2, file="bi.m2_FullModel_SpeciesREOnly.RData") # save model

# # WITH INTERACTION AND SPECIES RANDOM EFFECT
bi.m3 <- brm(
  formula = bf(hb ~ 1 + mcv + trbc + mcv*trbc + (1|species)),
  data = final,
  family = gaussian(),
  cores = 4,
  chains = 4,
...
save(bi.m3, file="bi.m3_FullModel_Interaction_SpeciesRE.RData") # save model
```


# Blood index: Summarize and check fit 
Explanation of what mcmc_plot output below means: https://cran.r-project.org/web/packages/bayesplot/vignettes/plotting-mcmc-draws.html
```{r}
# FIT AND CONVERGENCE 
mcmc_plot(bi.1, type = "trace") 
plot(bi.1, N = 2, ask = FALSE)
bayesplot::pp_check(bi.1, resp = "hb", nsamples = 100) # pull 100 draws from model and cast back to original data 
bayesplot::pp_check(bi.1, type = "stat", stat = 'median', nsamples = 100)
mcmc_plot(bi.1, type = "acf_bar") # looks at autocorrelation
plot(conditional_effects(bi.1), points = TRUE) # conditional effects 

# Quick plot of estimates and 95% CIs (default is 95% CI:
mcmc_plot(bi.1)  # adjust prob to get 95% CI

# Residual plots 
# typical residual plot but w/ addition of uncertainty associated w/ each residual given by generating draws from the dist 
# associated with each residual
final %>%
  add_residual_draws(bi.1) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

final %>%
  add_predicted_draws(bi.1) %>%
  summarise(
    p_residual = mean(.prediction < hb), # probability residual; Bayesian predictive p-value 
    z_residual = qnorm(p_residual) # quantile residuals 
  ) %>%
...
#pairs(hb.m3)
color_scheme_set("blue") # nice default
bi.1.p1 <- mcmc_plot(bi.1, pars=c("b_mcv", "b_trbc"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "bi.1: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("MCV","TRBC"))  
bi.1.p1
ggsave(bi.1.p1, filename = "bi.1.p1_BloodIndex_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(bi.1)
# Both MCV and TRBC have strong positive effects on Hb: As MCV and TRBC increase, Hb also increases. 
# But efffect of trbc is ~15x stronger than MCV. 

# Model 4
#color_scheme_set("red") # nice default
bi.m2.p1 <- mcmc_plot(bi.m2, pars=c("b_mcv", "b_trbc"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "bi.m2: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("MCV","TRBC"))   
bi.m2.p1
ggsave(bi.m2.p1, filename = "bi.m2.p1_BloodIndex_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(bi.m2)

# Model 4
#color_scheme_set("red") # nice default
bi.m3.p1 <- mcmc_plot(bi.m3, pars=c("b_mcv", "b_trbc","b_mcv:trbc"),
                                    prob_outer=0.95, # 95% outer CI 
                                    prob=0.50, # 50% inner CI 
                                    point_est="mean") + # mean point est; default is median 
labs(x="Effect on [Hb]", title = "bi.m3: Posterior mean estimates & 95% Credible Intervals") + 
scale_y_discrete(labels=c("MCV","TRBC", "MCV*TRBC"))   
bi.m3.p1
ggsave(bi.m3.p1, filename = "bi.m3.p1_BloodIndex_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")
summary(bi.m3)
# Interaction of MCV*TRBC squarely overlaps zero, suggesting that it isn't important/there isn't really one

```


# Blood Index: Collate model summaries and WAIC scores 
```{r}
# Print model summaries 
sink("blood_index_brms_model_summaries.txt",append = TRUE)
summary(bi.1, waic=TRUE)
summary(bi.m2, waic=TRUE)
sink()
# print(hb.test, prob=0.95) # change CI cut-off by adjusting prob; remember Bayesian default may not be 95%

# Get model WAIC scores  
bi.m1.waic <- waic(bi.1)
bi.m2.waic <- waic(bi.m2)
bi.waics <- cbind(bi.m1.waic, bi.m2.waic); bi.waics
write.csv(bi.waics, "bi_brms_models_waics.csv") # totally unintelligible cumbersome doc w/ all pointwise comparisons 
```


# Blood Index: CALCULATE INTRACLASS CORRELATION (ICC) EXPLAINED BY SPECIES GROUPING VARIABLE 
Calculated from Burkner's phylo  brms tutorial: https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html
Note from Burkner: Note that the phylogenetic signal is just a synonym of the intra-class correlation (ICC) used in the context phylogenetic analysis.
```{R}
# CALCULATE INTRACLASS CORRELATION (ICC) EXPLAINED BY SPECIES GROUPING VARIABLE 
...
# bi.m2 
hyp.sp.bi.m2 <- "sd_species__Intercept^2 / (sd_species__Intercept^2 + sigma^2) = 0"
(hyp.sp.bi.m2 <- hypothesis(bi.m2, hyp.sp.bi.m2, class = NULL))
plot(hyp.sp.bi.m2)
# 12% variance explained 
...
# Blood index: MODEL COMPARISON 
```{r}
# leave one out cross-validation, lowest is "best"
# if 2*standard error > delta LOOIC, models aren't necessarily distinguishable

library(loo)
sink("blood_index_brms_models_all_looic.txt", append=FALSE)
LOO(bi.1, bi.m2, reloo=FALSE) 
sink()
# According to LOO, model 5 is top model 
...
- Model 2 (with random effect of species) fits best by far! Variance explained by species: 12%. R^2 = 0.53. 
...
I think we might want an interaction term of mcv*trbc since there is some kind of interaction here.
--> Interaction isn't important, overlaps zero

But since MCV IS calculated partially using TRBC, shouldn't we expect that these two are correlated? 
