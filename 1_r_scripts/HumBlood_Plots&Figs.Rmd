...
date: "7/21/2020; last revised 7/16/21"
output:
  md_document:
...
final <- read.csv("AllHummingbirdBlood_Wrangled_withBioClim_andPCA_06-14-21.csv")
final <- final[ , !(colnames(final) %in% c("X"))] # drop weird X column 1 if reading in from read.csv

spmeans <- read.csv("AllHummingbird_SpeciesMeanBloodData_06-14-21.csv")
final <- final[ , !(colnames(final) %in% c("X"))] # drop weird X column 1 if reading in from read.csv
...
smp.mchc <- read.csv("HumBlood_SpeciesMeans_spm.mchc_06-14-21.csv")
...
# Figure 2 - individual-level plots of predictors and elevation
Figure 2 - 6-panel fig w/ relationship between predictors and elevation 
```{r}
# Scatterplots of each of the 6 blood characteristics with elevation, colored by clade
...
```{r}
# Where did this code go? Either pull a version from Dropbox before 6/12/21 or...don't worry about it?
```
...
ggsave(Fig3_PanelA, filename="Fig3_PanelA_BananaHeatGradient_2021-07-16_Alpha.pdf", height=5, width=6, units="in") 
...
ggsave(Fig3_PanelB, filename="Fig3_PanelB_RBC_vs_MCV_Scatterplot_ColoredByElev_2021-07-16.pdf", height=7, width=9, units="in") 
...
ggsave(Fig3_BloodComparison_3panel, filename = "Fig3_BloodComparison_3panel_2021-07-16.pdf", bg="transparent", height=8, width=5, units="in")
...
fig4a_hb_inset <- mcmc_plot(spm.hb.m2, pars=c("b_elev.z", "b_temp.z", "b_precip.z", "b_mass.z", "b_genotypeSerMSer", 
            ...
            theme_bw() +  #theme with white background
            annotate('text', x = -1.38, y = 0.78, label='bold("*")', size=4, parse=TRUE) + # ASTERISK 4 top model sig
            theme(plot.background=element_blank(),  # eliminates background, grid lines, and chart border
                  panel.grid.major=element_blank(),  panel.grid.minor=element_blank(), panel.border=element_blank()) +
            theme(axis.line = element_line(color = 'black')) + # draws back x and y axis lines
...
fig4b_hct_inset <- mcmc_plot(spm.hct.m2, pars=c("b_elev.z", "b_temp.z", "b_precip.z", "b_mass.z", "b_genotypeSerMSer", 
            ...
            theme(plot.title = element_text(family = "Helvetica", color="black", size=9)) + # font = panel header size
            theme_bw() +  #theme with white background
            annotate('text', x = -5.58, y = 0.78, label='bold("*")', size=4, parse=TRUE) + # ASTERISK 4 top model sig
            theme(plot.background=element_blank(),  # eliminates background, grid lines, and chart border
                  panel.grid.major=element_blank(),  panel.grid.minor=element_blank(), panel.border=element_blank()) +
            theme(axis.line = element_line(color = 'black')) + # draws back x and y axis lines
            theme(axis.text=element_text(size=5), axis.title=element_text(size=5)) + # y-axis text, x-axis text
...
fig4f_mchc_main <- mcmc_plot(mchc.m4, pars=c("b_elev.z", "b_temp.z", "b_precip.z", "b_mass.z", "b_genotypeSerMSer", 
            "b_genotypeGlyMSer","b_elev.pos.z",  "b_intravar.temp.z", "b_intravar.precip.z", "b_intravar.mass.z"),
              ...
              annotate('text', x = -1.42, y = 7.878, label='bold("*")', size=4, parse=TRUE) + # bottom asterisk
              theme(plot.background=element_blank(),  # eliminates background, grid lines, and chart border
                    panel.grid.major=element_blank(),  panel.grid.minor=element_blank(), panel.border=element_blank()) +
              theme(axis.line = element_line(color = 'black')) + # draws back x and y axis lines
              theme(text = element_text(family="Helvetica", size=10)) + # y axis labels
...
ggsave(Fig4_v2_Indiv_and_SpeciesMean, filename = "Fig4_PosteriorProbPlots_Indiv_and_SpeciesMean_07-16-21.pdf", bg="transparent", height=6.8, width=12.5, units="in")
```
...
(fig5b <- ggplot(cens.ss.mean, aes(x=elev, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=2.8, alpha=1.0) + # cens.ss.mean$mcv/10 = to size by MCV but they don't look super variable
...
(fig5d <- ggplot(cens.ss.mean, aes(x=round(wl,2), y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=2.8, alpha=1.0) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
 # scale_colour_viridis(discrete=F) + 
 # scale_color_manual(values = hum_colors) +
...
ggsave(fig5d, filename="fig5d_CeNSindex_by_WingLoading_06-14-21.pdf", height=7, width=9, units="in")
...
ggsave(Figure5_CeNS_Results, filename = "Figure5_CeNS_Results_06-15-21.pdf", bg="transparent", height=6.8, width=12, units="in")
