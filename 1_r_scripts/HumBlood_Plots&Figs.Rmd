...
final.nopgig <- read.csv("HumBlood.final.nopgig_12-01-20.csv")
...
Fig2_BloodVElev_6panel <- grid.arrange(p1,p2,p3,p4,p5,p6, ncol=2)
ggsave(Fig2_BloodVElev_6panel, filename = "Fig2_BloodVElev_6panel.pdf", bg="transparent", height=8, width=7, units="in")

# To really make this better, need to adjust individual dimensions of panels - cut width by 1"
# Figure out how to add legend while keeping plot length the same in panel D (see comment below)
...
# Basic scatterplot of mass v. Hb 
(p4 <- ggplot(final.nopgig, aes(x=mass, y=hb)) + # uncenter Tamin values
  geom_point() +
  geom_smooth(method = "lm", colour="darkturquoise", size=1.2) + 
  theme_classic() + 
  labs(x="Mass (g)", y="[Hb] g/dl") +
