...
date: "7/21/2020; last revised 01/23/22"
output:
  md_document:
...
This script contains plots and figures for Williamson et al., Comparative Hummingbird Blood. These are separated into a standalone script to not bog down main scripts because some multi-panel figs are beefy. 
...
ggsave(Fig4_PanelB, filename="Fig4_PanelB_RBC_vs_MCV_Scatterplot_ColoredByElev_2021-11-19.pdf", height=7, width=9, units="in") 
...
#  geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
 ...
 # geom_jitter(width=0.5, height=0.5, alpha=0.8, size=1.1) + # jitter to fill plot space 
  ...
  ggtitle("") + # Assign panel number/header; this will be (a) because first in series of 3
  theme(plot.title.position = "plot", # parameter "plot" specifies that you want "title" flush with y-axis
         plot.title = element_text(face="bold")) + # This makes panel header bold 
       # This is good for labeling figure panels! Avoids having to manually toy w/ hjust and vjust
  theme(plot.margin = unit(c(0.0,0.0,0.0,0.0), "cm")) +  # top, right, bottom, left
...
ggsave(fig4e, filename="fig4e_CeNSindex_by_MeanElev_2022-01-22.pdf", height=7, width=7, units="in")
...
ggsave(Figure4_TradeoffAndCeNS_Results, filename = "Figure4_TradeoffAndCeNSResults_2022-01-22.pdf", bg="transparent", height=7.4, width=7.6, units="in")
...
# FIGURE S1: In `HumBlood_CeNS.Rmd` script. Sigle-species posterior probability plots. 


# FIGURE S2: supplementary CeNS results. 
```{r}

# PANEL A: Full model results
# Main: Best full CeNS model (M3) 
color_scheme_set("viridisC") # Used viridisD for other posterior prob plots (figure 4)
FigS2_A <- mcmc_plot(cens.m3, variable=c("b_wl.z", "b_mass.z", "b_elev.z", "b_Ielev.zE2"),
              prob_outer=0.95, # 95% outer CI
              prob=0.89, # 50% inner CI
              point_est="mean", # mean point est; default is median
              point_size=4, inner_size=3) + # Adjust line and point thickness for inset formatting
             ...
             #theme(plot.margin = unit(c(0.2,0.1,0.0,0.2), "cm")) +  # top, right, bottom, left
FigS2_A
ggsave(FigS2_A, filename = "FigS2A_CensIndex_PosteriorEstimates&CredibleIntervals.pdf", bg="transparent", height=7, width=9, units="in")


# PANEL B - CENS V. WING LOADING SCATTERPLOT
#cens.ss.mean$wl.rounded <- round(cens.ss.mean$wl, 2)
(FigS2_B <- ggplot(cens.ss.mean, aes(x=round(wl,2), y=cens.index)) + # Add shape=Clade here to get shapes to work 
  geom_point(size=1.8, alpha=0.8) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
 # scale_colour_viridis(discrete=F) + 
 # scale_color_manual(values = hum_colors) +
...
ggsave(FigS2_B, filename="FigS2_B_CeNSindex_by_WingLoading_2022-01-22.pdf", height=7, width=9, units="in")
...
# plot
(FigS2_C <- ggplot(cens.ss.mean, aes(x=mcv, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=1.8, alpha=0.8) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
  scale_colour_viridis(option="rocket", discrete=FALSE, name="TRBC") + 
 # scale_color_manual(values = hum_colors) +
...
ggsave(FigS2_C, filename="FigS2_C _CeNSindex_by_MCV_2022-01-22.pdf", height=7, width=9, units="in")
...
# PLot
(FigS2_D <- ggplot(cens.ss.mean, aes(x=trbc, y=cens.index)) + # Had previously colored by MCV
  geom_point(size=1.8, alpha=0.8) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
...
ggsave(FigS2_D, filename="FigS2_D_CeNSindex_by_TRBC_2022-01-22.pdf", height=7, width=9, units="in")
...
# Alternative layout option = BETTER, USE THIS ONE:
# Alternative layout option = BETTER, USE THIS ONE:
FigureS6_CensSupp <- ((FigS2_A + FigS2_B) / (FigS2_C + FigS2_D) ) #+
                                  #  plot_layout(widths=c(1,1), heights = unit(c(3,3), c('cm', 'cm', 'cm'))) 
                                # widths = says make col 1 have 2x the width of column 2
                                # heights = controlling size in cm, order: 1st row, 2nd row, 3rd row
plot(FigureS6_CensSupp)
ggsave(FigureS6_CensSupp, filename = "FigureS6_CensSupp_2022-01-22.pdf", bg="transparent", height=6.8, width=7.4, units="in")
```


# FIGURE S3: Boxplot of Hb genotype and CeNS index
Figure S3 in paper. The version of this plot in the HumBlood_CeNS script is for diagnostics. 
```{r}
# HB GENOTYPE DOES NOT AFFECT CENS INDEX
(FigureS3 <- ggplot(cens.ss.mean, aes(x=genotype, y=cens.index)) +
...
ggsave(FigureS3, filename="CensIndex_vs_Genotype_2022_01-22.pdf", height=5, width=7, units="in")
```


# FIGURE S4: in `HumBlood_CeNS.Rmd` script, contmap showing CeNS across the phylogeny


# FIGURE S5: CENS DIAGNOSTIC PLOT - correlations between R^2, TRBC estimates, and MCV estimates
...
(s5.1 <- ggplot(cens.ss.mean, aes(x=cens.R2, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
...
ggsave(s5.1, filename="CensIndex_Versus_R^2.pdf", height=7, width=9, units="in")
...
(s5.2 <- ggplot(cens.ss.mean, aes(x=cens.trbc, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=2.4, alpha=1.0) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
 # geom_smooth(method = "lm", colour="black", size=1.0) + # linear 
  theme_classic() + 
...
ggsave(s5.2, filename="CeNSIndex_Versus_StandardizedTRBCEstimate.pdf", height=7, width=9, units="in")
...
# Scatterplot: CeNS Index v. unstandardized cens trbc estimate   
(s5.3 <- ggplot(cens.ss.mean, aes(x=trbc.unst, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=2.4, alpha=1.0) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
 # geom_smooth(method = "lm", colour="black", size=1.0) + # linear 
  theme_classic() + 
...
ggsave(s5.3, filename="CeNSIndex_Versus_UnstandardizedTRBCEstimate.pdf", height=7, width=9, units="in")
# Coeligena coeligena is the super weird outlier point 
...
(s5.4 <- ggplot(cens.ss.mean, aes(x=cens.mcv, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=2.4, alpha=1.0) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
  geom_smooth(method = "lm", colour="black", size=1.0) + # linear 
  theme_classic() + 
...
ggsave(s5.4, filename="CeNSIndex_Versus_MCVEstimateStandardized.pdf", height=7, width=9, units="in")
...
# Scatterplot: CeNS Index v. unstandardized cens MCV estimate   
(s5.5 <- ggplot(cens.ss.mean, aes(x=mcv.unst, y=cens.index)) + # Add shape=Clade here to get shapes to work properly
  geom_point(size=2.4, alpha=1.0) +
 # geom_text(aes(label=species),hjust=0, vjust=0) + # UNCOMMENT TO ADD SPECIES LABELS TO POINTS
  geom_smooth(method = "lm", colour="black", size=1.0) + # linear 
  theme_classic() + 
...
ggsave(s5.5, filename="CeNSIndex_Versus_MCVEstimateUnstandardized.pdf", height=7, width=9, units="in")
...
library(patchwork)
# Alternative layout option = BETTER, USE THIS ONE:
FigureS5_5panel_CeNSDiagnostics <- (s5.1 | s5.2 + s5.3 + s5.4 + s5.5 +
                                    plot_layout(guides = "collect"))
plot(FigureS5_5panel_CeNSDiagnostics)
ggsave(FigureS5_5panel_CeNSDiagnostics, filename = "FigureS5_5panel_CeNSDiagnostics_2022-01-22.pdf", bg="transparent", height=6.8, width=12, units="in")
```



# FIGURE S6: SPECIES BEHAVE DIFFERENTLY AT EXTREME ELEVS
...
ggsave(FigureS6_PanelA, filename="FigureS6_PanelA_BananaHeatGradient_ExtremeValues_2022-01-11.pdf", height=5, width=6, units="in")

# Box plot: Elev cat vs. TRBC 
(FigureS6_PanelB <- ggplot(final.abovebelow.trbc, aes(x=elev_cat, y=trbc)) +
       # facet_wrap(~sex, nrow=2) + 
...
ggsave(FigureS6_PanelB, filename="AboveBelow_Boxplot_ElevCat-vs-TRBC_2022_01-22.pdf", height=7, width=9, units="in")

# Box plot: Elev cat vs. MCV
(FigureS6_PanelC <- ggplot(final.abovebelow.mcv, aes(x=elev_cat, y=mcv)) +
       # facet_wrap(~sex, nrow=2) + 
        geom_boxplot(aes(fill = factor(elev_cat)), outlier.size=0.8, alpha=0.7) +
        #geom_jitter(position = position_jitter(width = 0.01), alpha = 0.5, size = 2.0) +
        geom_signif(comparisons = list(c("<= 4,200", "> 4,200")), 
...
ggsave(FigureS6_PanelC, filename="AboveBelow_Boxplot_ElevCat-vs-MCV_2022_01-22.pdf", height=7, width=9, units="in")

# Box plot: Elev cat vs. MCH
(FigureS6_PanelD <- ggplot(final.abovebelow.mch, aes(x=elev_cat, y=mch)) +
       # facet_wrap(~sex, nrow=2) + 
        geom_boxplot(aes(fill = factor(elev_cat)), outlier.size=0.8, alpha=0.7) +
       # geom_jitter(position = position_jitter(width = 0.01), alpha = 0.5, size = 2.0) +
        geom_signif(comparisons = list(c("<= 4,200", "> 4,200")), 
...
ggsave(FigureS6_PanelD, filename="AboveBelow_Boxplot_ElevCat-vs-MCH_2022_01-22.pdf", height=7, width=9, units="in")
...
library(patchwork)
# Alternative layout option = BETTER, USE THIS ONE:
FigureS6_4panel_ExtremeSpecies <- (FigureS6_PanelA / (FigureS6_PanelB + FigureS6_PanelC + FigureS6_PanelD) 
                                    )
plot(FigureS6_4panel_ExtremeSpecies)
ggsave(FigureS6_4panel_ExtremeSpecies, filename = "FigureS6_4panel_ExtremeSpecies_2022-01_22.pdf", bg="transparent", height=8.0, width=5.8, units="in")
...
ggsave(FigureS6_PanelA, filename="FigS6_PanelA_BananaHeatGradient_ExtremeValues_ForFig6Merge_2022-01-22.pdf", height=4, width=6, units="in")

# Join with: 
FigureS6_BottomPanel <- (FigureS6_PanelB + FigureS6_PanelC + FigureS6_PanelD)
plot(FigureS6_BottomPanel)
ggsave(FigureS6_BottomPanel, filename = "FigureS6_BottomPanel_2022-01_22.pdf", bg="transparent", height=4.4, width=6, units="in")
...
# FIGURE S7: BREAK POINT ANALYSIS and mondo figure 
...
# FIGURE S7: COMBINED INDIVIDUAL-LEVEL AND SPECIES MEAN DATA IN ONE MASTER PLOT 
Figure S7 - 6-panel fig w/ relationship between predictors and elevation 
```{r}
# IMPORTANT! Read in 'final' dataset, set levels and colors for hum_colors above
...
print(FigS7_BreaksnBox)
ggsave(FigS7_BreaksnBox, filename = "FigS7_BreaksnBox_BreakpointPlotsAndBoxPlots_2022-01-22.pdf", bg="transparent", height=9, width=9, units="in")
...
# FIGURE S8: 6-panel figure showing low phylogenetic signal for all mean blood trait values. See Illustrator file where I combined outputs. 


# FIGURE S9: "SIGNIFICANT" CLADE*ELEV INTERACTION FOR [HB]
```{R}
# Load in individual-level models for plots 
...
color_scheme_set("viridisD")
FigureS9 <- mcmc_plot(hb.m4.ce, variable=c("b_elev.z", "b_elev.pos.z","b_mass.z","b_temp.z","b_precip.z",
...
FigureS9
ggsave(FigureS9, filename = "FigureS9_HbModel_CladeVElevInteraction_PosteriorProbs_2022-01-22.pdf", bg="transparent", height=7, width=9, units="in")
summary(hb.m4.ce)
```
